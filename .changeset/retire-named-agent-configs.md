...
Retires named agent configurations. A workspace's AI setup is now exactly one model binding per purpose — practice detection and mentor — edited on the AI setup page, instead of a list of named configs plus separate pointers designating which one each feature used. Existing configurations were migrated to bindings automatically, so detection and the mentor keep running on the model they already used.
