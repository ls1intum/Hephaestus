...
**Operators:** the `/agent-configs` endpoints and `PUT /ai-settings/practice-config` / `PUT /ai-settings/mentor-config` are gone; use `GET|PUT|DELETE /workspaces/{workspaceSlug}/agents/{purpose}`. Agent jobs report the model they ran on rather than a config name. Any script calling the removed endpoints must be updated before upgrading.
