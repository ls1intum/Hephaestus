...
sandbox that has only just started, and keeps a network as long as a container still uses it. Practice
reviews are off by default, so no instance running the shipped defaults could have observed it.
