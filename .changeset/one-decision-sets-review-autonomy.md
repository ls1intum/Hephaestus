---
"hephaestus": minor
---

A practice can now be turned down without being turned off, and it is one decision instead of one per
practice. A workspace says how much Hephaestus does on its own — **Off**, **Observe** (it reviews and
records everything, and tells nobody) or **Deliver** (it sends the feedback) — and every area and
practice follows that until you say otherwise. An area can override the workspace, a practice can
override its area, and anything you have not decided shows as inherited, naming the level it came from
and offering a way to reset it.

Turning a practice down to Observe keeps its measurements unbroken, which turning it off does not: a
practice that stops being reviewed leaves a gap in its own history that later reads as a change in the
team's behaviour.

A new **Review autonomy** page under Administration → Practices summarises how many practices sit at
each setting, per area and overall, so you can see what a workspace is actually doing without opening
a hundred rows — and change a whole area, or a filtered selection, in one action.

Where feedback goes is a separate workspace setting — the mentor conversation only, or also on the
work itself — rather than being mixed into the per-practice choice. Narrowing it never makes a quiet
practice speak.

Nothing gets louder or quieter on upgrade: a practice that was used in new reviews reviews and
delivers as before, and one that was not stays off.
