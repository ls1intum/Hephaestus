...
**Operators:** set `HEPHAESTUS_LLM_DISPLAY_CURRENCY=EUR` to switch this on (optional; unset by default, and leaving it unset changes nothing). Once set, the application server fetches the ECB's free daily reference rates once each weekday — no API key, and no outbound request from the worker or webhook containers.
