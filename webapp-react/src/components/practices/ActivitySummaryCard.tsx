...
export function ActivitySummaryCard({
  ...
  return (
    <Card className="mb-4">
      <CardHeader>
        <CardTitle><Activity className="inline mr-2 h-4 w-4" /> Activity Summary</CardTitle>
        <CardDescription>
        <p className="text-muted-foreground">
          {userLabel} <span className="font-medium">{numberOfPullRequests} {prText}</span> with{" "}
          <span className="font-medium text-github-success-foreground">{numberOfGoodPractices} {goodText}</span> and{" "}
          <span className="font-medium text-github-attention-foreground">{numberOfBadPractices} {badText}</span>.
        </p>
        </CardDescription>
      </CardHeader>
      {currUserIsDashboardUser && (
