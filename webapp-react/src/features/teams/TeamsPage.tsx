...
export function TeamsPage({ teams, isLoading }: TeamsPageProps) {
  ...
  return (
    <section className="max-w-4xl mx-auto p-5">
      ...
      <p className="text-muted-foreground text-sm mb-4">Overview of contributors across different teams</p>
      ...
      {isLoading && (
        <>
          {Array(3).fill(null).map((_, index) => (
            <Card key={`loading-team-${index}`} className="flex flex-col mb-8 gap-3">
              <CardHeader>
                <Skeleton className="h-6 w-1/4" />
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-4 self-center">
                  {Array(4).fill(null).map((_, memberIndex) => (
                    <div key={`loading-member-${memberIndex}`} className="flex flex-col items-center text-center">
                      <div className="w-20 h-20 rounded-full flex items-center justify-center overflow-hidden">
