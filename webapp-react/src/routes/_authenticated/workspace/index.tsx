...
import { Github, PlusIcon, RefreshCw, TrashIcon } from "lucide-react";
...
function Workspace() {
	...
	const addRepositoryMutation = useMutation({
		...
		onSuccess: () => {
			...
			queryClient.invalidateQueries({
				queryKey: getRepositoriesToMonitorQueryKey({}),
			});
		...
		onError: (error) => {
			toast.error(
				`Failed to add repository: ${error.message || "Unknown error"}`,
			);
		},
	});
	...
	const removeRepositoryMutation = useMutation({
		...
		onError: (error) => {
			toast.error(
				`Failed to remove repository: ${error.message || "Unknown error"}`,
			);
		},
	});
			...
			<div className="flex flex-col gap-6">
				...
				<Card>
					<CardHeader>
						...
						<CardDescription>
							Manage the GitHub repositories that Hephaestus monitors for your
							account.
						</CardDescription>
					</CardHeader>
					<CardContent>
						{error ? (
							...
							<div className="text-center py-8 text-muted-foreground">
								You are not monitoring any repositories yet. Add your first
								repository below.
							</div>
						) : (
											...
											<AlertDialog>
												<AlertDialogTrigger asChild>
													<Button
														...
														size="sm"
														onClick={() =>
															setRepositoryToDelete({ owner, name })
														}
													>
												...
												<AlertDialogContent>
													<AlertDialogHeader>
														...
														<AlertDialogDescription>
															This will remove {owner}/{name} from monitoring.
