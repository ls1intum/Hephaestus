...
3. seed-loader pipes the dump into the preview postgres via `psql` using `docker exec -i $SERVICE_NAME_POSTGRES` (no DNS needed).
...
4. **seed-loader restores dump** via `psql` into preview postgres using `docker exec -i $SERVICE_NAME_POSTGRES` (no DNS/host resolution).
