...
Hephaestus is a self-hosted web platform that gives software-engineering contributors feedback on their collaborative work. Anyone with a GitHub or LRZ-GitLab account can sign in and create a workspace; workspace administrators are not restricted to TUM units. The administrator chooses which Git repositories the workspace synchronises, the catalogue of practices the platform checks for, the LLM provider used for AI-assisted feedback, whether practice reviews are auto-triggered on new pull/merge requests, whether anonymised cohort practice insights are visible to mentors only or to all workspace members, and whether to enable optional features (achievements, Slack notifications). Practice-review sandboxing of repository code, AI-assisted practice reviews, achievements, and Slack notifications are off by default and have to be turned on by the administrator.
...
- **Salesforce, Inc. / Slack Technologies, LLC** (USA) — workspace notifications and digests, only when your workspace administrator has enabled Slack.
...
For each workspace, the administrator chooses which Git repositories are connected, which LLM provider and credentials are used, whether Slack notifications are enabled, whether achievements are enabled, the workspace's catalogue of practices, the cohort-practice visibility tier, and whether practice reviews are auto-triggered on new pull/merge requests. In respect of these choices, the workspace administrator and TUM/AET jointly determine the purposes and means within the meaning of Art. 26 GDPR. The essence of that arrangement is:
