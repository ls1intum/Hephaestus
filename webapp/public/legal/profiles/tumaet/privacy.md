...
| **Account preferences and recognition signals** — notification preferences, UI options, workspace memberships, achievement/recognition signals (only when enabled in the workspace), cohort-practice visibility settings, and the "AI review comments" Art. 21 objection switch | Personalised feedback surfaces; honour your Art. 21 objection | Art. 6(1)(e) i.V.m. Art. 2 BayHIG (Allgemeine Aufgaben), Art. 4(1) BayDSG | While the account exists |
...
For each workspace, the administrator chooses which Git repositories are connected, which LLM provider and credentials are used, whether the Slack integration and monitored channels are enabled, whether achievements are enabled, the workspace's catalogue of practices, the cohort-practice visibility tier, and whether practice reviews are auto-triggered on new pull/merge requests. In respect of these choices, the workspace administrator and TUM/AET jointly determine the purposes and means within the meaning of Art. 26 GDPR. The essence of that arrangement is:
