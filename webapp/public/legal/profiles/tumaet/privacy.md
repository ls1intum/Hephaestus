...
> **In plain language.** Hephaestus is a feedback tool for software-engineering work. A workspace administrator connects one or more Git repositories from GitHub or LRZ-GitLab. The platform reads the pull/merge requests, issues, code reviews, review comments, and commit metadata authored in those repositories and gives the people who wrote them feedback on engineering practices, for example whether a pull request is small enough to review well or whether a review reply addresses the question raised. Some feedback is generated by an external large-language-model (LLM) provider chosen by the workspace administrator. The platform does not grade you or decide anything about employment or course access. You can stop new practice-feedback comments on your pull/merge requests in your profile, but that setting controls comment delivery only: the review still runs and its findings are stored.
...
| **Account preferences and recognition signals** — notification preferences, UI options, workspace memberships, leaderboard rank / league / achievements (only when enabled in the workspace), the pull/merge-request practice-feedback comment-delivery setting | Personalised feedback surfaces; remember whether new pull/merge-request practice-feedback comments should be posted | Art. 6(1)(e) i.V.m. Art. 2 BayHIG (Allgemeine Aufgaben), Art. 4(1) BayDSG | While the account exists |
| **AI-assisted feedback data** — your messages with the in-app conversational mentor and its replies; for automated practice review: the content and surrounding discussion of the pull/merge request or issue, including the diff for a pull/merge request, and the resulting findings | Generate practice feedback and answer requests in the mentor chat | TUM Contributors: Art. 6(1)(e) i.V.m. Art. 2 BayHIG (Allgemeine Aufgaben), Art. 4(1) BayDSG. Non-TUM voluntary sign-in: Art. 6(1)(b) | While the account exists. Turning off pull/merge-request practice-feedback comments changes delivery only; it does not stop the review or delete past findings. Use the rights process in section 7 to object to processing or request erasure |
...
Providing personal data is neither legally nor contractually required, but the platform cannot be used without authenticating through one of the federated identity providers, and that requires the identity attributes listed in section 3. If your workspace has AI-assisted feedback active, the corresponding processing of repository activity is part of how the workspace operates. The **Pull request comments** setting stops new comments on your pull/merge requests but does not stop review processing or transmission to the LLM provider. To exercise your Art. 21 right to object to processing, use the contact path in section 7.
