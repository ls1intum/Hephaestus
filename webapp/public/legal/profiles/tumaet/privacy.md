...
Hephaestus is a self-hosted web platform that gives software-engineering contributors feedback on their collaborative work. Anyone with a GitHub or LRZ-GitLab account can sign in and create a workspace; workspace administrators are not restricted to TUM units. The administrator chooses which Git repositories the workspace synchronises, the catalogue of practices the platform checks for, the LLM provider used for AI-assisted feedback, whether practice reviews are auto-triggered on new pull/merge requests, whether anonymised workspace practice insights are visible to mentors only or to all workspace members, and whether to enable optional features (achievements, Slack integration). Practice-review sandboxing of repository code, AI-assisted practice reviews, achievements, and Slack channel monitoring are off by default and have to be turned on by the administrator.
...
| **Account preferences and recognition signals** — notification preferences, UI options, workspace memberships, achievement/recognition signals (only when enabled in the workspace), workspace health visibility settings, and the "AI review comments" Art. 21 objection switch | Personalised feedback surfaces; honour your Art. 21 objection | Art. 6(1)(e) i.V.m. Art. 2 BayHIG (Allgemeine Aufgaben), Art. 4(1) BayDSG | While the account exists |
...
**Access and portability (Art. 15, Art. 20).** Hephaestus provides a self-service JSON export for signed-in contributors in the account settings. The export covers the Hephaestus account, federated identity links, workspace memberships, account preferences, and the contributor's own authentication-event history; it excludes credentials and signing/session material. AET operators add data outside that export scope on a verified Art. 15 request where lawful and proportionate, including contributor profile data, AI-assistant conversations, practice findings and feedback, recognition signals, disclosure-audit rows showing privileged access to your practice-report data, analytics identity where PostHog is activated, and access-log entries within the active 14-day retention window. Source-side content (commits, pull/merge requests, issues, reviews on GitHub or gitlab.lrz.de) is exported by those source platforms, not by Hephaestus. Identity verification is the same as for erasure (below).
...
For each workspace, the administrator chooses which Git repositories are connected, which LLM provider and credentials are used, whether the Slack integration and monitored channels are enabled, whether achievements are enabled, the workspace's catalogue of practices, the workspace health visibility tier, and whether practice reviews are auto-triggered on new pull/merge requests. In respect of these choices, the workspace administrator and TUM/AET jointly determine the purposes and means within the meaning of Art. 26 GDPR. The essence of that arrangement is:
