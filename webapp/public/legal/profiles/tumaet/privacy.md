...
**Retention:** container logs are subject to a deterministic per-container size cap enforced by the Docker `json-file` log driver: `max-size=50m` × `max-file=5` (≤ 250 MB per service) for application services in `docker/compose.app.yaml`, and `max-size=10m` × `max-file=3` (≤ 30 MB per service) for core infrastructure in `docker/compose.core.yaml`. Once a container exceeds its cap, the oldest log file is deleted automatically by the daemon. The cap is part of the deployment configuration itself — there is no manual host-level configuration. Logs containing an IP address are retained beyond this size cap only where strictly necessary to investigate a specific, ongoing security incident, and are then deleted as soon as the incident is closed. Logs are not merged with other data sources and are only accessible to AET operators.
