...
import {
	...
	removeFeedbackRatingMutation as removeFeedbackRatingMutationOptions,
	setFeedbackRatingMutation,
...
function PracticeAreaDetail() {
	...
	const removeFeedbackRatingMutation = useMutation({
		...removeFeedbackRatingMutationOptions(),
		onSuccess: invalidateReviewHistory,
		onError: (error) =>
		...
		<PracticeAreaDetailPage
			...
			onRateFeedback={(feedbackId, helpful) => {
				...
				rateFeedbackMutation.mutate({
					...
					// The row offers a thumb up or down; the API also knows INCORRECT and an explanatory
					// comment, which this surface does not collect yet.
					body: { state: helpful ? "HELPFUL" : "UNHELPFUL" },
				});
			}}
			pendingFeedbackId={
