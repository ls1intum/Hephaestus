 ...
 * - GitLab / non-SCM providers (e.g. SLACK): avatars are instance-specific and must come from the
 *   API. Returns `null` so callers render a fallback (e.g. initials).
 */
export function getWorkspaceAvatarUrl(
	...
	// (SLACK) never back a workspace's SCM avatar, so fall back to initials rather than a bogus link.
	return null;
}
