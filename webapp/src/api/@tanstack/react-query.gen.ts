...
export const getPrivateViewQueryKey = (options: Options<GetPrivateViewData>) => createQueryKey('getPrivateView', options, false, ['Private Feedback']);
 ...
 */
export const getPrivateViewOptions = (options: Options<GetPrivateViewData>) => queryOptions<GetPrivateViewResponse, DefaultError, GetPrivateViewResponse, ReturnType<typeof getPrivateViewQueryKey>>({
    queryFn: async ({ queryKey, signal }) => {
        const { data } = await getPrivateView({
            ...options,
            ...queryKey[0],
            signal,
            throwOnError: true
    ...
    queryKey: getPrivateViewQueryKey(options)
});

export const getReflectionFeedbackQueryKey = (options: Options<GetReflectionFeedbackData>) => createQueryKey('getReflectionFeedback', options, false, ['Reflection Feedback']);
 ...
 * Process-level messages prepared for the authenticated developer: for each habit that recurs in their work, what the pattern is, the pieces of work it was observed on, and one thing to try next. Distinct from in-context notes (which say what is wrong in one diff) and from the mentor conversation (which asks rather than tells). Reading a message is what delivers it, so this GET records the delivery.
 */
export const getReflectionFeedbackOptions = (options: Options<GetReflectionFeedbackData>) => queryOptions<GetReflectionFeedbackResponse, DefaultError, GetReflectionFeedbackResponse, ReturnType<typeof getReflectionFeedbackQueryKey>>({
    queryFn: async ({ queryKey, signal }) => {
        const { data } = await getReflectionFeedback({
            ...options,
            ...queryKey[0],
            signal,
            throwOnError: true
    ...
    queryKey: getReflectionFeedbackQueryKey(options)
});
