...
/**
 * Derived practice-area standings for the current developer
 *
 * Aggregates the authenticated developer's latest-run observations across every active practice area into qualitative standings, with supporting feedback attached so each standing stays traceable to real feedback. Without a displayable observation the standing reports why: NOT_OBSERVED or NO_OPPORTUNITY.
 */
export const getPracticeAreaStandings = <ThrowOnError extends boolean = false>(options: Options<GetPracticeAreaStandingsData, ThrowOnError>) => (options.client ?? client).get<GetPracticeAreaStandingsResponses, unknown, ThrowOnError>({
    responseTransformer: getPracticeAreaStandingsResponseTransformer,
...
/**
 * Respond to delivered feedback
 *
 * Appends a response event and returns the recipient's current response.
 */
export const submitFeedbackResponse = <ThrowOnError extends boolean = false>(options: Options<SubmitFeedbackResponseData, ThrowOnError>) => (options.client ?? client).post<SubmitFeedbackResponseResponses, SubmitFeedbackResponseErrors, ThrowOnError>({
    responseTransformer: submitFeedbackResponseResponseTransformer,
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/workspaces/{workspaceSlug}/practices/feedback/{feedbackId}/response',
    ...options,
    headers: {
