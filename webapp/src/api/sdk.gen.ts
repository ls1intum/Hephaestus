...
/**
 * The current developer's private view
 *
 ...
 */
export const getPrivateView = <ThrowOnError extends boolean = false>(options: Options<GetPrivateViewData, ThrowOnError>) => (options.client ?? client).get<GetPrivateViewResponses, unknown, ThrowOnError>({
    responseTransformer: getPrivateViewResponseTransformer,
    url: '/workspaces/{workspaceSlug}/practices/feedback/profile',
    ...options
});

/**
 * The current developer's reflection feedback
 *
 * Process-level messages prepared for the authenticated developer: for each habit that recurs in their work, what the pattern is, the pieces of work it was observed on, and one thing to try next. Distinct from in-context notes (which say what is wrong in one diff) and from the mentor conversation (which asks rather than tells). Reading a message is what delivers it, so this GET records the delivery.
 */
export const getReflectionFeedback = <ThrowOnError extends boolean = false>(options: Options<GetReflectionFeedbackData, ThrowOnError>) => (options.client ?? client).get<GetReflectionFeedbackResponses, unknown, ThrowOnError>({
    responseTransformer: getReflectionFeedbackResponseTransformer,
