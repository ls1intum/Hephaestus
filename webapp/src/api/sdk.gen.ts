 ...
 *
 * Aggregates the authenticated developer's latest-run findings across every active practice area into qualitative statuses, with supporting feedback items attached so each status stays traceable to real feedback. NO_DATA when an area has no displayable findings for the caller.
 */
export const getPracticeAreaStatuses = <ThrowOnError extends boolean = false>(options: Options<GetPracticeAreaStatusesData, ThrowOnError>) => (options.client ?? client).get<GetPracticeAreaStatusesResponses, unknown, ThrowOnError>({ url: '/workspaces/{workspaceSlug}/practice-areas/status', ...options });
