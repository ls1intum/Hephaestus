...
function HealthCell({ area }: { area?: AreaHealth }) {
	if (!area || area.availability === "NO_DATA") {
		return (
			<span
				role="img"
	...
	return (
		...
		>
			{(area.developingCount ?? 0) > 0 && <StatusDot status="DEVELOPING" className="size-1.5" />}
			{(area.mixedCount ?? 0) > 0 && <StatusDot status="MIXED" className="size-1.5" />}
			{(area.strengthCount ?? 0) > 0 && <StatusDot status="STRENGTH" className="size-1.5" />}
		</span>
	);
}
 ...
 * dims every other column; clicking a row opens the drill-down. The muted first row shows
 * anonymous workspace health per area, including suppression. Needs-attention developers come
 * first because the server sorts them first: the order is a triage aid, never a ranking.
 */
export function AreaMatrix({ roster, health, onOpenDeveloper }: AreaMatrixProps) {
	...
	const healthByArea = new Map(health.map((area) => [area.areaSlug, area]));
	...
	return (
		<div className="w-full overflow-x-auto rounded-lg border">
			<div className="min-w-fit">
				...
				{health.length > 0 && (
					<div
						className="grid items-center border-b bg-muted/20 px-2 py-1.5"
						style={gridTemplate(columns.length)}
					>
						...
						{columns.map((column) => (
							<div
								key={column.areaSlug}
								className={cn(areaFilter && areaFilter !== column.areaSlug && "opacity-40")}
							>
								<HealthCell area={healthByArea.get(column.areaSlug)} />
							</div>
						))}
					</div>
				)}
