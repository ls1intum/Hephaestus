...
export function PracticeDefinitionForm(props: PracticeDefinitionFormProps) {
	const { mode, areas, isPending, isSubmitDisabled, afterFields, cancelAction, initialData } =
		props;
			...
			<div className="max-w-3xl space-y-8">
				...
				{form.artifactType !== "CONVERSATION_THREAD" && (
					<>
						...
						>
							...
							{triggerError && <FieldError id="practice-trigger-error">{triggerError}</FieldError>}
						</FieldSet>
						<Separator />
					</>
				)}
				...
				<section>
					<Field data-invalid={criteriaError ? "true" : undefined}>
						...
						/>
						{criteriaError && <FieldError id="practice-criteria-error">{criteriaError}</FieldError>}
					</Field>
				</section>
				...
				<Collapsible open={showAdvanced} onOpenChange={setShowAdvanced}>
					<CollapsibleTrigger
						render={
							<Button type="button" variant="ghost" className="group -ml-3 text-lg font-semibold" />
						}
					>
					...
					<CollapsibleContent className="mt-4 space-y-4">
						<p className="text-sm text-muted-foreground">
							Optional TypeScript that runs static analysis before a review and provides structured
							context.
						</p>
						<CodeEditor
