...
function priceValueOf(model: WorkspaceLlmModel | null): PriceModeValue {
	return {
		...
		per1mReasoningUsd: model?.per1mReasoningUsd,
...
export function WorkspaceLlmModelFormDialog({
	...
	const handleSubmit = (event: React.FormEvent) => {
		...
		const shared = {
			...
			priceNote: price.pricingMode === "FREE" ? price.note?.trim() : undefined,
		};
		...
		onCreate({ ...shared, slug: slug.trim() } satisfies CreateWorkspaceLlmModelRequest);
	};
				...
				<form onSubmit={handleSubmit} className="space-y-4" noValidate>
					...
					{!isEdit && (
						<Field data-invalid={Boolean(errors.slug)}>
							<FieldLabel htmlFor="wm-slug">Slug</FieldLabel>
							<Input
								id="wm-slug"
								value={slug}
