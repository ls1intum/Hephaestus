...
/**
 * The one shared visual vocabulary for status and trend across all three candidates.
...
export function StatusDot({ status, className }: { status: PracticeStatus; className?: string }) {
...
export function StatusChip({ status, className }: { status: PracticeStatus; className?: string }) {
	const meta = STATUS_META[status];
	return (
		<span
			className={cn(
...
export function TrendGlyph({ trend, className }: { trend: PracticeTrend; className?: string }) {
...
export function TrendNote({ trend, className }: { trend: PracticeTrend; className?: string }) {
	const meta = TREND_META[trend];
	if (!meta) return null;
	const Icon = meta.icon;
	return (
...
const ARTIFACT_ICON: Record<
	ArtifactKind,
	Record<ArtifactState, { icon: LucideIcon; toneClassName: string; label: string }>
> = {
	PULL_REQUEST: {
		OPEN: {
...
export function ArtifactStateIcon({
	kind,
	state,
	className,
}: {
	...
	const meta = ARTIFACT_ICON[kind][state];
	const Icon = meta.icon;
	return (
		<span
			role="img"
