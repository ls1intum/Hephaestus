...
export function ActivityBadges({
	...
	const reviewActivityBadges = activityBadgeMetadata.filter((item) => item.primaryReviewSignal);
	const contextActivityBadges = activityBadgeMetadata.filter((item) => !item.primaryReviewSignal);
	const hasReviewActivity =
		reviewedPullRequests.length > 0 || reviewActivityBadges.some((item) => counts[item.key] > 0);
	const hasContextActivity = contextActivityBadges.some((item) => counts[item.key] > 0);

	const hasActivity = hasReviewActivity || hasContextActivity;
	...
	return (
		<div className={cn("flex items-center gap-2", className)}>
			<span className="sr-only">
				Activity badges. Review activity appears first. The rest is shown for context.
			</span>
	...
	return (
		<Tooltip>
			<TooltipTrigger
				...
				render={<button type="button" className={cn("flex items-center gap-1", item.colorClass)} />}
			>
