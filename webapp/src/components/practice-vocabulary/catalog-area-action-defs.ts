import { CircleAlert, CircleCheck, CornerDownLeft, Plus } from "lucide-react";
import type { CatalogAreaPracticeAction } from "@/api/types.gen";
import type { StatusDefs } from "./status-def";

export type CatalogAreaAction = CatalogAreaPracticeAction["action"];

/**
 * What adding an area does to each practice in it. Every row states its own outcome from here, so
 * the panel needs no prose explaining which of four lists a name ended up in.
 */
export const CATALOG_AREA_ACTION_DEFS: StatusDefs<CatalogAreaAction> = {
	ADD: {
		label: "Adds",
		icon: Plus,
		badgeVariant: "secondary",
		description: "A new copy this workspace owns.",
	},
	MOVE_TO_AREA: {
		label: "Moves back",
		icon: CornerDownLeft,
		badgeVariant: "secondary",
		description: "An unassigned copy returns here. Local edits are kept.",
	},
	KEEP: {
		label: "Already here",
		icon: CircleCheck,
		badgeVariant: "outline",
		description: "Nothing changes.",
	},
	BLOCKED: {
		label: "Blocked",
		icon: CircleAlert,
		badgeVariant: "warning",
		description: "Another practice already uses this name.",
	},
};

/** The actions that change the workspace, so a preview can count them without re-listing them. */
export const CATALOG_AREA_CHANGE_ACTIONS: readonly CatalogAreaAction[] = ["ADD", "MOVE_TO_AREA"];
