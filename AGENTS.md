...
### 2.1 GitHub Copilot Agent Environment
The repository includes a `.github/copilot-environment.yml` configuration that automatically sets up the complete development environment for GitHub Copilot agents. This ensures all tools and dependencies are properly configured for smooth agent sessions.

**What the environment provides:**
- Node.js 22.10.0 (exact version from `.node-version`)
- Java 21 (OpenJDK via Eclipse Temurin/Adoptium)
- Python 3.13 with Poetry 2.x
- Docker for database operations
- Pre-installed npm dependencies
- Bootstrapped Python virtual environments for both services
- Pre-configured environment variables for development

**Environment validation:**
After the environment is set up, verify all tools are available:
```bash
node --version    # Should show v22.10.0
java -version     # Should show version 21
python3 --version # Should show 3.13.x
poetry --version  # Should show 2.0.0 or later
docker --version  # Should be available
```

### 2.2 Local Development Setup
For local development outside of GitHub Copilot, ensure you have:
...
- **Java**: JDK 21 (see `pom.xml`). Maven wrapper is checked in; **always run builds through `./mvnw`** (Maven wrapper) instead of direct `mvn` commands.
...
| Application-server tests | Use Maven groups to mirror CI: `cd server/application-server && ./mvnw test -Dgroups=unit`, `./mvnw test -Dgroups=integration`, `./mvnw test -Dgroups=architecture`. |
| Application-server compile | `cd server/application-server && ./mvnw compile -DskipTests` |
| Application-server verify | `cd server/application-server && ./mvnw verify` (runs tests + package) |
| Intelligence service lint/type check | `cd server/intelligence-service && poetry run black --check .`, `poetry run flake8 .`, `poetry run mypy .` |
| Webhook ingest lint | `cd server/webhook-ingest && poetry run black --check .` and `poetry run flake8 .` |

**Important: Always use `./mvnw` (Maven wrapper) instead of `mvn`** — the Maven wrapper ensures the correct Maven version is used and is the standard way to run Maven commands in this project.

### 3.1 Common Development Commands
...
# Bootstrap Python environments (requires Poetry)
...
```bash
# Format all code (Java + Python + TypeScript)
npm run format

# Format specific parts
npm run format:java
npm run format:client  # TypeScript/webapp
npm run format:server  # Java + Python
```
...
# Lint specific parts
npm run lint:java
npm run lint:client    # TypeScript + type checking
npm run lint:server    # Java + Python
```

**Testing:**
```bash
# Webapp tests
npm --workspace webapp run test

# Application server tests (by category)
cd server/application-server
./mvnw test -Dgroups=unit
./mvnw test -Dgroups=integration
./mvnw test -Dgroups=architecture

# Run all application server tests
./mvnw test
```
...
# Compile application server
cd server/application-server && ./mvnw compile -DskipTests

# Full application server build (compile + test + package)
cd server/application-server && ./mvnw verify
```
...
- **Always use `./mvnw` instead of `mvn`** — the Maven wrapper ensures the correct Maven version is used consistently across all environments.
...
#### Poetry not found
**Problem:** `poetry: command not found` when running `npm run bootstrap:py`

**Solution:**
```bash
# Install Poetry 2.x
pip3 install poetry==2.0.0 --user
...
**Problem:** Poetry reports "The currently activated Python version X.Y.Z is not supported by the project (>=3.13,<4.0.0)"

**Solution:**
- Install Python 3.13 for your system
- Ensure `python3 --version` shows 3.13.x
- If multiple Python versions are installed, use `poetry env use python3.13` in each service directory

#### Java version mismatch
**Problem:** Maven or tests fail with "class file has wrong version" or similar Java version errors
...
- Set JAVA_HOME to point to JDK 21:
  ...
  ```
- The Maven wrapper (`./mvnw`) will use the Java version from your PATH

#### Node.js version mismatch
**Problem:** npm commands fail or show unexpected behavior with wrong Node version
...
**Problem:** Database-related commands fail with Docker errors
...
- For macOS/Windows, ensure Docker Desktop is running
- Check Docker is accessible without sudo (Linux): add your user to the docker group

#### Maven dependency download failures
**Problem:** Maven commands fail with "Could not resolve dependencies" or connection timeouts

**Solution:**
- Check internet connectivity
- Try cleaning Maven cache: `rm -rf ~/.m2/repository`
- Re-run the command: `cd server/application-server && ./mvnw clean compile -DskipTests`
- Maven downloads hundreds of MB on first run; be patient

#### npm ci fails with package errors
**Problem:** `npm ci` fails or reports missing packages
...
#### Python Poetry install hangs or fails
**Problem:** `poetry install` in Python services hangs or fails
...
poetry install --with dev --no-root -vvv

# If still failing, recreate virtualenv
rm -rf .venv
poetry install --with dev --no-root
```

#### Tests fail with "address already in use"
**Problem:** Integration tests fail because ports are already in use

**Solution:**
- Check for running services: `lsof -i :5432` (PostgreSQL), `lsof -i :8080` (application server)
- Stop conflicting services or use different ports
- Clean up Docker containers: `docker ps -a` and `docker rm -f <container>`

#### Prettier/Biome formatting errors
**Problem:** Formatting or linting commands fail
...
# Try formatting with fix flag
npm --workspace webapp run check -- --write

# For Java formatting issues
npm run format:java
```

### Getting Help
If you encounter issues not covered here:
1. Check the error message carefully and search for it in GitHub Issues
2. Verify all prerequisites in Section 2 are properly installed
3. Try the relevant troubleshooting steps above
4. Consult the [local development guide](https://ls1intum.github.io/Hephaestus/contributor/local-development) documentation
5. Ask in the project discussions or open an issue with full error details
