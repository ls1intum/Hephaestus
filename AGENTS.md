...
| Server tests | **Four test tiers:** <br>• `mvn test -P'!quick'` runs unit tests (`@Tag("unit")`) <br>• `mvn test -P'!quick,architecture-tests'` runs ArchUnit + Modulith verification (`@Tag("architecture")`) <br>• `mvn verify` runs unit + integration tests (`@Tag("integration")`) <br>• `mvn test -Plive-tests` runs live GitHub API tests (`@Tag("live")`) <br><br>**Always deactivate `quick` on a test run.** It is active by default and skips surefire entirely, so `mvn test` and `mvn test -Parchitecture-tests` print BUILD SUCCESS having run **zero** tests, and `mvn test-compile` reports success while test sources fail to compile. <br><br>Live tests require GitHub App credentials configured in `application-live-local.yml` (gitignored). The Maven profile is the single guard—tests only run when explicitly activated. |

**Script naming conventions:** `format` applies formatting, `format:check` verifies it read-only for CI,
`lint` lints, and `check` is the comprehensive verification (format + lint + typecheck). A `:webapp` or `:java`
suffix scopes any of them to one service.
...
- Styling: Tailwind utility classes + shadcn primitives in `src/components/ui`. Compose class names with `clsx`/`tailwind-merge`; use the shadcn tokens the codebase already uses (`bg-background`, `bg-muted`, `bg-card`, `text-muted-foreground`) rather than raw colours.
...
- **React Compiler**: The webapp runs React Compiler at build time — `vite.config.ts` feeds `reactCompilerPreset()` through `@rolldown/plugin-babel`. This means:
...
- HMAC / hex / locale safety enforced by ArchUnit: `HexEncodingArchTest` (only `HexFormat.of()`), `LocaleSafetyArchTest` (no naked `toLowerCase`/`toUpperCase`). Coverage gate: the webhook packages carry per-package JaCoCo branch floors (0.60–0.75), enforced by the unit-test CI job via `-DskipCoverage=false`. The floors record what the tier covers today and exist to fail a regression — raise one whenever its package clears the next step.
...
- **`generate:api:application-server:specs` exits 0 when the app never started.** It boots the server to scrape
  springdoc, so it needs free HTTP, management **and JMX** ports. A busy JMX 9001 prints
  `Port already in use: 9001`, writes no spec, and still exits 0 — you commit a spec missing your new endpoint.
  Pass free ports: `SERVER_PORT=38111 MANAGEMENT_SERVER_PORT=38113 ./mvnw verify -DskipTests=true -Dapp.profiles=specs -Dopenapi.jmx.port=9031`.
- **Two Maven runs in one worktree corrupt each other.** They share `target/`, so a build started while another is
  running fails on half-written generated sources. Let one finish.
- **`server/.env` leaks into Maven test JVMs.** A local `MANAGEMENT_PORT` collides across worktrees, and local
  OAuth variables make environment-sensitive tests fail only on your machine. Run tests with
  `MANAGEMENT_PORT=0 SERVER_PORT=0`.
- `pnpm run db:draft-changelog` requires Docker on PATH. In CI we set `CI=true`; locally ensure the daemon is running.
- `pnpm run generate:api:application-server:specs` performs a full Maven `verify`. The first run downloads the whole
  Spring Boot dependency tree (~hundreds of MB); expect several minutes on a cold cache.
