...
- Follow the container/presentation split already in place (route files under `src/routes/**` fetch data and pass it to components under `src/components/**`). Keep components pure and side-effect free. Exception: a section may own its own reads and mutations when the data is used by that section alone and no sibling or route needs it (existing examples: `AdminOutlineSettings`, `AdminSlackNotificationSettings`, `settings/SessionsSection`, `settings/DangerZoneSection`, `AdminDangerZoneSettings`). Such a section owns its loading and error states, and every story that renders it — page-level stories included — must supply an `msw` parameter covering its requests. Note that an unhandled request does **not** fail the story: MSW bypasses it, the dev server answers with the app's HTML, and the section then errors or throws, often after the play has finished.
