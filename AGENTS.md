...
| **Java**                 | `pnpm run format:java`                 | `pnpm run format:java:check`                 | `pnpm run lint:java` (PMD)          | —                                    |
...
| Server tests | **Four test tiers:** <br>• `mvn test -P'!quick'` runs unit tests (`@Tag("unit")`) <br>• `mvn test -P'!quick,architecture-tests'` runs ArchUnit + Modulith verification (`@Tag("architecture")`) <br>• `mvn verify -P'!quick'` runs unit + integration tests (`@Tag("integration")`) <br>• `mvn test -P'!quick,live-tests'` runs live GitHub API tests (`@Tag("live")`) <br><br>**Always deactivate `quick` on a test run, in every tier.** It is active by default and sets `maven.test.skip=true`, so `mvn test`, `mvn verify`, `mvn test -Parchitecture-tests` and `mvn test -Plive-tests` all print BUILD SUCCESS having run **zero** tests, and `mvn test-compile` reports success while test sources fail to compile. No other profile deactivates it — `!quick` has to be in the same `-P` list. <br><br>Live tests require GitHub App credentials configured in `application-live-local.yml` (gitignored). The Maven profile is the single guard—tests only run when explicitly activated. |
...
- **`generate:api:application-server:specs` exits 0 when the app never started.** It boots the server to scrape
  ...
  Pass free ports: `SERVER_PORT=38111 MANAGEMENT_PORT=38113 ./mvnw verify -DskipTests=true -Dapp.profiles=specs -Dopenapi.jmx.port=9031`.
