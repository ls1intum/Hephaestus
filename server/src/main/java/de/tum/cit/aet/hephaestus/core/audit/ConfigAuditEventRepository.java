...
public interface ConfigAuditEventRepository extends JpaRepository<ConfigAuditEvent, Long> {
    String FILTER_PREDICATES = """
          AND (CAST(:#{#f.entityTypeName()} AS text) IS NULL OR e.entity_type = CAST(:#{#f.entityTypeName()} AS text))
          ...
          AND (CAST(:#{#f.actionName()} AS text) IS NULL OR e.action = CAST(:#{#f.actionName()} AS text))
