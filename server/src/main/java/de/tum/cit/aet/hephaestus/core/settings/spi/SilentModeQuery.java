...
/**
 * Read port for the instance-wide emergency silent mode, consulted immediately before any write to an
 * external system. Reads hit the singleton row uncached: a cache would keep sending just after an
 * engage, which is the one direction an emergency brake must never be stale in.
 *
