 ...
 * {@link SummaryChannel}, a diff-anchored note through {@link InlineFindingChannel}, and the two are
 * separate SPIs with separate failure modes. That split is why the enum is finer-grained than the
 * persisted {@code practices.feedback.FeedbackChannel}, where both in-context lanes collapse to one
 * value: the ledger records only that the developer was told in place, while the delivery path has to
 * know which bean to call.
 *
