...
/**
 * Queue-health gauges for the {@code agent_job} queue. Sampled on a timer, never on a request path.
 *
 * <ul>
 ...
 * </ul>
 *
 * <p>All three come from ONE query ({@link AgentJobRepository#queueHealthSnapshot}): separate COUNT/MIN
 * scans would cost the most exactly when the backlog is worst — an incident — which is when the signal
 * matters most. The 30s interval is the same trade: sub-30s resolution buys nothing here. On a sampler
 * failure the gauges keep their LAST-GOOD values (a transient DB blip must not make the dashboard read
 * "queue is empty") and {@code agent.queue.health.sampler.failures} counts the miss, so a sustained
 ...
 * {@code ServerSchedulingConfig} already silences {@code @Scheduled} on the worker and webhook pods,
 * but an ungated bean still REGISTERS its three gauges there, and a gauge that is never sampled reads
 * a permanent 0 — so those pods would publish flat zeros into the same {@code agent.queue.*} series
 * the server pod publishes real values into, and any aggregate across replicas would read low.
 */
...
public class AgentQueueHealthSampler {
    ...
    public void sample() {
        ...
        try {
            ...
            // momentary "queue is empty" reading.
