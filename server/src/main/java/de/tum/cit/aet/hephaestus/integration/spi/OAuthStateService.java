...
public interface OAuthStateService {
     ...
     * flow; {@code null} for tokens issued via the legacy no-actor overload. Callers
     * that need actor attribution should fall back to a sentinel (e.g. {@code "oauth-callback"})
     * when this is null.
     */
    record StateBinding(long workspaceId, IntegrationKind kind, Instant issuedAt, @Nullable String actorRef) {
        /** Convenience constructor that defaults actorRef to {@code null} — keeps pre-existing call sites compiling. */
