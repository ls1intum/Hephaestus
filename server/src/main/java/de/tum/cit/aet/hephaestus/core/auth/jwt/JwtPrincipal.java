 ...
 *
 * <p>The {@code login} + {@code roles} fields are what make our JWT a drop-in replacement
 * for the prior Keycloak token: {@code login} becomes the {@code preferred_username} claim
 * (consumed by {@code SecurityUtils.getCurrentUserLogin} and ~100 call sites), and
 * {@code roles} becomes {@code realm_access.roles} (consumed by the authority converter,
 ...
 * @param roles     realm roles ({@code "admin"} when app-admin, plus enabled feature-flag
 *                  keys) → {@code realm_access.roles}
 */
