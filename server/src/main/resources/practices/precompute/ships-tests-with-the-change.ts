...
const TEST = /(^|\/)(tests?|specs?|__tests__)(\/)|[._-](test|tests|spec|specs)\.[a-z]+$|Tests?\.[a-z0-9]+$|Spec\.[a-z0-9]+$/i;
const CODE = /\.(swift|ts|tsx|js|jsx|py|java|kt|go|rb|cs|cpp|c|m|mm)$/i;
...
export default async function (repoPath: string, diffFiles: Map<string, DiffFile>, _m: PullRequestMetadata) {
  ...
  if (repoTestFileCount === 0) {
    ...
    directions.push(`The change adds/edits ${diffTestFiles} test file(s) alongside ${diffProdFiles} production file(s).`);
  }
