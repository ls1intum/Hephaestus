...
3. Evidence snippets must be copied character-for-character from `+` or `-` lines in the diff. Do not paraphrase or reconstruct from memory. Line numbers use the `[L<n>]` annotations and OLD/NEW side from `diff.patch`.
   3a. `inputs/sources/scm/repo/` is there so you can resolve what a changed line calls into — the signature it invokes, the invariant its caller already guarantees, whether the helper it replaced is still referenced elsewhere. Read it to decide whether a changed line is wrong, and quote it in `guidance` when the reason lives outside the diff. It does not widen what you may flag: the finding must still be about a `+` or `-` line, and rule 3's evidence snippet still comes from the diff.
