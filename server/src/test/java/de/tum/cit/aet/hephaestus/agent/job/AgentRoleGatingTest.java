...
class AgentRoleGatingTest extends BaseUnitTest {
    ...
    void shouldRequireServerRoleWhenRecoveringJobs() {
        assertThat(AgentJobZombieSweeper.class.isAnnotationPresent(ConditionalOnServerRole.class)).isTrue();
        assertAgentEnabledCondition(AgentJobZombieSweeper.class);
    }

    @Test
    void shouldAllowSettingsServiceWithoutServerOnlyAuditLogger() {
        assertThat(InstanceSettingsService.class.isAnnotationPresent(ConditionalOnServerRole.class)).isFalse();
        assertThat(InstanceSettingsService.class.getDeclaredConstructors())
            .singleElement()
            .satisfies(constructor ->
                assertThat(constructor.getGenericParameterTypes()).anySatisfy(type -> {
