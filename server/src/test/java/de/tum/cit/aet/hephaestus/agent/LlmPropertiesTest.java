 ...
 * the shape of a currency code. The distinction is the whole point: {@code GBP} is a perfectly
 * well-formed code, and an instance that accepted it could only ever have shown USD — a missing
 * feature the operator would go looking for in the wrong place.
 */
class LlmPropertiesTest extends BaseUnitTest {
    ...
    /**
     * {@code GBP} and {@code CHF} are the finding: well-formed codes that a shape-only pattern waves
     * through and that then produce USD-only output with nothing but a log line to say why.
     */
    ...
    void refusesToBootOnAnythingItCannotConvertTo(String configured) {
        ...
        // The message has to name the set, since "invalid" would send the operator looking for a typo
        // in a value that is spelled perfectly.
