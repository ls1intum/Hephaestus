    ...
    class Conjunction {
        ...
        void theTierAppliesUniformlyToEveryChannel() {
            for (FeedbackChannel channel : FeedbackChannel.values()) {
                assertThat(
                    PracticeAutonomyPolicy.delivers(ObservationOrigin.LIVE, PracticeAutonomy.HUMAN_APPROVAL, channel)
                )
        ...
        void shouldFailClosedWhenAutonomyCannotBeResolved() {
            assertThat(
                PracticeAutonomyPolicy.delivers(ObservationOrigin.LIVE, null, FeedbackChannel.IN_CONTEXT)
            ).isFalse();
            assertThat(
                PracticeAutonomyPolicy.delivers(ObservationOrigin.BACKFILL, null, FeedbackChannel.IN_CONTEXT)
            ).isFalse();
        }
    }
}
