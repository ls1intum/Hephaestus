...
/**
 * {@code lastReflectionSurfacedAt} — the cooldown that stops one habit being restated on every pull
 ...
 * catch-all.
 */
class ReflectionCooldownQueryIntegrationTest extends BaseIntegrationTest {
    ...
    void setUp() {
        ...
        workspace = workspaceRepository.save(WorkspaceTestFixtures.activeWorkspace("reflection-cooldown"));
        otherWorkspace = workspaceRepository.save(WorkspaceTestFixtures.activeWorkspace("reflection-cooldown-other"));
    ...
    void answersEmptyWhenNothingWasEverSurfaced() {
        assertThat(
            feedbackRepository.lastReflectionSurfacedAt(workspace.getId(), recipient.getId(), PRACTICE_SLUG)
        ).isEmpty();
    }
    ...
    /** One REFLECTION unit about {@link #PRACTICE_SLUG}, written for {@link #recipient} in this workspace. */
