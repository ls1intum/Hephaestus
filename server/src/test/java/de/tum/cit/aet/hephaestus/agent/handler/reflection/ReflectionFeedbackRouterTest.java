...
/**
 * Every refusal the reflection lane can give, and the one admission. Each case differs from the admitted
 ...
 */
class ReflectionFeedbackRouterTest extends BaseUnitTest {
    ...
    void admitsACorroboratedLivePatternNobodyHasBeenShownLately() {
        ...
        ).isEqualTo(ReflectionRoutingDecision.ADMIT);
    }

    @Test
    void refusesAMessageMissingItsNextStep() {
        ComposedReflectionMessage incomplete = new ComposedReflectionMessage("ships-tests", "Title", "Body", "  ");

        assertThat(
            ReflectionFeedbackRouter.route(
                incomplete,
        ...
        ).isEqualTo(ReflectionRoutingDecision.INCOMPLETE);
    }
    ...
    void refusesAPracticeThatJudgesSomebodyOtherThanTheAuthor() {
        ...
        ).isEqualTo(ReflectionRoutingDecision.REVIEWER_ATTRIBUTED);
    }
    ...
    void refusesAPracticeWithNoProblemsBehindIt() {
        ...
        assertThat(route(List.of(strength), PracticeReviewTier.DELIVER, ActorRole.AUTHOR, null)).isEqualTo(
            ReflectionRoutingDecision.NO_EVIDENCE
        );
    }
    ...
    void refusesAPracticeWhoseTierDoesNotAdmitTheLane() {
        ...
        ).isEqualTo(ReflectionRoutingDecision.PRACTICE_TIER_QUIET);
    }
    ...
    void refusesAPatternWhoseEveryMeasurementCameFromABackfill() {
        ...
        ).isEqualTo(ReflectionRoutingDecision.BACKFILL_HELD);
    }
    ...
    void refusesAProblemSeenOnOnlyOnePieceOfWork() {
        ...
        ).isEqualTo(ReflectionRoutingDecision.UNCORROBORATED);
    }
    ...
    void refusesAHabitTheDeveloperWasShownInsideTheCooldown() {
        ...
        ).isEqualTo(ReflectionRoutingDecision.RECENTLY_SURFACED);
    }

    @Test
    void admitsAHabitLastShownBeforeTheCooldownElapsed() {
        Instant longAgo = NOW.minus(Duration.ofDays(ReflectionFeedbackRouter.RESURFACE_COOLDOWN_DAYS + 1));
    ...
    void narrowsAWindowOfMeasurementsToJustTheProblems() {
        ...
        assertThat(ReflectionFeedbackRouter.problemsIn(List.of(problem, strength, abstention))).containsExactly(
            problem
        );
    }

    private static ReflectionRoutingDecision route(
        List<Observation> evidence,
        PracticeReviewTier tier,
        ActorRole subjectRole,
        Instant lastSurfaced
    ) {
        return ReflectionFeedbackRouter.route(message(), evidence, tier, subjectRole, lastSurfaced, NOW);
    }

    private static ComposedReflectionMessage message() {
        return new ComposedReflectionMessage(
