...
/**
 * The developer's own reflection surface, end to end: who may read it, and what reading it records.
 */
class ReflectionFeedbackControllerIntegrationTest extends AbstractWorkspaceIntegrationTest {

    private static final ObjectMapper OBJECT_MAPPER = new ObjectMapper();
    private static final String REFLECTION = "/workspaces/{slug}/practices/feedback/reflection";
    ...
    @BeforeEach
    void setUpWorkspace() {
        User owner = persistUser("reflection-owner");
        workspace = createWorkspace("reflection-ws", "Reflection WS", "reflection-org", AccountType.ORG, owner);
    ...
    void returnsTheDevelopersOwnPreparedMessage() {
        Feedback unit = persistReflectionUnit(
            workspace,
            job,
            developer,
            7000,
    ...
    void doesNotReturnAnotherDevelopersMessage() {
        Feedback theirs = persistReflectionUnit(
            workspace,
            job,
            teammate,
            7000,
    ...
    private WebTestClient.BodyContentSpec getOk(Workspace ws) {
        return webTestClient
            .get()
            .uri(REFLECTION, ws.getWorkspaceSlug())
    ...
    private Feedback persistReflectionUnit(
        Workspace ws,
        AgentJob agentJob,
        User recipient,
        int position,
        ...
        return feedbackRepository.save(
            Feedback.builder()
                ...
                .position(position)
                .deliveryState(state)
                .body(ReflectionFeedbackBody.render(headline, message, "Write the test first next time."))
