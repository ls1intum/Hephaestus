...
import {
    ...
    newUsageLedger,
} from "../../../main/resources/agent/pi-runner-usage.mjs";

function assistant(id, { input = 0, output = 0, reasoning = 0, cacheRead = 0, cacheWrite = 0, cost = 0 } = {}) {
    return {
        id,
        role: "assistant",
        model: "gpt-5",
        stopReason: "end_turn",
        usage: { input, output, reasoning, cacheRead, cacheWrite, cost: { total: cost } },
    };
}
...
test("every bucket of an assistant message lands on the ledger", () => {
    ...
    addAssistantUsage(ledger, assistant("a", { input: 100, output: 20, reasoning: 5, cacheRead: 7, cacheWrite: 3, cost: 0.5 }));

    assert.equal(ledger.totalCalls, 1);
    assert.equal(ledger.inputTokens, 100);
    assert.equal(ledger.outputTokens, 20);
    assert.equal(ledger.reasoningTokens, 5);
...
test("only assistant messages that carry usage are billable", () => {
    ...
    addAssistantUsage(ledger, { id: "u", role: "user", usage: { input: 999 } });
    addAssistantUsage(ledger, { id: "a", role: "assistant" });
