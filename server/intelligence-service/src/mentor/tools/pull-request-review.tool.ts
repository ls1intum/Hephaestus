...
export const getPullRequestBadPractices = tool({
	...
	inputSchema,
	execute: async ({ pullRequestId, repositoryNameWithOwner, pullRequestNumber }: Input) => {
		try {
			...
			const firstBadPracticeId = badPracticeIds[0];
			const feedbackResults =
				firstBadPracticeId !== undefined
					? await db
							.select()
							.from(badPracticeFeedback)
							.where(eq(badPracticeFeedback.pullRequestBadPracticeId, firstBadPracticeId))
					: [];

			// For multiple bad practices, we need to fetch feedback for each
			const feedbackByBadPracticeId: Record<number, (typeof feedbackResults)[number][]> = {};
			...
			return {
				...
				badPractices: badPractices.map((bp) => ({
					...
					userState: bp.userState !== null ? (userStateMap[bp.userState] ?? "UNKNOWN") : null,
					detectionLifecycleState:
						bp.detectionPullrequestLifecycleState !== null
							? (lifecycleStateMap[bp.detectionPullrequestLifecycleState] ?? "UNKNOWN")
							: null,
