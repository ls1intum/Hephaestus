...
export const getPullRequests = tool({
	...
	execute: async ({ userLogin, state, limit }: Input) => {
		try {
			...
			const conditions = [eq(issue.authorId, userId), eq(issue.issueType, "PULLREQUEST")];
			...
			return {
				...
				pullRequests: pullRequests.map((pr) => ({
					...
					title: pr.title,
					state: pr.state,
					body: pr.body ? pr.body.slice(0, 500) + (pr.body.length > 500 ? "..." : "") : null,
					htmlUrl: pr.htmlUrl,
			...
			return {
				success: false,
				error:
					error instanceof Error ? error.message : "Failed to fetch pull requests from database",
				pullRequests: [],
			};
		}
	},
