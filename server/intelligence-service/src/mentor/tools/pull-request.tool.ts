	...
	execute: async ({ pullRequestId, repositoryNameWithOwner, pullRequestNumber }: Input) => {
		try {
			...
			const [
				...
				// Author
				prRecord.authorId
					? db.select().from(user).where(eq(user.id, prRecord.authorId)).limit(1)
					: Promise.resolve([]),
				// Merged by
				prRecord.mergedById
					? db.select().from(user).where(eq(user.id, prRecord.mergedById)).limit(1)
					: Promise.resolve([]),
				// Repository
				prRecord.repositoryId
					? db.select().from(repository).where(eq(repository.id, prRecord.repositoryId)).limit(1)
					: Promise.resolve([]),
				// Comments
				db
					.select({
				...
				prRecord.milestoneId
					? db.select().from(milestone).where(eq(milestone.id, prRecord.milestoneId)).limit(1)
					: Promise.resolve([]),
				// Reviews
				db
					.select({
			...
			return {
				...
				pullRequest: {
					...
					reviews: reviewsResult.map((r) => ({
						id: r.id,
						state: r.state,
						body: r.body ? r.body.slice(0, 500) + (r.body.length > 500 ? "..." : "") : null,
