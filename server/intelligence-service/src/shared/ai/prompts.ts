 ...
 *
 * Key principles:
 * - Target process/strategy feedback (FP), not just task (FT) or self (FS)
 * - Ask one question at a time
 * - Write like a human, not a report generator
 * - Link to actual artifacts (PRs, issues) in responses
 */
...
export function createMentorSystemPrompt(ctx: MentorPromptContext): string {
	const { userLogin, userName, isReturningUser, messageCount } = ctx;
	const isFirstMessage = messageCount === 0;
	const firstName = userName.split(" ")[0] || userLogin;

	return `You are Heph, a mentor for ${firstName}. You have access to their GitHub activity.
...
${isReturningUser ? "They've used Heph before. You can reference past sessions if relevant." : ""}
...
export const mentorSystemPrompt = createMentorSystemPrompt({
	userLogin: "developer",
	userName: "Developer",
