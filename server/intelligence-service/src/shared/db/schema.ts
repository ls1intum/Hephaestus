...
export const repository = pgTable(
	...
	(table) => [
		index("idx_repository_name_with_owner").using(
			"btree",
			table.nameWithOwner.asc().nullsLast().op("text_ops"),
		),
...
export const issue = pgTable(
	...
	(table) => [
		index("idx_issue_author_id").using("btree", table.authorId.asc().nullsLast().op("int8_ops")),
		index("idx_issue_issue_type_id").using(
			"btree",
			table.issueTypeId.asc().nullsLast().op("text_ops"),
		),
		index("idx_issue_parent_issue_id").using(
			"btree",
			table.parentIssueId.asc().nullsLast().op("int8_ops"),
		),
		index("idx_issue_repository_id").using(
			"btree",
			table.repositoryId.asc().nullsLast().op("int8_ops"),
		),
		index("idx_issue_state").using("btree", table.state.asc().nullsLast().op("text_ops")),
		foreignKey({
...
export const discussion = pgTable(
	...
	(table) => [
		index("idx_discussion_author").using("btree", table.authorId.asc().nullsLast().op("int8_ops")),
		index("idx_discussion_category").using(
			"btree",
			table.categoryId.asc().nullsLast().op("text_ops"),
		),
		index("idx_discussion_created_at").using(
			"btree",
			table.createdAt.asc().nullsLast().op("timestamptz_ops"),
		),
		index("idx_discussion_repository").using(
			"btree",
			table.repositoryId.asc().nullsLast().op("int8_ops"),
		),
		index("idx_discussion_state").using("btree", table.state.asc().nullsLast().op("text_ops")),
		foreignKey({
			columns: [table.repositoryId],
			foreignColumns: [repository.id],
...
export const issueComment = pgTable(
	...
	(table) => [
		index("idx_issue_comment_author_id").using(
			"btree",
			table.authorId.asc().nullsLast().op("int8_ops"),
		),
		index("idx_issue_comment_created_at").using(
			"btree",
			table.createdAt.asc().nullsLast().op("timestamptz_ops"),
		),
		foreignKey({
...
export const pullRequestReview = pgTable(
	...
	(table) => [
		index("idx_pr_review_author_id").using(
			"btree",
			table.authorId.asc().nullsLast().op("int8_ops"),
		),
		index("idx_pr_review_pull_request_id").using(
			"btree",
			table.pullRequestId.asc().nullsLast().op("int8_ops"),
		),
		index("idx_pr_review_submitted_at").using(
			"btree",
...
export const activityEvent = pgTable(
	...
	(table) => [
		...
		index("idx_activity_event_actor_time").using(
			"btree",
			table.actorId.asc().nullsLast().op("timestamptz_ops"),
		...
		index("idx_activity_event_leaderboard_covering").using(
			...
			table.occurredAt.desc().nullsFirst().op("int8_ops"),
			table.actorId.asc().nullsLast().op("timestamptz_ops"),
			table.xp.asc().nullsLast().op("float8_ops"),
		),
		index("idx_activity_event_target").using(
			"btree",
			table.targetId.asc().nullsLast().op("int8_ops"),
			table.targetType.asc().nullsLast().op("text_ops"),
		),
		index("idx_activity_event_type_time").using(
			"btree",
			table.eventType.asc().nullsLast().op("timestamptz_ops"),
...
export const issueType = pgTable(
	...
	(table) => [
		index("idx_issue_type_organization_id").using(
			"btree",
			table.organizationId.asc().nullsLast().op("int8_ops"),
		),
		foreignKey({
...
export const projectItem = pgTable(
	...
	(table) => [
		index("idx_project_item_content_type_archived").using(
			...
			table.archived.asc().nullsLast().op("int8_ops"),
		),
		index("idx_project_item_creator_id").using(
			"btree",
			table.creatorId.asc().nullsLast().op("int8_ops"),
		),
		index("idx_project_item_issue_id").using(
			"btree",
			table.issueId.asc().nullsLast().op("int8_ops"),
		),
		index("idx_project_item_orphaned_relink")
			.using("btree", table.contentDatabaseId.asc().nullsLast().op("int8_ops"))
			.where(
...
export const project = pgTable(
	...
	(table) => [
		index("idx_project_creator_id").using(
			"btree",
			table.creatorId.asc().nullsLast().op("int8_ops"),
		),
...
export const projectField = pgTable(
	...
	(table) => [
		index("idx_project_field_project_id").using(
			"btree",
...
export const projectStatusUpdate = pgTable(
	...
	(table) => [
		...
		index("idx_project_status_update_creator_id").using(
			"btree",
			table.creatorId.asc().nullsLast().op("int8_ops"),
		),
		foreignKey({
...
export const gitCommit = pgTable(
	...
	(table) => [
		index("idx_git_commit_author_id").using(
			"btree",
			table.authorId.asc().nullsLast().op("int8_ops"),
		),
		index("idx_git_commit_authored_at").using(
			"btree",
			table.authoredAt.asc().nullsLast().op("timestamptz_ops"),
		),
		index("idx_git_commit_committer_id").using(
			"btree",
			table.committerId.asc().nullsLast().op("int8_ops"),
		),
		index("idx_git_commit_repository_id").using(
			"btree",
			table.repositoryId.asc().nullsLast().op("int8_ops"),
		),
		index("idx_git_commit_unresolved_author_email")
			.using(
				"btree",
				table.repositoryId.asc().nullsLast().op("int8_ops"),
				table.authorEmail.asc().nullsLast().op("text_ops"),
			)
		...
		index("idx_git_commit_unresolved_committer_email")
			.using(
				"btree",
				table.repositoryId.asc().nullsLast().op("int8_ops"),
				table.committerEmail.asc().nullsLast().op("text_ops"),
			)
...
export const commitFileChange = pgTable(
	...
	(table) => [
		index("idx_commit_file_change_commit_id").using(
			"btree",
			table.commitId.asc().nullsLast().op("int8_ops"),
		),
		index("idx_commit_file_change_filename").using(
			"btree",
...
export const commitContributor = pgTable(
	...
	(table) => [
		index("idx_commit_contributor_commit_id").using(
			"btree",
			table.commitId.asc().nullsLast().op("int8_ops"),
		),
		index("idx_commit_contributor_user_id").using(
			"btree",
...
export const discussionComment = pgTable(
	...
	(table) => [
		index("idx_discussion_comment_author").using(
			"btree",
			table.authorId.asc().nullsLast().op("int8_ops"),
		),
		index("idx_discussion_comment_discussion").using(
			"btree",
			table.discussionId.asc().nullsLast().op("int8_ops"),
		),
		index("idx_discussion_comment_discussion_created").using(
			"btree",
			table.discussionId.asc().nullsLast().op("timestamptz_ops"),
			table.createdAt.asc().nullsLast().op("timestamptz_ops"),
		),
		index("idx_discussion_comment_is_answer").using(
			"btree",
			table.isAnswer.asc().nullsLast().op("bool_ops"),
		),
		index("idx_discussion_comment_parent").using(
			"btree",
			table.parentCommentId.asc().nullsLast().op("int8_ops"),
		),
		foreignKey({
...
export const projectFieldValue = pgTable(
	...
	(table) => [
		index("idx_project_field_value_field_id").using(
			"btree",
			table.fieldId.asc().nullsLast().op("text_ops"),
		),
		index("idx_project_field_value_item_id").using(
			"btree",
...
export const issueBlocking = pgTable(
	...
	(table) => [
		index("idx_issue_blocking_blocked_id").using(
			"btree",
			table.blockedIssueId.asc().nullsLast().op("int8_ops"),
		),
		index("idx_issue_blocking_blocking_id").using(
			"btree",
			table.blockingIssueId.asc().nullsLast().op("int8_ops"),
		),
		foreignKey({
...
export const commitPullRequest = pgTable(
	...
	(table) => [
		index("idx_commit_pull_request_pr_id").using(
			"btree",
			table.pullRequestId.asc().nullsLast().op("int8_ops"),
		),
		foreignKey({
...
export const workspaceTeamSettings = pgTable(
	...
	(table) => [
		uniqueIndex("IX_workspace_team_settingsPK").using(
			"btree",
			table.teamId.asc().nullsLast().op("int8_ops"),
			table.workspaceId.asc().nullsLast().op("int8_ops"),
		),
		index("idx_workspace_team_settings_team_id").using(
			"btree",
			table.teamId.asc().nullsLast().op("int8_ops"),
		),
		foreignKey({
...
export const workspaceTeamLabelFilter = pgTable(
	...
	(table) => [
		uniqueIndex("IX_workspace_team_label_filterPK").using(
			"btree",
			table.labelId.asc().nullsLast().op("int8_ops"),
			table.teamId.asc().nullsLast().op("int8_ops"),
			table.workspaceId.asc().nullsLast().op("int8_ops"),
		),
		index("idx_workspace_team_label_filter_team_id").using(
			"btree",
			table.teamId.asc().nullsLast().op("int8_ops"),
		),
		foreignKey({
...
export const workspaceMembership = pgTable(
	...
	(table) => [
		index("idx_workspace_membership_user_id").using(
			"btree",
			table.userId.asc().nullsLast().op("int8_ops"),
		),
		foreignKey({
