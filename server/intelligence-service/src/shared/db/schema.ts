...
export const repositoryCollaborator = pgTable(
	...
	(table) => [
		...
		primaryKey({
			columns: [table.repositoryId, table.userId],
			name: "pk_repository_collaborator",
		}),
	],
);
...
export const workspace = pgTable(
	"workspace",
	{
		...
		usersSyncedAt: timestamp("users_synced_at", {
			precision: 6,
			mode: "string",
		}),
		accountLogin: varchar("account_login", { length: 120 }),
		gitProviderMode: varchar("git_provider_mode", { length: 255 }),
		githubRepositorySelection: varchar("github_repository_selection", {
			length: 255,
		}),
		// You can use { mode: "bigint" } if numbers are exceeding js number limitations
		installationId: bigint("installation_id", { mode: "number" }),
		...
		leaderboardNotificationTeam: varchar("leaderboard_notification_team", {
			length: 100,
		}),
		leaderboardScheduleDay: integer("leaderboard_schedule_day"),
		leaderboardScheduleTime: varchar("leaderboard_schedule_time", {
			length: 10,
		}),
