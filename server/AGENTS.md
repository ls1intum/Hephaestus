...
selects the adapter's `WebhookSignatureVerifier` and `SubjectKeyDeriver` through
`IntegrationKindRouting` and publishes the verified envelope to JetStream, all gated on
`RuntimeRole.WEBHOOK_PROPERTY`. Configuration binds to `hephaestus.webhook.*` through
