...
export const repositoryToMonitor = pgTable(
	"repository_to_monitor",
	{
		...
		issuesAndPullRequestsSyncedAt: timestamp(
			"issues_and_pull_requests_synced_at",
			{ precision: 6, mode: "string" },
		),
...
export const pullrequestbadpractice = pgTable(
	"pullrequestbadpractice",
	{
		...
		detectionPullrequestLifecycleState: smallint(
			"detection_pullrequest_lifecycle_state",
		),
...
export const chatThread = pgTable(
	...
	(table) => [
		...
		unique("uc_chat_threadselected_leaf_message_id_col").on(
			table.selectedLeafMessageId,
		),
	],
);
...
export const document = pgTable(
	...
	(table) => [
		...
		index("idx_document_id").using(
			"btree",
			table.id.asc().nullsLast().op("uuid_ops"),
		),
		index("idx_document_user_id").using(
			"btree",
