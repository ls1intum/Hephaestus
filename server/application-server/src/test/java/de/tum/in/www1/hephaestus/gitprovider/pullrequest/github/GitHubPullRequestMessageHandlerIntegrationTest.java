...
class GitHubPullRequestMessageHandlerIntegrationTest extends BaseIntegrationTest {
  ...
  void setupTestData() {
    ...
    // Create test organization - use HephaestusTest to match fixtures
    ...
    // Create test repository - use IDs from fixtures
    testRepository = new Repository();
    testRepository.setId(998279771L);
  ...
  void shouldParsePullRequestEventCorrectly() throws Exception {
    String payload = loadPayload("github/pull_request.opened.json");
  ...
  void shouldPersistPullRequestOnOpenedEvent() throws Exception {
    ...
    // Repository already set up with correct ID in @BeforeEach
    handler.handleEvent(event);

    PullRequest pr = pullRequestRepository.findById(event.pullRequest().getDatabaseId())
  ...
  void shouldUpdatePullRequestOnClosedEvent() throws Exception {
    // First create the PR
    String openPayload = loadPayload("github/pull_request.opened.json");
    ...
    // Now close it
    String closePayload = loadPayload("github/pull_request.closed.json");
    ...
    PullRequest pr = pullRequestRepository.findById(closeEvent.pullRequest().getDatabaseId())
  ...
  void shouldHandleLabeledEvent() throws Exception {
    ...
    // Now add a label
    String labeledPayload = loadPayload("github/pull_request.labeled.json");
    GitHubPullRequestEventDTO labeledEvent = objectMapper.readValue(labeledPayload, GitHubPullRequestEventDTO.class);
    handler.handleEvent(labeledEvent);

    PullRequest pr = pullRequestRepository.findById(labeledEvent.pullRequest().getDatabaseId())
        .orElseThrow(() -> new AssertionError("Pull request should exist"));
  ...
  void shouldHandleReadyForReviewEvent() throws Exception {
    String payload = loadPayload("github/pull_request.ready_for_review.json");
    GitHubPullRequestEventDTO event = objectMapper.readValue(payload, GitHubPullRequestEventDTO.class);
  ...
  void shouldHandleConvertedToDraftEvent() throws Exception {
    String payload = loadPayload("github/pull_request.converted_to_draft.json");
    GitHubPullRequestEventDTO event = objectMapper.readValue(payload, GitHubPullRequestEventDTO.class);
