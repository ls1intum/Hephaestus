...
class ModelPricingServiceTest extends BaseUnitTest {
    ...
    void unknownModelReturnsEmpty() {
        when(repository.findByModelId("nope")).thenReturn(Optional.empty());
    ...
    void knownModelComputesCost() {
        when(repository.findByModelId("gpt-4o-mini")).thenReturn(Optional.of(fixture()));
    ...
    void cacheTokensAreSummed() {
        when(repository.findByModelId("claude-3-5-sonnet")).thenReturn(Optional.of(claudeFixture()));
