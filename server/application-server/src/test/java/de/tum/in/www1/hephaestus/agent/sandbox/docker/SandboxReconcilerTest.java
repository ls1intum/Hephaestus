...
class SandboxReconcilerTest extends BaseUnitTest {
  ...
  void setUp() {
    ...
    reconciler =
        new SandboxReconciler(
            jobRepository, containerManager, networkManager, properties, meterRegistry);
  }
  ...
  class StartupReconciliation {
    ...
    void shouldMarkOrphanedJobsAsFailed() {
      ...
      assertThat(meterRegistry.counter("sandbox.reconciler.orphaned", "resource", "job").count())
          .isEqualTo(1.0);
    }
  ...
  class PeriodicReconciliation {
    ...
    void shouldRemoveOrphanedContainers() {
      ...
      assertThat(
              meterRegistry.counter("sandbox.reconciler.orphaned", "resource", "container").count())
          .isEqualTo(1.0);
    }
    ...
    void shouldRemoveOrphanedNetworks() {
      ...
      assertThat(
              meterRegistry.counter("sandbox.reconciler.orphaned", "resource", "network").count())
          .isEqualTo(1.0);
    }
    ...
    void shouldContinueOnContainerCleanupFailure() {
      ...
      when(containerManager.listManagedContainers())
          .thenReturn(
              List.of(
                  new DockerOperations.ContainerInfo(
                      "ctr-1", "t1", Map.of("hephaestus.job-id", jobId1.toString()), "exited"),
                  new DockerOperations.ContainerInfo(
                      "ctr-2", "t2", Map.of("hephaestus.job-id", jobId2.toString()), "exited")));
    ...
    void shouldContinueNetworkCleanupOnContainerScanFailure() {
      ...
      when(containerManager.listManagedContainers())
          .thenThrow(new RuntimeException("Docker unreachable"));
