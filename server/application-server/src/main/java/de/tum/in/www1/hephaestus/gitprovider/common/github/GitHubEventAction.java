...
public sealed interface GitHubEventAction {
    ...
    /**
     * Action type for push webhook events.
     * <p>
     * Push events don't have a traditional action field - they represent
     * commits being pushed to a branch. We use PUSHED as the default action.
     */
    enum Push implements GitHubEventAction {
        /** Default action for push events (commits pushed to a branch). */
        PUSHED,
        UNKNOWN;
        ...
        public static Push fromString(String action) {
            // Push events don't have an action field, default to PUSHED
            if (action == null || action.isBlank()) return PUSHED;
            return switch (action.toLowerCase()) {
                case "pushed" -> PUSHED;
                default -> UNKNOWN;
            };
