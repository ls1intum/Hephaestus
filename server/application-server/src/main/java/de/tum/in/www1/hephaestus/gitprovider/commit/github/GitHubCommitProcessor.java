...
public class GitHubCommitProcessor extends BaseGitHubProcessor {
    ...
    public Commit process(GitHubCommitDTO dto, ProcessingContext context) {
        ...
        // Prepare commit data
        String message = dto.messageHeadline() != null ? dto.messageHeadline() : extractHeadline(dto.message());
        ...
        Commit commit = commitRepository.findByShaAndRepositoryId(sha, repository.getId()).orElseThrow(() ->
            new IllegalStateException("Commit not found after upsert: sha=" + sha)
        );
