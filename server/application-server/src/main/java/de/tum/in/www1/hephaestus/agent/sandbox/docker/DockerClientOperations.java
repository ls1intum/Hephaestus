 ...
 *
 * <p>Implements all three focused operation interfaces. Each method is a thin
 * delegate to {@link DockerClient}, translating {@link DockerException} to
    ...
    implements DockerContainerOperations, DockerNetworkOperations, DockerFileOperations
{
    ...
    private HostConfig buildHostConfig(DockerOperations.HostConfigSpec spec) {
        ...
        if (spec.capDrop() != null && !spec.capDrop().isEmpty()) {
            hostConfig.withCapDrop(spec.capDrop().stream().map(Capability::valueOf).toArray(Capability[]::new));
        }
