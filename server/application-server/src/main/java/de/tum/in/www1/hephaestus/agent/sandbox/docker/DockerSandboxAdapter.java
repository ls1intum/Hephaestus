 ...
 * <ol>
 *   <li><b>PREPARE</b> — create network, connect app-server, create container, inject files</li>
 *   <li><b>EXECUTE</b> — start container, wait for completion with timeout</li>
 *   <li><b>COLLECT</b> — extract output files, capture logs</li>
 *   <li><b>CLEANUP</b> — remove container, disconnect app-server, remove network</li>
 * </ol>
 *
 * <p>Each call to {@link #execute} is self-contained and blocking. Callers submit to
 * {@code sandboxExecutor} (bounded platform thread pool). Cleanup runs in {@code finally}
 * regardless of outcome — partial failures are logged and left for reconciliation.
 *
 * <p>Cancellation uses a simple {@link AtomicBoolean} flag per job. The execute loop
 * checks the flag between phases and stops the container if set.
 */
public class DockerSandboxAdapter implements SandboxManager {
    ...
    /**
     * Best-effort cleanup of all resources. Each step is independent —
     * failures are logged but don't prevent other cleanup steps.
     */
