 ...
 *
 * <p>Activated only when {@code hephaestus.sandbox.enabled=true}. All sandbox beans
 * are defined here as {@code @Bean} methods so they are only created when the
 * subsystem is active — no component scanning surprises.
 */
@Configuration
...
public class DockerSandboxConfiguration {
     ...
     *
     * <p>docker-java's Apache HttpClient5 has {@code synchronized} blocks
     * that pin virtual threads in Java 21, causing cascading failures.
     * A dedicated bounded pool of platform threads avoids this.
     */
     ...
     *
     * <p>Uses direct handoff ({@code queueCapacity=0}) so that when all threads
     * are occupied, submissions are rejected immediately — clean backpressure
     * to the NATS consumer, which will redeliver the message later.
     */
     ...
     *
     * <p>Fails hard if the resource is missing — a missing seccomp profile
     * would silently degrade container security, which is unacceptable.
     *
