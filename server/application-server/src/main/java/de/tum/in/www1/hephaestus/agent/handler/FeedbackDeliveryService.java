...
class FeedbackDeliveryService {
    ...
    private void doDeliver(AgentJob job, @Nullable DeliveryContent delivery, boolean hasNegative) {
        ...
        // Dedup uses stable external identifiers (repo name + PR number) to survive PR re-creation
        String repoFullName = (metadata != null && metadata.has("repository_full_name"))
            ? metadata.get("repository_full_name").asText()
            : null;
        Integer prNumber = (metadata != null && metadata.has("pr_number"))
            ? metadata.get("pr_number").asInt()
            : null;

        String previousCommentId = (repoFullName != null && prNumber != null)
            ? agentJobRepository
                  .findPreviousDeliveryCommentId(job.getWorkspace().getId(), repoFullName, prNumber, job.getId())
                  .orElse(null)
            : null;
