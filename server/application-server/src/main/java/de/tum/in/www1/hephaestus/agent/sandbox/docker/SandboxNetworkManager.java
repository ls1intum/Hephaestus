 ...
 *
 * <p>For {@code allowInternet=false}: creates an {@code --internal} network
 * with zero external connectivity. The app-server container is multi-homed
 * onto the network so agent containers can reach the LLM proxy.
 *
 * <p>For {@code allowInternet=true}: creates a normal bridge network.
 * The app-server is still connected to provide LLM proxy access.
 */
public class SandboxNetworkManager {
    ...
    /**
     * Disconnect the app-server from a job network.
