 ...
 *
 * <p>Controls Docker daemon connectivity, container resource limits, security
 * defaults, and reconciliation intervals. Bound from {@code hephaestus.sandbox.*}
 * in {@code application.yml}.
 *
 * <h2>Activation</h2>
 * <p>The sandbox subsystem is disabled by default ({@code enabled=false}).
 * Set {@code hephaestus.sandbox.enabled=true} to activate. Requires a reachable
 * Docker daemon.
 *
 * <h2>Deployment Requirements</h2>
 * <ul>
 *   <li>Docker Engine &ge; 25.0.5 (CVE-2024-29018 DNS exfiltration fix)</li>
 *   <li>Cloud metadata blocking: {@code iptables -I DOCKER-USER -d 169.254.0.0/16 -j DROP}</li>
 *   <li>Recommended: Tecnativa docker-socket-proxy with {@code EXEC=0}</li>
 *   <li>Recommended: {@code userns-remap=default} in {@code /etc/docker/daemon.json}</li>
 *   <li>Optional: gVisor for {@code container-runtime=runsc}</li>
 * </ul>
 *
 ...
 * @param appServerContainerId       Docker container ID of the app-server (null=auto-detect from HOSTNAME)
...
public record SandboxProperties(
     ...
     *
     * <p>When null, the caller should fall back to the {@code HOSTNAME} environment
     * variable, which Docker sets to the container's short ID by default.
     *
