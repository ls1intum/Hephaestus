...
public class NatsConsumerService {
  ...
  public void init() throws IOException, InterruptedException, JetStreamApiException {
    ...
    MessageHandler handler = msg -> {
      msg.ack();
      logger.info("Received message: " + msg.getSubject() + " metaData: " + msg.metaData());
      ...
      if (subject.endsWith(".pull_request")) {
        try {
          GHEventPayload.PullRequest pullRequestEvent = github.parseEventPayload(reader, GHEventPayload.PullRequest.class);
          logger.info("Received pull request: " + pullRequestEvent.getPullRequest().getTitle());
        } catch (IOException e) {
          logger.error("Failed to parse pull request payload.", e);
        }
      } else if (subject.endsWith(".pull_request_review_comment")) {
        try {
          GHEventPayload.PullRequestReviewComment pullRequestReviewCommentEvent = github.parseEventPayload(reader, GHEventPayload.PullRequestReviewComment.class);
          logger.info("Received pull request review comment: " + pullRequestReviewCommentEvent.getComment().getBody());
        } catch (IOException e) {
          logger.error("Failed to parse pull request review comment payload.", e);
        }
      } else if (subject.endsWith(".pull_request_review")) {
        try {
          GHEventPayload.PullRequestReview pullRequestReviewEvent = github.parseEventPayload(reader, GHEventPayload.PullRequestReview.class);
          logger.info("Received pull request review: " + pullRequestReviewEvent.getReview().getBody());
        } catch (IOException e) {
