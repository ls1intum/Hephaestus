 ...
 * <p>Two triggers:
 * <ol>
 *   <li><b>Startup</b> ({@link ApplicationReadyEvent}): RUNNING jobs with no matching
 *       container are marked FAILED. Handles crash recovery.</li>
 *   <li><b>Periodic</b> ({@link Scheduled}): orphaned containers and networks
 *       are cleaned up on a configurable interval.</li>
 * </ol>
 *
 * <p>Each operation is idempotent and wrapped in try-catch. A partial failure
 * in one resource does not block cleanup of others.
 */
...
public class SandboxReconciler {
     ...
     *
     * <p>Each job is saved independently (via Spring Data's default transactional
     * save) so a failure on one job does not roll back others.
     */
