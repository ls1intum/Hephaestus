 ...
 */
@WorkspaceAgnostic(
    "Sandbox reconciler operates on Docker infrastructure, not workspace-scoped data")
public class SandboxReconciler {
  ...
  public SandboxReconciler(
    ...
    this.orphanedJobs =
        Counter.builder("sandbox.reconciler.orphaned")
            .tag("resource", "job")
  ...
  public void onStartup() {
    ...
    Set<UUID> jobIdsWithActiveContainers =
    ...
    for (AgentJob job : runningJobs) {
      if (!jobIdsWithActiveContainers.contains(job.getId())) {
        try {
          ...
          log.error(
              "Failed to mark orphaned job as FAILED: jobId={}, error={}",
              job.getId(),
              e.getMessage());
        }
      }
  ...
  @Scheduled(
      fixedDelayString = "${hephaestus.sandbox.reconciliation-interval-seconds}",
