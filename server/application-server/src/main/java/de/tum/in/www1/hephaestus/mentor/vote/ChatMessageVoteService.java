...
public class ChatMessageVoteService {
    ...
    public ChatMessageVoteService(
            ChatMessageVoteRepository voteRepository,
    ...
    public ChatMessageVoteDTO voteMessage(UUID messageId, Boolean isUpvoted) {
        ...
        ChatMessageVote vote = voteRepository.findById(messageId)
    ...
    public Optional<ChatMessageVoteDTO> getVote(UUID messageId) {
        return voteRepository.findById(messageId)
                .map(ChatMessageVoteDTO::from);
    }
    ...
    public List<ChatMessageVoteDTO> getVotes(List<UUID> messageIds) {
        return voteRepository.findByMessageIdIn(messageIds)
                .stream()
                .map(ChatMessageVoteDTO::from)
                .toList();
    }
    ...
    public List<ChatMessageVoteDTO> getVotesForThread(UUID threadId, User user) {
        ...
        return voteRepository.findByThreadId(threadId)
                .stream()
                .map(ChatMessageVoteDTO::from)
                .toList();
    }
