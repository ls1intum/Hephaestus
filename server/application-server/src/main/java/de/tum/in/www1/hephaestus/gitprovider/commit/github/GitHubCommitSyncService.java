...
public class GitHubCommitSyncService {
    ...
    public SyncResult syncForRepository(Long scopeId, Long repositoryId) {
        return syncForRepository(scopeId, repositoryId, null);
    }
    ...
    public SyncResult syncForRepository(Long scopeId, Long repositoryId, @Nullable Instant since) {
        ...
        String sinceStr = since != null
            ? DateTimeFormatter.ISO_OFFSET_DATE_TIME.format(since.atOffset(ZoneOffset.UTC))
            : null;
        ...
        while (hasMore) {
            ...
            try {
                ...
                if (graphQlClientProvider.isRateLimitCritical(scopeId)) {
                    log.warn(
                        "Aborting commit sync due to critical rate limit: repoName={}",
                        safeNameWithOwner
                    );
        ...
        log.info(
            "Completed commit sync: repoName={}, commitCount={}, incremental={}, status={}",
            safeNameWithOwner,
            totalCommitsSynced,
            since != null,
            finalStatus
