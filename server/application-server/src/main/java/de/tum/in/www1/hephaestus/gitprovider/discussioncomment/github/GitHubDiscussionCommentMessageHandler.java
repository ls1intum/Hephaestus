...
public class GitHubDiscussionCommentMessageHandler extends GitHubMessageHandler<GitHubDiscussionCommentEventDTO> {
    ...
    protected void handleEvent(GitHubDiscussionCommentEventDTO event) {
        ...
        if (event.actionType() == GitHubEventAction.DiscussionComment.DELETED) {
            ...
            // Process the comment
            var savedComment = commentProcessor.process(commentDto, discussion, context);

            // Resolve reply threading for webhooks
            // The parent comment should already exist since webhooks arrive in order
            if (savedComment != null && commentDto.replyToNodeId() != null) {
                boolean resolved = commentProcessor.resolveParentCommentByNodeId(
                    savedComment,
                    commentDto.replyToNodeId()
                );
                if (!resolved) {
                    log.warn(
                        "Could not resolve parent comment for webhook reply: commentId={}, parentNodeId={}",
                        savedComment.getId(),
                        commentDto.replyToNodeId()
                    );
                }
    ...
    private void processDeleted(GitHubDiscussionCommentDTO commentDto) {
        commentProcessor.delete(commentDto.getDatabaseId());
    }
}
