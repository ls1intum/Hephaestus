...
public class GitHubDiscussionSyncService {
     ...
     */
    private record DiscussionWithCommentCursor(Discussion discussion, String commentCursor) {}
    ...
    public SyncResult syncForRepository(
        ...
        while (hasMore) {
            ...
            try {
                ...
                ClientGraphQlResponse response = Mono.defer(() -> client
                ...
                if (graphQlClientProvider.isRateLimitCritical(scopeId)) {
                    log.warn(
                        "Aborting discussion sync due to critical rate limit: repoName={}",
                        safeNameWithOwner
                    );
                ...
                GHDiscussionConnection connection = response.field("repository.discussions")
        ...
        if (!discussionsNeedingCommentPagination.isEmpty()) {
            ...
            for (DiscussionWithCommentCursor discussionWithCursor : discussionsNeedingCommentPagination) {
                int additionalComments = syncRemainingComments(
                    scopeId,
                    discussionWithCursor.discussion(),
            ...
            if (commentConn != null && commentConn.getNodes() != null) {
                ...
                if (commentPageInfo != null && commentPageInfo.getHasNextPage()) {
                    discussionsNeedingPagination.add(
                        new DiscussionWithCommentCursor(discussion, commentPageInfo.getEndCursor())
                    );
                }
            }
        }
     ...
     *
     * @param scopeId      the scope ID for authentication
     * @param discussion   the discussion to fetch remaining comments for
        ...
        while (hasMore) {
            ...
            try {
                ClientGraphQlResponse response = client
                    ...
                    .variable("owner", ownerAndName.owner())
                    .variable("name", ownerAndName.name())
                    .variable("discussionNumber", discussion.getNumber())
                    .variable("first", DEFAULT_PAGE_SIZE)
                ...
                Integer pageSynced = transactionTemplate.execute(status -> {
                    Repository repo = discussion.getRepository();
                    if (repo == null) {
                        return 0;
                    }
                    ProcessingContext context = ProcessingContext.forSync(scopeId, repo);
