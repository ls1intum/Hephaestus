 ...
 *
 * <p>Domain-agnostic — the sandbox manager never interprets the contents of
 * {@code inputFiles} or the data written to {@code outputPath}. Handlers
 * and adapters produce a {@code SandboxSpec}; the sandbox manager executes it.
 *
 * <p>Files are injected via {@code docker cp} (tar archive API), not bind mounts.
 * This works identically for local and remote Docker daemons, enabling future
 * multi-node runner support without any change to this contract.
 *
