...
1. **Read context** — load these files:
...
3. **Evaluate each practice** — for each practice in `index.json`, decide POSITIVE, NEGATIVE, or NOT_APPLICABLE based on the criteria in `all-criteria.md`. Verify NEGATIVE findings against actual `+`/`-` lines before reporting. Re-examine POSITIVE verdicts — if a practice's criteria are partially violated, the verdict is NEGATIVE. Use `repo/` for surrounding context but never flag pre-existing code.
