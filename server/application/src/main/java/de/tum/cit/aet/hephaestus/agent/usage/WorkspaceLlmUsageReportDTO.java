...
public record WorkspaceLlmUsageReportDTO(
    ...
    @NonNull
    @Schema(description = "Whether work on this workspace's OWN provider is currently paused.")
    Boolean ownProviderPaused,
    @NonNull List<LlmUsageByJobTypeDTO> byJobType,
    @NonNull List<LlmUsageByDayDTO> byDay,
    @Nullable
    @Schema(
        description = "Display-only conversion when the instance has a display currency. " + "Absent = show USD only."
    )
    FxRateInfoDTO fx
) {}
