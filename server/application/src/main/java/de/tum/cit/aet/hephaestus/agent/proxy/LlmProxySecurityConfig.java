...
class LlmProxySecurityConfig {
    ...
    SecurityFilterChain hideEverythingElseOnGatewayConnector(
            HttpSecurity http, SandboxGatewayProperties gatewayProperties) throws Exception {
        return hidden(http, onGatewayConnector(gatewayProperties));
    }

    @Bean
    @Order(Ordered.HIGHEST_PRECEDENCE + 3)
    SecurityFilterChain hideCapabilitiesOnOtherConnectors(HttpSecurity http) throws Exception {
        return hidden(http, PathPatternRequestMatcher.withDefaults().matcher("/internal/llm/**"));
    }

    /** Shared by every chain that matches on the gateway connector, so they provably claim the same requests. */
