...
public record OutlineCollectionDTO(
        @NonNull @Schema(description = "Internal registry row id")
        Long id,

        @NonNull @Schema(description = "Outline collection id (UUID; the natural key)")
        String collectionId,

        @Schema(description = "Collection name as shown in Outline, if known") @Nullable
        String name,
        ...
        @NonNull @Schema(description = "Mirror lifecycle state (PAUSED freezes sync but keeps documents)")
        MirrorState state,

        @NonNull @Schema(description = "Whether a clean full pass has completed since registration or the last resume")
        SyncStatus syncStatus,

        @NonNull @Schema(description = "Live (non-tombstoned) mirrored document count")
        Long documentCount,
        ...
        @Schema(description = "When this collection last completed a full reconcile pass, if any") @Nullable
        Instant lastSyncedAt,

        @Schema(description = "Last sync failure for this collection, cleared on the next clean pass") @Nullable
        String lastSyncError,

        @NonNull @Schema(description = "When the collection was registered for mirroring")
        Instant createdAt) {
