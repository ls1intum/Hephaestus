...
final class ImagePullBootstrapperSupport {
    ...
    static void applyPolicy(
        ...
        if (!imageOps.ping()) {
            ...
            meterRegistry.counter(skippedMetric, "reason", "docker_unreachable").increment();
            return;
        }
        log.info("Pulling image {} (policy={}) …", image, policy);
        Timer.Sample sample = Timer.start(meterRegistry);
        boolean ok = imageOps.pullImage(image);
        sample.stop(meterRegistry.timer(durationMetric, "outcome", ok ? "success" : "failure"));
        if (ok) {
