...
public record ConnectionSyncStatusDTO(
        ...
        @NonNull @Schema(description = "Raw connection lifecycle state")
        IntegrationState connectionState,

        @NonNull @Schema(description = "Derived health") ConnectionHealth health,

        @Schema(description = "Most recent successful job's finish time") @Nullable
        Instant lastSuccessfulSyncAt,

        @Schema(description = "Currently PENDING/RUNNING job, if any") @Nullable
        SyncJobDTO activeJob,

        @Schema(description = "Most recently finished job, if any") @Nullable
        SyncJobDTO lastJob,

        @Schema(description = "When the next periodic reconciliation is expected") @Nullable
        Instant nextScheduledSyncAt,
        ...
        @Schema(description = "When the last inbound webhook/event was processed for this connection, if any") @Nullable
        Instant lastEventProcessedAt,

        @Schema(description = "Current rate-limit budget, if known") @Nullable
        RateLimitSnapshotDTO rateLimit,
        ...
        @Schema(description = "Connection-level backfill rollup, if applicable") @Nullable
        BackfillSummaryDTO backfill,

        @NonNull @Schema(description = "Resource-level rollup")
        ResourceCountsDTO resourceCounts) {}
