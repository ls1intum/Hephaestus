...
description: Visualize the PostgreSQL schema generated for Hephaestus.
---

:::note
Regenerate the diagram with `npm run db:generate-erd-docs`. The command updates `schema.mmd`, which feeds the Mermaid diagram below.
:::
