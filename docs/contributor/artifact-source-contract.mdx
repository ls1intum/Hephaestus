...
refuse the signal with a typed reason rather than silently doing nothing, and the reason — not the caller —
...
- **Practice autonomy.** A practice at `OFF` admits no review; `HUMAN_APPROVAL` and `AUTOMATIC` both run it
  ...
  `OFF`, the signal is refused with `PRACTICE_AUTONOMY_OFF`, which is retryable: turning a practice back on
  should let the waiting occurrence through.
- **Review coverage.** The repository/base-branch axis and the person axis are intersected, and both must
  admit the work. A repository or base branch outside the selection, a selected list left empty, or an
  author who is not a member of the workspace is refused with terminal `OUT_OF_REVIEW_SCOPE`. Work with no
  identifiable author is refused with retryable `SUBJECT_UNLINKED` instead, because an identity can still
  arrive later.
