...
unresolved thread. Where the corpus was not whole, the honest answer is `Presence.INCONCLUSIVE`, never a
...
Neither can be reviewed, so a practice declaring one is forced to autonomy tier `OFF` when it is created or
...
- **Autonomy tier.** A practice at `OFF` admits no review; `PROPOSE` and `DELIVER` both run it and differ
  only in whether the result is sent. When every practice bound to a signal sits at `OFF`, the signal is
  refused with `PRACTICE_TIER_OFF`.
...
`CONSENT_NOT_ACTIVE`, and `PRIVACY_POLICY`. There is no redacted model-facing variant. Treat a reason code
