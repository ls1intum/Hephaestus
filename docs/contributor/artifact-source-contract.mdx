...
2. **Practice binding:** the occasion a review runs on and the evidence that review _requires_ before it may
...
`SourceAbsenceReason` saying _why_ — `GOVERNANCE_NOT_EFFECTIVE`, `NO_PROVIDER`, `NO_WORKING_COPY`,
...
watermark recording when a copy was last _verified_ against upstream; the mirror records only when a row was
last _written_, so an age derived from it measures time since the last upstream change — reporting a
...
version the runtime no longer ships is _unreplayable_ rather than invalid: the recorded result stands, and
...
front of it. Those are two practices, not two occasions on one — one row, one autonomy tier and one feedback
...
| `EXHAUSTIVE` | The claim reads this source and asserts something is _not_ in it. Required, and additionally not satisfiable by a partial capture whatever the source contract's floor happens to be. |
...
- **Autonomy tier.** A practice at `OFF` admits no review; `HUMAN_APPROVAL` and `AUTOMATIC` both run it and differ
