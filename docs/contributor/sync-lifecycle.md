...
| **Upstream deletion** | `RECONCILIATION` sweep tombstones issues/PRs (fail-closed). `repository.deleted` webhook removes repo + monitors. | `RECONCILIATION` sweep tombstones issues/MRs (fail-closed). GitLab emits **no** issue/MR deletion webhook at all. | **No sweep.** `message_deleted` → tombstone; `channel_deleted` → erase our copy; `channel_archive` / `channel_left` → PAUSED. | Every **clean** full enumeration tombstones that collection's vanished documents; `documents.delete` / `documents.permanent_delete` tombstone immediately. |
...
- **Slack and Outline have no deletion *sweep* in the SCM sense.** Slack's content model is
  append-plus-watermark and a message absent from a `conversations.history` page usually means
  pagination truncation, a filtered subtype, or a thread reply — inferring deletion from absence
  would mass-delete. Outline does tombstone vanished documents, but inline in every clean
  enumeration rather than gated on job type, so an Outline `INITIAL` can tombstone where an SCM one
  cannot.
