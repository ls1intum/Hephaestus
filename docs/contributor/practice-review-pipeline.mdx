...
| `PRACTICE_TIER_OFF` | An admin raises a watching practice above `OFF` — at the practice, its area, or the workspace default |
...
- **`PracticeReviewTier`** — how much autonomy the system has over this practice: `OFF`, `OBSERVE`,
  `PROPOSE`, `DELIVER`. Only `DELIVER` delivers without a person approving it. The tier is the
  **effective** one, resolved practice → area → workspace, because either of the first two levels may hold
  no opinion. Withheld here means `PRACTICE_TIER_QUIET`.
...
So a practice at `OBSERVE` is `REVIEWED` with `deliveredCount = 0` and `PRACTICE_TIER_QUIET` in
...
`OBSERVE` does, and `PRACTICE_TIER_QUIET` already covers it.
