...
**An observation carries `reasoning` and no advice.** `reasoning` is the measurement's own
justification — *"the change adds a tax-exempt branch, and no test file in this diff calls `total`"* —
and it is the only prose the observation persists. The detector also emits a `guidance` string, but
that string is not a column and never becomes one: advice is a property of a *delivery*, so it lives on
the feedback ledger instead (ADR 0021, ADR 0022). What `guidance` is still for is the subject of the
next section.
...
**An intervention cannot be true or false.** *"Write the assertion that distinguishes the new branch
before you write the branch"* is only useful or useless, and whether it is useful depends entirely on
...
starts bending toward whatever makes good advice — the finding you can write a nice tip about gets
reported, the one you cannot gets quietly dropped. And advice written at the moment of measurement can
...
tools — `read`, `grep` and `report_feedback`. No `bash`, and no `report_finding`, so the stage cannot
...
`IN_CHAT` is composed as a **move**, not a script: an `opener` (a question asked before anything
is told), the `evidence` (shown only once they answer), and the `target` (what the turn should leave
them able to do). It is stored as JSON on `feedback.body` via `ConversationBriefBody` and handed to the
mentor under a `move` key by `PreparedConversationFeedbackContentSource` — deliberately never under
`body` or `message`, because it is not text to speak. The mentor still writes the words of the turn, so
nothing goes stale waiting to be raised. A unit with no composed move has that key **missing**, not
empty, so "the composer said nothing here" cannot read as "the composer had nothing to say".
...
- **No typed citation, path or line number.** An `IN_CONTEXT` unit names `{ observationId,
  ...
  this change's diff. The composer therefore *cannot* invent an anchor, which retires the class of
...
- **In context has a fallback, so it needs no durability.** `DeliveryComposer` prefers the composed
  unit for a practice and otherwise renders the finding's own `reasoning` and `guidance`. That fallback
  is not a degraded stub — it is exactly the comment that shipped before composition existed. **This is
  why `report_finding` still requires `guidance` even though `guidance` is still not a column:** it is
  the floor under a stage that is permitted to fail.
...
`IN_CONTEXT` bodies are already public on the work, and an `IN_CHAT` body holds a *move* for the mentor
