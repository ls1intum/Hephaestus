...
- **`PracticeReviewTier`** — how much autonomy the system has over this practice: `OFF`, `PROPOSE`,
  `DELIVER`. Only `DELIVER` delivers. `PROPOSE` runs the review and records every observation, and the
  findings are dropped before a body is composed, so nothing is sent and nothing is stored to send later.
...
finding reaches a reader at all; the `SUPPRESSED` ledger row behind it is storage, not a screen.
