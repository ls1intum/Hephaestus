...
belongs to the reason, not to the caller: each `SignalStateReason` declares the state it resolves to. Which
reason to reach for, and the two that are easy to confuse, are in
