...
`DeliveryComposer`'s own contract says so: an observation's reasoning and guidance are written by the
detector, and the renderer only sanitises and lays them out. The composer reads
...
- **`ObservationOrigin`** — `LIVE` and `MANUAL` deliver everywhere; `BACKFILL` is entitled only to
  `REFLECTION`. Withheld here means `BACKFILL_QUIET`.

`FeedbackChannel.REFLECTION` is the developer's own **My feedback** page: the process level of the three,
where a habit that recurs across several pieces of their work is named along with what to do differently
next time. Its body is the only text the system composes about a named person that the person is the sole
audience for — `IN_CONTEXT` bodies are already public on the work, `CONVERSATION` bodies are NULL until
the mentor speaks them — so an ArchUnit rule (`ReflectionChannelProducerArchTest`) keeps the set of
classes that may write one or read one back small and named, and `ReviewFeedbackQueryService` withholds
the body from every operator surface. The `BACKFILL` entitlement is a ceiling, not an instruction: a
