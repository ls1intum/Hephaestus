...
flowchart TB
    ...
    Observe -->|cannot tell| Indeterminate[observation.presence INDETERMINATE]
...
| Observation | `Presence.INDETERMINATE` | `observation.presence` | We looked and could not tell |
...
| Reason | What lifts it |
| --- | --- |
| `WORKSPACE_INACTIVE` | The workspace becomes active again |
| `PRACTICES_DISABLED` | An admin starts practice reviews |
| `NO_ACTIVE_PRACTICE` | A practice covering this kind of work is installed |
| `REVIEW_MODEL_UNBOUND` | An admin binds a model for practice review |
| `PRACTICE_TIER_OFF` | An admin raises a watching practice above `OFF` |
| `BUDGET_EXHAUSTED` | The cap is raised, or the month rolls over |
| `SUBJECT_UNLINKED` | The author links a Hephaestus account |
| `MODEL_UNAVAILABLE` | The bound model or its connection becomes usable |
...
`Presence` — `PRESENT`, `ABSENT`, `NOT_APPLICABLE` or `INDETERMINATE` — and, only when the presence carries
valence, an assessment. `PracticeDetectionResultParser` force-nulls the assessment on an `INDETERMINATE`
finding, so a detector that hedges and still attaches a strength cannot slip an unearned one into the
series.

`INDETERMINATE` means the evidence the practice needs *was* present and *was* read and does not settle the
...
(`insertIfAbsent`, `ON CONFLICT DO NOTHING`), and a recurrence key hashes the locus so the same finding is
recognisable across runs.
...
Delivery turns admitted findings into a message and records what happened to it, including when nothing was
sent. A withheld finding is written to the ledger as `SUPPRESSED` with a reason rather than dropped, so "we
saw it and chose to stay quiet" reads differently from "we missed it" in every later evaluation.

Two gates decide admission, and they are ANDed:

- **`PracticeReviewTier`** — the workspace's per-practice loudness: `OFF`, `MEASURE`, `COACH`, `ENGAGE`.
  `IN_CONTEXT` needs `ENGAGE`; `CONVERSATION` needs `COACH` or `ENGAGE`. Withheld here means
  `PRACTICE_TIER_QUIET`.
...
one findings model. Provider adapters own provider identifiers, diff and comment APIs, and conversation
...
- **Findings** — the agent reports through `report_finding`; `PracticeDetectionResultParser` validates and
  never throws, and persistence assigns occurrence and recurrence identity.
...
So a practice at `MEASURE` is `REVIEWED` with `deliveredCount = 0` and `PRACTICE_TIER_QUIET` in
`withheldReasons` — measured, and deliberately quiet. `TURNED_OFF` is the workspace's own choice and is
checked *before* any mechanical reason, so it never masks one. It is not called `SILENCED`: silence is what
`MEASURE` does, and `PRACTICE_TIER_QUIET` already covers it.
