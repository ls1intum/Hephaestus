...
The stages are named Occasion, Capture, Observation and Delivery in code, in commits and on screen. Any other
name for one of them — an act number, a phase letter, a sprint label — is ungreppable and invites a fifth.
...
An occasion is an _occurrence_ — one named signal on one revision of one artifact — recorded in
...
| `PRACTICES_DISABLED`   | An admin starts practice reviews                                                                      | Practices → Review → _When and where_                                |
...
| `PRACTICE_TIER_OFF`    | An admin raises a watching practice above `OFF` — at the practice, its area, or the workspace default | Practices → Review → _How much_                                      |
...
- **Three doors collapse into `MANUAL`.** `ManualReviewRequests` is the single recording site for both the
  `/hephaestus review` comment command (via `BotCommandProcessor`) and the _Review this now_ button (via
...
to say something to a person. Those are two different acts, they run as two separate model turns
inside one agent job, and keeping them apart is the reason one fact can read three different ways.
...
anybody should do, and like any instrument reading it is kept forever, unedited, so later readings can
be compared with it.

**An intervention cannot be true or false.** _"Write the assertion that distinguishes the new branch
before you write the branch"_ is only useful or useless, and whether it is useful depends entirely on
who is reading it, where they are reading it, what they were told last week, and whether anybody has
said it to them already.
...
flowchart TB
    ...
    subgraph phase1["Phase 1 — measurement"]
        measure["Read the work against each practice"]
        measure --> obs["Observations: what is there,<br/>with the citation that proves it.<br/>Kept forever, never edited."]
    end

    obs --> compose
    record["Everything measured before<br/>about this person"] --> compose
    said["Everything already said to them,<br/>and everything queued but unread"] --> compose

    subgraph phase2["Phase 2 — intervention"]
        ...
        compose --> chat["IN_CHAT: a question, then the evidence"]
...
`runCompositionStage` in `pi-runner.mjs` opens a **separate in-memory session** with exactly three
tools — `read`, `grep` and `report_feedback`. No `bash`, and no `report_observation`, so the stage cannot
touch the review's own result file. It is a separate session rather than another turn on the review's
for two reasons worth keeping: re-using the review's session would re-bill the whole conversation, and
it would invite the composer to treat its own reasoning as evidence.

It is prompted with `agent/feedback-composer.md` and reads:

| Input                                                      | Where                                                       | Staged by                                          |
| ---------------------------------------------------------- | ----------------------------------------------------------- | -------------------------------------------------- |
| This run's observations, projected                         | `work/composition/observations.json`                        | the runner itself, from the review's result file   |
...
Output is `out/feedback.json`, parsed by `FeedbackCompositionResultParser` into `ComposedFeedbackUnit`s.

Three properties of that list matter more than the paths:
...
- **The delta is arithmetic, not judgement.** `ObservationDelta.classify` groups a person's window by
  ...
  `RESOLVED` is the whole reason it exists: _"the gap from last week is closed"_ originates from a locus
  that is in the prior set and absent from the newest run, so no mapping that walks this run's
  observations can ever reach it. A vanished _strength_ is deliberately not `RESOLVED` — that would
...
direction. All three names answer one question — _where does this land?_ — and
...
| Messages per run | at most 3                                                                 | at most 2                                | at most 3                                                                                        |

**The mentor renders in the app too, and that does not make it `IN_APP`.** `IN_CHAT` is the _dialogic_
channel — the unit becomes a turn the developer can answer — wherever the conversation runs. `IN_APP` is
the _non-dialogic_ practice surface — the unit is written down, and reading it is the whole interaction.
Ask _is it a turn?_ before asking _which screen?_: a new written surface inside Hephaestus is `IN_APP`,
and a mentor that moves to another chat product is still `IN_CHAT`.

Two of the rows are the load-bearing ones. The public lane never says _"you keep doing this"_ — that
...
An issue review has no diff to anchor to, so `IssueReviewHandler.ISSUE_REVIEW_CHANNELS` is written as
the complement of `IN_CONTEXT` rather than as a list — a fourth channel would join it by default, which
is the safe direction. The per-lane message caps above restate caps the lanes themselves own;
`FeedbackCompositionCapsTest` is what stops the two copies drifting.
...
The notes are stored as JSON on `feedback.body` via `ConversationBriefBody` and handed to the mentor under
a `notes` key by `PreparedConversationFeedbackContentSource`; the composed title is staged separately as
`topic`. Neither is placed under `body` or
`message`, because none of it is text to speak. The mentor re-composes the whole turn against the live
conversation, so nothing goes stale waiting to be raised. A unit with no composed notes has that key
**missing**, not empty, so "the composer said nothing here" cannot read as "the composer had nothing to
say".

`ConversationBriefBody` accepts exactly this shape. There are no field aliases, version readers, or fallback
conversation schemas: producer, parser, persistence and mentor context share the same four names, and a
structural test fails when the Java record, runner schema and composer prompt drift.
...
- **No typed citation, path or line number.** An `IN_CONTEXT` unit names `{ observationId,
citationIndex }` and the server resolves the file, side and line from that observation's own
  citation — and refuses a citation whose `anchorable` flag is false, meaning it does not fall inside
  this change's diff. The composer therefore _cannot_ invent an anchor, which retires the class of
  failure that grounding checks previously had to catch after the fact.
- **No invented supersession target.** `action: "SUPERSEDE"` must name a `threadKey` that was staged in
  `prepared.json`.
- **No second message on the same lane about the same practice.** One unit per `(channel,
...
`DELIVERED` is never retired on the longitudinal lanes, because on those lanes delivered means _read_.
...
- **`PracticeReviewTier`** — how much autonomy the system has over this practice: `OFF`, `PROPOSE`,
  ...
  axis narrowing _where_ a delivering practice may speak: `DELIVER` delivers wherever the reviewed work
...
`GET /practices/feedback/in-app`; the developer's practice pages are where it will surface. Its body is
...
rather than words for the developer (and is NULL when nothing was composed for the lane) — so an
...
The most counter-intuitive facts about this system are all about _where_ code runs, and none of them are
...
sequenceDiagram
    ...
    W->>W: parse observations, gate delivery, compose
...
- **Evidence capture runs on the claiming worker, not on the server.** `handler.prepareInputs` — and
  ...
  after the claim. The beans themselves carry no runtime-role condition; it is the _call chain_ that is
...
- **Observations** — the agent reports through `report_observation`;
  ...
  recurrence identity.
- **Feedback** — the agent reports through `report_feedback` in a second session;
...
`TURNED_OFF` is the workspace's own choice and is checked _before_ any mechanical
