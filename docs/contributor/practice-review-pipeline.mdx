...
indistinguishable from "delivery policy refused it".
...
- `HUMAN_APPROVAL` stores in-context feedback as `AWAITING_APPROVAL`. A workspace owner or administrator may
  approve the exact content and target or reject it with an optional structured reason. Approval changes the
  unit to `PREPARED` for asynchronous release; rejection records `DISCARDED`.
...
At release, approved feedback rechecks Silent Mode, workspace availability, artifact validity, repository
scope and recipient preference, then reconciles a stable proposal marker with the provider before posting.
An approval overrides none of those controls. Missing effective autonomy denies automatic delivery.
