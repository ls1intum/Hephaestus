...
values are in [Practice autonomy](./practice-review-glossary.mdx#practice-autonomy), and the ordered list of
gates every delivery walks is
[the delivery policy check order](./practice-review-glossary.mdx#the-delivery-policy-check-order). Do not
restate either here. Four facts belong to this stage rather than to that vocabulary.

**One resolver, four stages.** `DeliveryPolicyResolver` runs the same ordered list at `COMPOSITION`,
`AUTOMATIC`, `APPROVED` and `EGRESS`, so a lane cannot acquire a gate the others do not have, and a
question that cannot be asked yet is recorded as `NOT_APPLICABLE` instead of being answered `false`. The
difference between the stages is only which facts are knowable, never which rules apply.

**`PracticeAutonomyPolicy` is a second, narrower predicate, and each channel's router applies it itself.**
It admits a unit only when the observation's immutable `ObservationOrigin` entitles it to that channel *and*
the effective autonomy is `AUTOMATIC`. That is why a backfilled observation can never reach the artifact or a
mentor turn whatever anybody configures: the entitlement is a property of how the measurement was obtained.
The two halves answer different questions — origin asks what a provenance may *ever* be said on, and the
lane asks whether today is the day.

**Prospective by construction.** Existing `AWAITING_APPROVAL` rows stay proposals when autonomy changes, so a
promotion to `AUTOMATIC` cannot release historical output; demoting to `HUMAN_APPROVAL` keeps measurement
running while returning new interventions to supervised release. Missing effective autonomy denies automatic
delivery.

**Approval is not a bypass.** Release re-verifies the approved content against what is about to be sent
(`APPROVAL_STALE`) and that the contributing practices still require approval
(`APPROVAL_NO_LONGER_ELIGIBLE`), then walks the whole check order again at the `APPROVED` stage, then
reconciles a deterministic provider marker so a retried dispatch cannot post twice.
