...
flowchart TD
    ...
    accDescr: A user task moves through issue creation, implementation, validation, review, and completion.
    A[👤 User sends task] --> B[🤖 Agent creates beads issue]
    B --> C[🤖 Agent implements]
    C --> D{🤖 Agent validates}
    D -->|Fail| C
    D -->|Pass| E[👤 Review in Source Control]
    ...
    R --> S{Approved?}
    S -->|Changes requested| T([👤 /resolve-review])
    T --> C
    S -->|Approved| U[Merge]
```
...
These slash commands work in **GitHub Copilot** (`.github/prompts/`) and **Antigravity** (`.agent/workflows/`):
