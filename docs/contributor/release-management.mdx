...
- **Migrations are first-class:** if the PR adds a Liquibase changelog, write
  `Includes an automatic database migration.` and bump at least `minor`. The release notes additionally
  get an automatic "back up before upgrading" banner whenever a release touches
