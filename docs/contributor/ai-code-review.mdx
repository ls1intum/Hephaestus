...
A single backend powers practice detection: the **Pi practice agent** (`PiPracticeAgent`). It uses a single-pass architecture — one agent evaluates all relevant practices for the diff and persists structured findings via custom Pi tools (`report_finding`, `set_review_summary`).
...
```bash
curl -X POST "http://localhost:${SERVER_PORT}/actuator/dev/trigger-review?prId=123&workspaceId=1"
```
