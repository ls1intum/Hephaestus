...
description: Architecture and internals of the automated practice review system.
---

Hephaestus reviews pull requests and merge requests against configurable engineering practices. When a developer opens an eligible, non-draft change request or uses `/hephaestus review`, the system detects relevant practices, runs an LLM agent inside a sandboxed container, parses the structured output, and posts feedback as a summary comment and inline comments.
...
sequenceDiagram
    ...
    participant DB as PostgreSQL (agent_job)
    ...
    AS->>DB: INSERT agent_job (QUEUED)
    Note over AS,DB: The agent_job table IS the queue
    DB->>AS: Poll + claim FOR UPDATE SKIP LOCKED
...
| ----------------- | ------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Detection gate    | `PracticeReviewDetectionGate`   | 10-check gate, ordered cheap-to-expensive: workspace resolution, draft skip, `practicesEnabled` flag, trigger mode, runnable practice binding, practice matching, `runForAllUsers` bypass, assignee presence, role-checker health, assignee role |
...
| Job executor      | `AgentJobExecutor`              | Polls the `agent_job` table for `QUEUED` work — the table is the queue — claiming with `FOR UPDATE SKIP LOCKED`, dispatching to the sandbox executor pool, and persisting results in micro-transactions. See [ADR 0025](https://github.com/ls1intum/Hephaestus/blob/main/docs/decisions/0025-agent-job-queue-on-postgresql.md) for why this is not NATS |
...
The current deployment uses 37 practices across 12 practice areas. Practices are fully configurable per workspace and can be added or modified without code changes.
...
4. **Compose** -- `DeliveryComposer` partitions findings into:
   ...
   - When all findings are `GOOD` strengths, composes a short positive comment naming the top strengths
...
Developers can type `/hephaestus review` in a change-request comment to retrigger a review. The flow:
...
| ------------------ | ------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| `workspace_agent_binding` | `workspace_id`, `purpose` (`PRACTICE_DETECTION` or `MENTOR`), `enabled`, `instance_model_id` XOR `workspace_model_id` (binds to the instance catalog or the workspace's own connected provider), `timeout_seconds`, `max_concurrent_jobs`, `allow_internet` | Which model each purpose runs on, at most one row per `(workspace, purpose)`. Credentials never live here: they sit on the connection row and the LLM proxy resolves them live at call time, so a rotated key takes effect immediately |
| `agent_job`        | `status`, `available_at`, `idempotency_key`, `job_token` (encrypted), `config_snapshot` (JSONB), `delivery_status`, `llm_*` token columns | Job lifecycle: QUEUED → RUNNING → COMPLETED/FAILED, and the queue itself. Tracks exit code and per-attempt token usage; cost is not stored here — it is priced once into `llm_usage_event` when the attempt ends |
...
hephaestus:
  agent:
    ...
    enabled: false # AGENT_ENABLED — turns on agent job processing (the queue is the agent_job table)
  sandbox:
