...
The layout is split by **location**, not lore (ADR 0020): `inputs/` is read-only (the path-guard whitelists exactly this subtree), `work/` is writable scratch that is never collected, and `out/` is the only directory collected back into SQL. The constants live in `WorkspaceAbi`; see `docs/developer/agent/workspace-abi.mdx` for the full ABI.
