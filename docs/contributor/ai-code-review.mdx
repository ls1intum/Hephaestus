...
| ----------------- | ------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Detection gate    | `PracticeReviewDetectionGate`   | 9-check gate: label opt-out, draft skip, workspace resolution, agent config, practice matching, `runForAllUsers` bypass, assignee presence, Keycloak health, assignee role |
...
The system supports two LLM backends, both running single-pass (one agent evaluates all practices):
...
4. **Compose** -- `DeliveryComposer` partitions findings into:
   ...
   - Returns `null` if all findings are positive (silence = approval)
