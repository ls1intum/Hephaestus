...
The system supports two LLM backends. Both are selected per workspace via the `agent_config` table and both return the same normalized findings schema:
...
- **Pi** (`PiAgentAdapter`) -- Uses a staged `settings.json` file plus a Node runner (`.run-pi.mjs`) that validates the output, retries once with session continuation, and persists usage diagnostics for post-mortem analysis.

The server is backend-agnostic after the agent returns.
...
/workspace/
  ...
  .run-pi.mjs                        # Runner script (Pi only)
  CLAUDE.md                          # Orchestrator instructions (Claude Code only)
  .pi/AGENTS.md                      # Pi runtime instructions (Pi only)
  .pi-runtime/settings.json          # Pi runtime settings (Pi only)
...
3. The agent prompt files (`CLAUDE.md`, `PI-AGENTS.md`) may need minor adjustments for language-specific tooling (e.g., build commands, linter integration)
