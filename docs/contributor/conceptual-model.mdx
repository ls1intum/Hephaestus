...
The domain model captures **what exists in the problem world** — independent of how Hephaestus implements it. Seven concepts form a pipeline from raw activity to developmental insight.
...
classDiagram
    ...
    Project "1" --> "*" Artifact : scopes
    Participant "1" --> "*" Activity : performs
    Activity "*" --> "1" Artifact : on
    Activity "*" --> "*" Signal : aggregates into
    Signal "*" --> "*" Practice : bears on
    Practice "*" --> "*" Trajectory : tracked over time
...
| 2 | **Participant** | Person (or agent) developing professional practice | The subject of observation and coaching |
| 3 | **Artifact** | Tangible work product mediating activity | What we can observe — PRs, reviews, issues, comments |
| 4 | **Activity** | Normalized, immutable record of what happened | Subject–verb–object, platform-agnostic, idempotent |
...
| 7 | **Trajectory** | Developmental arc over time | Direction, rate, phase — not just snapshots |
...
| **Cognitive** | Learning & self-regulation | Declining bad practice rate, reflection depth, achievement velocity |
...
| Trajectory | Isolated coder → collaborative engineer | Pixel-pusher → user-centered designer | Protocol-follower → independent investigator | Surface learner → self-regulated |
...
2. **Interact**: Participant → Web App → Application Server (via Keycloak auth)
3. **Coach (AI Mentor)**: Participant → Web App → Intelligence Service → LLM Provider
4. **Detect (Practices)**: Application Server → Intelligence Service → LLM → PostgreSQL
...
Hephaestus operationalizes all six methods of **Cognitive Apprenticeship** (Collins, Brown & Newman, 1989). The mapping emerged organically from sound pedagogical intuition and is now made explicit:
