...
| Application Server   | `server/**`, `scripts/**` (includes webhook receiver — ADR 0008) |
...
would otherwise skip the workflow that runs it.
...
login step and no repository secret to configure. Base image metadata is resolved anonymously, which
is occasionally rate-limited.
