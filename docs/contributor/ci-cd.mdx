...
| `ci-build.yml`         | Called by cicd.yml    | Packages the server reactor once; `Test` and the application-server image job consume the artifact |
| `ci-tests.yml`         | Called by cicd.yml after `Build` | Server unit, architecture, integration, API and database contracts, and browser E2E, all against the packaged reactor |
| `ci-docker-build.yml`  | Called by cicd.yml    | Dockerfile image builds (webapp, agent, PostgreSQL) and aliases for unchanged images |
| `reusable-docker-build.yml` | Called by cicd.yml for the application-server image and by ci-docker-build.yml | Builds, signs, attests, and blocks on the release vulnerability policy for the `linux/amd64` image |
...
| `setup-caches` | Sets up the JDK and the Maven dependency cache; `application-server-reactor` jobs also restore the generated-client build cache, `application-server-tests` jobs run goals against the packaged reactor and need no build cache |
| `restore-server-build` | Downloads the packaged reactor and installs its generated-clients JAR so single-module Maven goals resolve it; outputs the executable JAR path |
...
The server reactor is compiled and packaged exactly once per run, by `Build`. The unit and
architecture tier, the integration tier, the API and database contracts, the browser E2E suite and
the application-server image all download that artifact and run Maven goals or `pack` against it;
none of them compiles. This is the ["only build your binaries
once"](https://www.informit.com/articles/article.aspx?p=1621865&seqNum=3) rule of the deployment
pipeline: the JAR the tests ran is the JAR inside the image, and the OpenAPI spec is scraped from
that same JAR.

### Parallel Execution

- Quality legs (App Server, Tooling and Docs, Webapp, Stories) start immediately from source.
- Test legs (unit and architecture, integration, API and database, E2E) start as soon as the reactor is packaged and run in parallel. Webhook reception is part of the app-server test surface since ADR 0008.
