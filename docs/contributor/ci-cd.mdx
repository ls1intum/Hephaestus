...
| Webapp TypeScript | Biome (`webapp/biome.json`) + tsc | Lint + format + typecheck |
| Agent + scripts TypeScript | Biome (`biome.jsonc`) + tsc | Lint + format + typecheck for the Bun runtime, its specs, both precompute trees and `scripts/**` |
| Agent runtime   | Bun         | Runner and precompute specs, on the Bun the sandbox ships |
...
`biome.jsonc`, `package.json` and `pnpm-lock.yaml` are listed because they decide the verdict of the
App Server leg's Biome step: the rule set, the `:agents` scripts that invoke it, and the Biome binary
version respectively. A gate whose own configuration can change without re-running it is not a gate.
...
cd server && ./mvnw test -P'!quick'  # Unit tests — see the testing guide for why `!quick` is mandatory

# Agent runtime (Bun) — the Pi runner and the practice precompute scripts
pnpm run test:agents           # Runner + precompute specs
pnpm run check:agents          # Biome lint + format + import order, then both typechecks
