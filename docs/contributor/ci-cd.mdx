...
| semantic-release      | ~1 min                     | Creates tag + GitHub Release |
...
| `release.yml`          | On CI/CD Success      | Creates tag + GitHub Release, deploys staging, gates production |
...
| Application Server   | `server/**`, `scripts/db-utils.sh`         |
| Webhook Ingest       | `webhook-ingest/**`, root package files                |
...
- 7 test types run in parallel (3 app-server, 3 webapp, 1 webhook-ingest)
...
# Webhook Ingest
pnpm run check:webhook-ingest
cd webhook-ingest && pnpm run test
```
...
- **Application Server**: Unit, integration, and architecture tests (incl. the in-process Pi mentor agent)
