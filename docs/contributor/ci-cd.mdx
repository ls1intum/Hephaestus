...
| `setup-caches` | Sets up the JDK and restores the Maven dependency cache with a prefix fallback; `application-server-reactor` jobs also restore the generated-client build cache and are the only jobs that write caches, `application-server-tests` jobs run goals against the packaged reactor |
...
`Build` compiles and packages the server reactor once. The OpenAPI and schema gates, the database
contract tests, the browser E2E suite and the application-server image all download that artifact
and run Maven goals or `pack` against it; none of them compiles. This is the ["only build your
binaries once"](https://www.informit.com/articles/article.aspx?p=1621865&seqNum=3) rule of the
deployment pipeline: the JAR the E2E suite ran is the JAR inside the image, and the OpenAPI spec is
scraped from that same JAR.

The unit, architecture and integration suites are the deliberate exception. Each is longer than the
package job, so waiting for the artifact would delay the longest paths by more than the compile they
save, and neither suite ships anything. They compile from source in `Test` and start immediately.

### Parallel Execution

- Quality legs (App Server, Tooling and Docs, Webapp, Stories) start immediately from source.
- The unit, architecture and integration suites start immediately from source; the artifact gates (OpenAPI, schema, database, E2E, image) start as soon as the reactor is packaged. Webhook reception is part of the app-server test surface since ADR 0008.
