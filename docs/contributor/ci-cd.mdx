...
GitHub separately enforces independent review: the latest reviewable push requires approval from
someone other than its pusher. The `Maintainer` team can bypass that requirement when merging a pull
request, but cannot bypass required checks or the merge queue.
