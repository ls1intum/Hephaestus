...
description: Trunk-based CI/CD through the merge queue; staging follows main, and a release deploys to production after approval.
---

# CI/CD Architecture

Hephaestus uses trunk-based delivery through GitHub's merge queue. Pull requests provide early
feedback, merge groups validate projected `main`, and pushes to `main` produce release images. Staging
follows every successful build of `main`; a release with a signed image lock deploys to production
after approval.
...
flowchart TD
    ...
    accDescr: Pull-request and merge-group validation protect main; staging follows every main commit, and final-SHA images become signed releases that deploy to production after approval.
    ...
    Images --> Staging[Staging follows the commit]
    ...
    Verify -->|Pass| Await[Wait for staging to run this commit]
    Await -->|Approve| Prod[Deploy to production]
```
...
| Staging | every successful `main` build | `promote.yml`, dispatched by `continuous-staging.yml`; the host pulls the commit the channel names | none |
| Production | a published release, once staging runs its commit | release publication | required reviewer |

Staging follows `main` and production runs releases — [ADR 0042](https://github.com/hephaestus-build/Hephaestus/blob/main/docs/decisions/0042-hosts-pull-their-own-releases.md)
has the reasoning and [Pull-based deployment](../admin/pull-based-deployment) the host side.
...
A deploy starts the stacks in the order [Production setup](../admin/production-setup#deployment-steps-reference-stack)
gives, on the push workflow and the pull reconciler alike. The `proxy` stack publishes :80 and :443,
so a host that already has an edge in front of it leaves `proxy` out: the push workflow reads the
environment variable `DEPLOY_EDGE_PROXY=false`, which drops `proxy` from both the render and the
deploy, and a pulling host omits it from `HEPHAESTUS_STACKS`. Staging is such a host: it shares one
with the Coolify instance that serves preview deployments, and that proxy holds both ports — a second
edge there fails the deploy on a port that is already allocated.
...
| `continuous-staging.yml` | Successful CI/CD push on `main` | Dispatches `promote.yml` to follow the commit on staging and watches that run |
| `promote.yml` | Manual dispatch, or `continuous-staging.yml` | Signs the channel that moves an environment to a release or commit, behind that environment's gate, and waits for the host to converge |
| `release.yml` | Successful CI/CD push on `main` | Publishes the release, waits for staging to be running its commit, then deploys production behind the Production environment gate |
...
| `deploy-prod.yml` | Called by `release.yml` or manual dispatch | Deploys a verified release lock to production through `deploy-locked-compose.yml` |
...
`detect-changes` in `cicd.yml` owns the filters that select legs from the changed paths; read them
there. What the list does not say on its own:

- Same-repository pull requests run `Build` and `Docker` regardless of paths, because previews need
  every image at the head SHA.
- `vite.config.ts`, `.vite-hooks/**`, `.java-version`, `cicd.yml`, `ci-quality-gates.yml` and the
  setup actions select every quality leg: the task graph, the hooks and the JDK decide every leg's
  verdict, so a change to one runs all of them.
...
- The boot-smoke filter is the whole tree Spring binds configuration from, and
  `ci-contract.test.ts` fails if a source carrying `@ConfigurationProperties` or
  `@ConfigurationPropertiesScan` is not selected.
- Each leg has a filter naming its own workflow files and actions, and any `.github/workflows/**`
  or `.github/actions/**` change runs Actionlint and Zizmor.
