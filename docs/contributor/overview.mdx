...
Hephaestus is maintained by the Applied Education Technologies (AET) team at TUM. It detects practices from full pull-request lifecycle activity and routes findings — with evidence and a recommended action — through the channel that fits the moment.

Start with the [Conceptual Model](./conceptual-model) for the entity vocabulary, the four feedback channels, and the codebase mapping. This guide covers everything else you need to develop, test, and ship changes.
...
│   ├── intelligence-service/  # TypeScript + Hono intelligence service (AI mentor, guidance)
│   └── webhook-ingest/        # NATS-based GitHub/GitLab webhook adapter
