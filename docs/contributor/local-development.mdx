...
1. [Create a new GitHub OAuth application](https://github.com/settings/developers) with the callback URL `http://localhost:8081/realms/hephaestus/broker/github/endpoint`. Request the `user:email` and `read:org` scopes so Keycloak can populate profile data and (optionally) map GitHub teams to roles later.
