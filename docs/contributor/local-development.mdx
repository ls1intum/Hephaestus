...
4. **Bun** – Runs the agent runtime and precompute tests (`pnpm run test:agents`). `corepack` does not
   provide it; install it separately. CI runs the version the agent image pins in
   `docker/agents/pi/Dockerfile`. Bun runs the code but does not lint or format it — Bun ships no
   `bun fmt` or `bun lint` in the pinned version, so Biome covers that tree (see below).
...
formats it to that tree's rules on save. Biome also lints the first tree, but not the SPA — there
oxlint does, from `webapp/.oxlintrc.json`, which is why the oxlint extension is a recommendation too.

Run the same checks from the shell with:

```bash
pnpm run check:agents          # Biome lint + format + import order, then typecheck, outside the webapp
...
pnpm run check                 # Everything, the way CI runs it
```
...
have their own `application-<name>.yml` under `server/src/main/resources/` — `prod` and `specs`, plus
