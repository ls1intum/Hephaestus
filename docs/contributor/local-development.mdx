...
4. Run the application server with `pnpm run dev:server`; it brings up the Compose services, installs
...
- **Reset everything:** `pnpm run dev:reset` (see Running the stack); on next login the account and identity link are recreated.
...
```bash
pnpm run generate:api   # regenerates server/openapi.yaml from the packaged JAR, then the client
```

`pnpm run generate:api:application-server:client` regenerates the client alone from the committed
