...
property, so use `-Dapp.profiles=…` to override it; `pnpm dev:server:e2e` uses that interface.
...
4. Run the application server with `pnpm dev:server`; it builds the generated-client module before
