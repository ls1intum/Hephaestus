...
3. **Bun** – Match `package.json#packageManager`; it runs JavaScript and TypeScript tooling and
   installs dependencies. See [ADR 0033](https://github.com/ls1intum/Hephaestus/blob/main/docs/decisions/0033-bun-is-the-javascript-runtime-and-package-manager.md).
...
```bash
bun run check:agents          # Format check, lint, and both typechecks outside the webapp
bun run check:agents:fix      # Apply every safe fix
bun run check:webapp          # The SPA's format and lint checks
bun run typecheck:webapp      # The SPA's typecheck — a separate leg, not part of check:webapp
...
property, so use `-Dapp.profiles=…` to override it; `bun run dev:server:e2e` uses that interface.
...
2. From the repo root, start everything with one command:
   ...
   ```bash
   bun run dev:server   # terminal 1 — brings up Postgres, then Spring Boot
   bun run dev:webapp   # terminal 2 — Vite dev server
   ```
...
3. Access the API at `http://localhost:8080` and the webapp at `http://localhost:4200`.
   ...
   `bun run generate:api:application-server:specs` boots under the `specs` profile, which turns them on
   itself.

To wipe the local database (e.g. after a bad migration or sync), run `bun run dev:reset`. The Postgres data is a bind-mounted folder (`server/postgres-data`), so `docker compose down -v` alone does not clear it — `dev:reset` removes the folder and recreates the stack.
...
Run `bun run check:ports` before starting the stack. It checks the three ports above; a NATS
...
```bash
WEBAPP_PORT=5200 bun run --filter webapp dev
```
...
4. Run the application server with `bun run dev:server`; it builds the generated-client module before
   invoking the application module. Postgres + NATS start automatically via Spring
   Boot's Docker Compose support. Open the webapp and click **Sign in with GitHub**.
...
- **Reset everything:** run `bun run dev:reset` (Postgres is a bind-mounted folder, so `docker compose down -v` does **not** clear it — `dev:reset` removes `server/postgres-data` and recreates the stack); on next login the account + identity link are recreated.
...
so `bun run dev:server` exposes the unified `/webhooks/{kind}` endpoint (kinds: `github`,
...
```bash
bun run --filter webapp storybook:dev           # Local playground on http://localhost:6006
bun run --filter webapp build-storybook     # Static bundle for Chromatic
bun run --filter webapp chromatic:ci        # Visual regression in CI
```
...
```bash
bun run --filter webapp generate:api
```
