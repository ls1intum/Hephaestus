...
If another service on your machine already uses port 5432 (PostgreSQL) or 8081 (Keycloak), override the host-side port via environment variables in your `.env` file:
...
Spring Boot reads the same variables, so the application server connects to the right port automatically. Update the GitHub OAuth callback URL when changing `KEYCLOAK_PORT`:
