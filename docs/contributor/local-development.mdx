...
3. **Node.js** – Match `package.json#devEngines.runtime`; it executes repository scripts and
   application tooling.
4. **pnpm** – Match `package.json#packageManager`. Use pnpm's
   [pnpm 12 standalone installer](https://pnpm.io/installation#pnpm-12-using-a-standalone-script)
   for that exact version; this repository does not use Corepack.

Read both versions from `package.json`, then verify them with `node --version && pnpm --version`.
...
```bash
pnpm run check:agents          # Format check, lint, and both typechecks outside the webapp
pnpm run check:agents:fix      # Apply every safe fix
pnpm run check:webapp          # The SPA's format and lint checks
...
property, so use `-Dapp.profiles=…` to override it; `pnpm run dev:server:e2e` uses that interface.
...
2. From the repo root, start everything with one command:
   ...
   ```bash
   pnpm run dev:server   # terminal 1 — brings up Postgres, then Spring Boot
   pnpm run dev:webapp   # terminal 2 — Vite dev server
   ```
...
3. Access the API at `http://localhost:8080` and the webapp at `http://localhost:4200`.
   ...
   `pnpm run generate:api:application-server:specs` boots under the `specs` profile, which turns them on
   itself.

To wipe the local database (e.g. after a bad migration or sync), run `pnpm run dev:reset`. The Postgres data is a bind-mounted folder (`server/postgres-data`), so `docker compose down -v` alone does not clear it — `dev:reset` removes the folder and recreates the stack.
...
Run `pnpm run check:ports` before starting the stack. It checks the three ports above; a NATS
...
For an automated local end-to-end setup, use `pnpm run e2e:setup`. It creates the connection,
...
4. Run the application server with `pnpm run dev:server`; it builds the generated-client module before
...
- **Reset everything:** run `pnpm run dev:reset` (Postgres is a bind-mounted folder, so `docker compose down -v` does **not** clear it — `dev:reset` removes `server/postgres-data` and recreates the stack); on next login the account + identity link are recreated.
...
```bash
pnpm --filter webapp run storybook:dev           # Local playground on http://localhost:6006
...
```bash
pnpm --filter webapp run generate:api
```
