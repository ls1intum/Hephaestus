...
Webhook reception runs as part of the Java server (`gitprovider.webhook` package) — there is no
separate Node service. In local development the monolith default boots all three runtime roles
(`server`, `worker`, `webhook`), so `pnpm run dev:server` exposes the `/gitlab` and `/github`
endpoints alongside the rest of the API.

In production the receiver is deployed as a separate `webhook-server` container from the same
image, activated with `SPRING_PROFILES_ACTIVE=prod,webhook`. The app-server's deploy cycle does NOT
interrupt webhook reception — push events on GitHub/GitLab are not manually redeliverable, so
restart-independence is operationally required. See ADR 0008.
