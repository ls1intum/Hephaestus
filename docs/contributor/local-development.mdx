...
4. **Bun** – Runs the agent runtime and precompute tests (`pnpm run test:agents`). `corepack` does not
   ...
   line in `docker/agents/pi/Dockerfile`, which is also what CI installs. Why Bun and not Node:
...
The Biome extension picks up both configurations automatically: `biome.jsonc` at the repository root
covers the Bun agent runtime, its specs, both precompute trees and `scripts/**`, and
`webapp/biome.jsonc` (declared `"root": false`) covers the SPA. Editing a file in either tree
formats it to that tree's rules on save. Both configs set `"linter": { "enabled": false }` — Biome
formats and sorts imports and lints nothing anywhere, which is why the oxlint extension is a
recommendation too.
...
Create `server/application/src/main/resources/application-local.yml` to override defaults. This file is gitignored.
...
4. Run the application server with `pnpm dev:server`; it installs or restores the generated-client
   dependency before invoking the application module. Postgres + NATS start automatically via Spring
