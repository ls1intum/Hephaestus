...
Practice detection and the mentor need at least one enabled model. You can add one through the admin UI (**AI → Agents**), or seed one automatically at startup so a fresh boot is ready without clicking through the UI:
...
The seeder runs once the default workspace exists, is idempotent (skips if a model of the same name is already present), and stores the key encrypted. Models always use the in-app LLM proxy: the key is injected by the proxy and never reaches the sandbox, so also set the upstream the proxy forwards to (`LLM_PROXY_OPENAI_URL` — any OpenAI-compatible gateway works).
