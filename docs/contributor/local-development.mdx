...
`webapp/biome.json` (declared `"root": false`) covers the SPA. Editing a file in either tree
formats it to that tree's rules on save.

Run the same checks from the shell with:

```bash
pnpm run check:agents          # Lint + format + import order + typecheck, outside the webapp
pnpm run check:agents:fix      # Apply every safe fix
pnpm run check:webapp          # The SPA equivalent
