...
- Run the formatter and linter for the area you touched. Examples: `bun run format:webapp`, `bun run lint:server`.
- Smoke-test the critical scripts. `bun run build:webapp` must pass. If you changed migrations, run `bun run db:generate-erd-docs`.
...
- Rerun the quality gates that match your changes: `bun run check`, `bun run generate:api`, or `cd server && ./mvnw test`.
...
- Co-locate `*.stories.tsx` files. Show the states the component can actually reach, with realistic data. Run `bun run --filter webapp chromatic:ci` whenever stories change.
- Defer analytics work to effects that run after paint.

### Quality gates

- Install dependencies with `bun install` at the repo root so optional binaries resolve.
- Run `bun run check:webapp` and `bun run build:webapp` before opening a pull request.
- Execute Vitest suites with `bun run test:webapp`. Add unit tests beside the feature you touched.
- Keep Storybook stories in sync with design. Launch `bun run --filter webapp storybook:dev` when pairing with design reviews.
...
- Run `bun run check:java-nullness` for the policy check and `cd server && ./mvnw test-compile -DskipTests` for compiler analysis.
...
- After updating Liquibase migrations, run `bun run db:generate-erd-docs`. Regenerate OpenAPI and
...
- Docusaurus 3 powers the documentation. Install dependencies with `bun install` and run
  `bun run docs:dev` from the repository root to preview.
...
- Run `bun run docs:build` before committing structural changes to confirm the static export succeeds.
