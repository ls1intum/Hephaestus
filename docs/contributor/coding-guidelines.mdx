...
- Run the modular test suites with `cd server && ./mvnw verify`. The reactor builds the generated-client dependency before the application, and ArchUnit checks enforce package boundaries.
...
- Configuration is shared with auto-registration: `core.webhook.WebhookProperties` bound to
  ...
  `server/pom.xml`; read them there rather than from a copy.
