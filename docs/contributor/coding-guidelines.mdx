...
## Webhook receiver (`server/.../gitprovider/webhook/`)

- Pure verifier/builder classes (`HmacVerifier`, `GitLabTokenVerifier`, `GitLabSubjectBuilder`,
  `GitHubSubjectBuilder`, `DedupIdResolver`) — Spring-free; exercised by 1:1 ported Vitest cases
  and parameterised tests over the captured fixtures in `server/src/test/resources/{github,gitlab}/`.
...
- Configuration is shared with auto-registration: `core.webhook.WebhookProperties` bound to
  `hephaestus.webhook.*`. JaCoCo package coverage gate ≥ 0.95 branch on `gitprovider.webhook`.
