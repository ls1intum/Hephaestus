...
- Rerun the quality gates that match your changes: `pnpm run check`, `pnpm run generate:api`, or `./mvnw -pl server test`.
...
- Co-locate `*.stories.tsx` files. Show default, loading, empty, and error states with realistic data. Run `pnpm --filter webapp run chromatic:ci` whenever stories change.
...
- Feature packages live under `integration/scm/`, `achievement/`, `activity/`, `practices/`, `mentor/`, `notification/`, `syncing/`, and `workspace/`. Shared wiring sits in `config/` and `core/`. Cross-cutting integration traits (webhook ingress, oauth ingress, SPI, registry) live under `integration/` directly.
- Controllers return DTOs located near the controller. `AchievementDTO`, beside `AchievementController`, is the reference example.
- Repositories return `Optional<T>` instead of `null`.
- Services encapsulate a single responsibility, as in `AchievementService`. If a service depends on many collaborators, split the responsibility.
...
- Pure verifier/builder classes (`HmacVerifier`, `GitLabTokenVerifier`, `GitLabSubjectBuilder`,
  `GitHubSubjectBuilder`, `DedupIdResolver`) — Spring-free; exercised by JUnit unit tests plus
  parameterised tests over the captured fixtures in `server/src/test/resources/{github,gitlab}/`.
...
- Configuration is shared with auto-registration: `core.webhook.WebhookProperties` bound to
  `hephaestus.webhook.*`. JaCoCo package coverage gate ≥ 0.95 branch on `integration.core.webhook`.
...
- Liquibase changesets live in `server/src/main/resources/config/liquibase`.
