...
description: Shared principles for the React client, Spring Boot server, and TypeScript intelligence service.
---
...
- Uses Hono for HTTP routing and Vercel AI SDK for model orchestration.
...
- Drizzle ORM for database access; regenerate the schema with `npm run db:generate-models:intelligence-service` (writes to `src/shared/db/schema.ts`).
- Export the OpenAPI spec with `npm run openapi:export` after API changes.
- Format and lint with Biome (`npm run format:intelligence-service`, `npm run check:intelligence-service`).
- Add or update service tests with Vitest (`npm -w server/intelligence-service run test`).

## Webhook ingest service (`/server/webhook-ingest`)

- Uses Hono for HTTP routing and `@nats-io/jetstream` for NATS publishing.
- Verify GitHub signatures via HMAC-SHA256 and GitLab tokens via constant-time comparison.
- Configure settings via Zod in `src/env.ts` and keep `.env.example` up to date.
- Format and lint with Biome (`npm run check:webhook-ingest`) and run tests with Vitest.
...
- After schema updates, regenerate the ERD, refresh the Drizzle schema, and run server tests that touch the affected tables.
