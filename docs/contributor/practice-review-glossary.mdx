...
| **Practice review** | One evidence-bounded operation within AI-supported mentoring: Hephaestus checks one or more practices against a specific piece of work and may produce observations. | practice detection, scan, mentoring relationship |
...
| **Automated-review policy** | The frame a review runs in for one practice: the source-contract version, the review mode, the refusal behaviour, and the known limits. It no longer lists sources. | evidence requirements |
...
`PracticeReviewTier` is the workspace's answer to "how loud may this practice be", and it replaced the
boolean that used to answer only "is this practice used at all". Each tier adds to the one before it.
...
practice down no longer puts a hole in the behaviour series, because the measurement and the intervention
are separate decisions. `ENGAGE` is the default and the tier a workspace that never touches the setting
stays at.
...
reflection surface. `PROFILE` is declared and unwritten; when a producer exists it joins `COACH`
alongside `CONVERSATION` without the tier's meaning changing.
...
binding and only ever narrows: a workspace that never sets it behaves exactly as before.
...
Keep availability, content, completeness, and collection outcome separate. In particular:
...
There is no staleness state. A capture either names something immutable — a commit SHA — or it does not,
and nothing in the pipeline can tell "collected an hour ago and unchanged" from "collected an hour ago and
since superseded". `IdentityPolicy` records only the honest half: whether a source can be anchored at all.
