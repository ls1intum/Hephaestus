...
a membership row plus `User.Type.USER`; nothing consults accounts, sign-ins or identity links, so do not
write that a person must have signed in. Membership itself arrives from the organization or group roster
sync, the team graph, an administrator, or workspace activity. `SELECTED` is validated against the same
population, so a person who is not an eligible member cannot be selected.
...
- **At `COMPOSITION` the contributing practices are not yet known**, so `PRACTICE_AUTHORITY` is
  ...
  repository, no author and no unsolicited destination to judge. That is not a bypass: Silent Mode,
  workspace-enabled, the rollout fence and the workspace delivery pause are all decisive at composition,
  so a paused or superseded workspace composes nothing.

Which lanes get a second evaluation is not symmetric, and it is worth knowing before reading a trace. The
artifact lane is evaluated again at `AUTOMATIC`, at `APPROVED` when a person released it, and at `EGRESS`
...
6. A workspace administrator sets repository and member coverage, delivery status, and the workspace default autonomy, then overrides autonomy per area or practice only where the default is wrong.
