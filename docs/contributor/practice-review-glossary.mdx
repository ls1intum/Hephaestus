...
Both axes must admit an artifact. An empty base-branch list on a selected repository means every branch in
that repository. Branch restrictions apply to pull and merge requests; issues have no base branch and are
matched by repository and person only. Names are exact—there are no glob or path rules.
