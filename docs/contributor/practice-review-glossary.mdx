...
| **Observation** | One measurement — one practice's answer about one piece of work, immutable, kept forever. Carries an `evidenceRationale` and never advice. | finding, verdict, result |
...
| **Composition** | The named LLM turn that performs the intervention, in its own sandbox session after the measurements are final. | the composer's turn as part of the review, second pass |
| **Composed feedback unit** | One message the composition turn proposes for one channel and one practice, before any server gate has admitted it. | feedback row, draft feedback |
...
| **Notes** | What is composed for the mentor conversation: the situation, coaching goal, evidence summary, and an observable success signal. Notes *to* the mentor, never a sentence for it to say — it writes the turn's words from the live conversation and original evidence. | move, opener, conversation message, prepared reply, script |
| **Anchor** | The observation and citation an in-context note is placed on. The composer names them; the server resolves the file, side and line. | line reference, position, diff note |
...
| --- | --- | --- | --- | --- |
| `IN_CONTEXT` | on the work artifact — pull request summary or inline note, issue comment | task | "what is wrong here?" | public — the team reads it on the work |
| `IN_APP` | on the developer's own practice pages | process | "what keeps happening in how I work?" | private — the developer alone |
| `IN_CHAT` | in a turn of a conversation, wherever it runs — the in-app mentor at `/w/:slug/mentor`, or Slack | self-regulation | "how would I have caught this myself?" | private — a live mentor turn |
...
Supersession only ever claims a `PREPARED` message. Say **superseded** for a queued message that was
replaced before anybody read it, and **continued** for a newer message written beside one that was
