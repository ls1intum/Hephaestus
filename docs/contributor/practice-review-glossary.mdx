...
| **Practice autonomy**               | Who may authorize feedback for one practice: nobody (`OFF`), an authorized person (`HUMAN_APPROVAL`), or the system (`AUTOMATIC`). Nullable at practice and area levels, where null means “inherit”.                                       | review tier, loudness tier, shadow mode, enabled                  |
...
ground truth. A statement that a person _could_ observe something must never let Hephaestus attempt an
automated review.
...
| The _Review this now_ button                    | `MANUAL`                      | Requested by hand          | `MANUAL`           | yes                                  |
...
_profile_ for this channel: **Profile** is the public page at `/user/{username}`, and the two are
...
| ---------------- | ----------- | -------------------- | -------------------------------------- |
| `OFF`            | no          | no                   | nobody                                 |
| `HUMAN_APPROVAL` | yes         | yes                  | an authorized workspace reviewer       |
...
`AWAITING_APPROVAL` with its immutable content and target. An authorized reviewer can approve that exact
proposal or reject it with a structured reason. Approval moves the unit to `PREPARED` and triggers the normal
delivery path; it does not bypass recipient choice, workspace pause, Silent Mode, artifact validity,
idempotency, or any other release-time control. Rejection moves the unit to the terminal `DISCARDED` state.
...
`GET /workspaces/{workspaceSlug}/practices/autonomies`. Workspace review settings expose
...
and a `PATCH` replaces it wholesale rather than merging, because the lists _are_ the setting and a merge
could only ever add.
...
refuses with `PRACTICE_AUTONOMY_OFF`, which is retryable (`PENDING`): an admin raising the tier lifts it
...
| **Suppress**               | The _server's_ refusal to deliver a message it was given, recorded as `FeedbackSuppressionReason`.                                                                                                                                                   | withhold, silence                                          |
| **Notes**                  | What is composed for the mentor conversation: `situation`, `capability`, `evidenceSummary`, and `inConversationSignal`. Notes _to_ the mentor, never a sentence for it to say — it writes the turn from the live conversation and admitted evidence. | move, opener, conversation message, prepared reply, script |
...
Timperley's model, and every name answers one question — _where does this land?_ — because a set of names
...
developer can answer, and `IN_APP` is written down, where reading it is the whole interaction. Ask _is it
a turn?_ before _which screen?_ — a new written surface inside Hephaestus is `IN_APP`, and a mentor in
...
wrong. Do not write _profile_ for `IN_APP` — **Profile** is the public page at `/user/{username}` — and
do not write _reflection_ either: it named what the developer was meant to do rather than where the
message lands, and it is the name this lane was renamed away from
([ADR 0029](https://github.com/ls1intum/Hephaestus/blob/main/docs/decisions/0029-measurement-intervention-seam-and-channel-levels.md)).
Do not write _conversation_ for `IN_CHAT`: `chat.conversation_thread` is a kind of work Hephaestus
_reviews_, not a place it delivers to. And do not describe `IN_CHAT` as a prepared reply; what is stored
...
(`FeedbackThreadKey`) rather than by prose. The longitudinal channels key on the _practice_, because a
...
keeps happening across your work" is a claim about a _practice_ over the window and not something the
...
that a capture is _stale_ — there is no such state, and the contract says why.
...
8. Never write bare **binding**. Two unrelated things carry the word — a _practice binding_ (an occasion
   a practice is reviewed on, `PracticeBinding`) and a _model binding_ (a workspace's AI model for a
