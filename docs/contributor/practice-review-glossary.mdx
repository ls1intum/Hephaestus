...
| **Review coverage**                 | Which repositories and linked members' work may be reviewed. Repository and person modes are independent; effective coverage is their intersection.                                                                                        | filter, audience, pilot allowlist, review scope                   |
...
`WorkspaceReviewScope` defines the workspace population that automatic review may admit. It has two
independent modes:
...
| Person     | `ALL_ELIGIBLE` linked members | `SELECTED` linked members                                                | no people            |
...
requests; issues have no base branch and are matched by repository and person only. Names are
exact—there are no glob or path rules. Coverage lives on the practice-review settings resource as
`reviewScope` (`GET`/`PATCH /workspaces/{workspaceSlug}/practices/review-settings`), and a `PATCH`
replaces it wholesale rather than merging, because the lists *are* the setting and a merge could only
ever add.

The review subject for pull requests, merge requests, and issues is their author. A missing, ambiguous,
unlinked, non-human, or non-member author is not expanded to all eligible people. This makes coverage an
...
`GET /workspaces/{workspaceSlug}/practices/review-settings`. A `PATCH` replaces the complete coverage value;
`POST /workspaces/{workspaceSlug}/practices/review-settings/coverage-preview` returns authoritative current
and proposed repository/member counts before a widening change. Updates require the current resource ETag
in `If-Match`.

`deliveryStatus` is a separate workspace brake. `PAUSED` stops external practice feedback without stopping
review or deleting coverage. Resuming is prospective: feedback admitted under an earlier rollout revision
is not released. Instance Silent Mode remains the stronger instance-wide egress boundary.

Every composition and egress decision appends a versioned policy evaluation. It records the admitted and
current rollout revisions, destination, stage, first decisive reason, ordered check results, and a bounded
snapshot of the repository, branch, subject resolution, coverage modes, and contributing-practice
autonomies that were evaluated. Check results are `PASSED`, `DENIED`, `NOT_APPLICABLE`, or `NOT_REACHED`;
`NOT_REACHED` preserves precedence by showing which checks were skipped after the first denial. The trace
must not persist feedback bodies or member profile data.

Automatic admission records the current workspace `revision`. Workspace coverage, delivery, and default-autonomy
changes advance it. Area and practice autonomy changes remain prospective through the proposal lifecycle and
the current-autonomy egress check rather than a second revision counter. Composition and egress compare the
admitted and current workspace revisions and re-evaluate identity, coverage, autonomy, recipient preference,
artifact validity, and egress controls. An artifact refused before review records a typed terminal reason; changing
coverage affects later work, not the refused occurrence.
...
6. A workspace administrator sets repository and linked-member coverage, delivery status, and the workspace default autonomy, then overrides autonomy per area or practice only where the default is wrong.
