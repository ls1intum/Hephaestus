...
2. Run the appropriate command:
   ...
   Generated clients have their own module, and the reactor restores their compiled artifact from
   Maven Build Cache when its inputs are unchanged. After one reactor build, repeated application or
   test edits can use the application-only recipes below without touching generated clients.
...
Use `*Test.java` for unit tests and `*IntegrationTest.java` or `*LiquibaseTest.java` for integration
tests. Maven's Surefire and Failsafe plugins use these patterns.
:::
...
./mvnw test -Parchitecture-tests     # ArchUnit + Modulith verification
./mvnw verify                       # Unit + integration tests and packaging
./mvnw -f application/pom.xml test -Dtest=SomeTest         # Single test class
./mvnw -f application/pom.xml test -Dtest=SomeTest#method  # Single test method
```

Run a reactor command first after a fresh checkout or after changing a schema, generator, generated-client
dependency, or `generated-clients/pom.xml`. The application-only commands then preserve Maven's
incremental compilation; they fail immediately if the generated-client artifact has not been installed.
