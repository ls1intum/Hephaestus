...
2. Run the appropriate command:
   ...
   :::danger Always deactivate `quick`
   The `quick` profile activates itself as soon as GraphQL codegen has produced its sources — that
   is, on every machine that has built the server once. It sets `maven.test.skip=true`, so a bare
   `./mvnw test` prints **BUILD SUCCESS having run zero tests**, and `./mvnw test-compile` reports
   success while test sources fail to compile. Pass `-P'!quick'` on every test run — including
   alongside another profile (`-P'!quick,live-tests'`), because no other profile deactivates it.
   :::
...
- Reads recorded webhook payloads from the fixture folder and deserialises them into the handler's event DTO.
...
class GitHubLabelMessageHandlerIntegrationTest extends BaseIntegrationTest {
    ...
    void shouldPersistLabel() throws IOException {
        var payload = objectMapper.readValue(
            new ClassPathResource("github/label.created.json").getContentAsString(UTF_8),
...
Two rules, and everything below follows from them: test a controller at the HTTP boundary, because
that is where a security misconfiguration shows; and keep fixtures reusable, so a new endpoint needs
no bespoke bootstrapping.
...
- **Query keyed on anything but `workspace_id`** (an org login, a login string, an external native id) is a red flag — scope by `workspace_id`, or add the provider/workspace predicate, before shipping. The `#1372` leaks (leaderboard roster by org-login, team lists by org-string) were exactly this shape.
...
```bash
./mvnw test -P'!quick'                        # Unit tests
./mvnw test -P'!quick,architecture-tests'     # ArchUnit + Modulith verification
./mvnw verify -P'!quick'                      # Full suite + packaging
./mvnw test -P'!quick' -Dtest=SomeTest        # Single test class
./mvnw test -P'!quick' -Dtest=SomeTest#method # Single test method
```

The tag filter is a property, not the `-Dgroups` user property: surefire is configured with
`<groups>${surefire.includedGroups}</groups>`, which an explicitly-configured plugin parameter wins
over. `-Dgroups=…` is silently ignored; use `-Dsurefire.includedGroups=…` when you need to change the
tag set directly, or the profiles above, which set it for you.

## Live external-service tests

Some regressions can only be validated against real systems — the GitHub API, a real LLM endpoint, a Docker daemon. Live tests are tagged `@Tag("live")` and **excluded from every other tier**; they only run under `./mvnw test -P'!quick,live-tests'`. The profile flips Surefire's group filter and adds `failIfNoSpecifiedTests=false` so single-test invocations don't fail when the gating env var is unset. It does not deactivate `quick`, so `!quick` has to ride along or the run executes nothing.
...
```bash
./mvnw test -P'!quick,live-tests'
```

The `live-tests` profile tells Maven to run only tests tagged with `@Tag("live")`. This is the **single guard** for live tests – if you don't pass the profile, the tests simply won't run.

**Live tests never run during normal CI.** They are excluded from the other tiers by tag filtering in the Surefire and Failsafe plugins.
