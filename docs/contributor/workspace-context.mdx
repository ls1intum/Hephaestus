...
Global admins (users with the Keycloak `admin` realm role, configured via `KEYCLOAK_GITHUB_ADMIN_USERNAME`) are automatically elevated to workspace ADMIN level across all workspaces. They can perform all ADMIN and MEMBER operations but cannot perform OWNER-only operations (ownership transfer remains explicit).
...
if (!ctx.hasMembership()) {
    ...
    // Note: global admins always have implicit membership
}
```
...
@RequireAtLeastWorkspaceAdmin  // Also allows global admins
