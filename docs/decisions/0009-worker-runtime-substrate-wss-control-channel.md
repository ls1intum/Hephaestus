...
**Status:** Accepted (amended 2026-07-21 #1368 and 2026-09-03 #1719 — see the updates below)
**Date:** 2026-05-21
...
worker's Sandbox Gateway port, not WSS. `WorkerControlFrame` permits `WorkerHello`, `WorkerWelcome`,
`Heartbeat`, `CapacityReport`, `ForceReconnect` and `CancelJob`; the mentor session quartet, the
`HubSessionInbox` bridge and the `WorkerControlPublisher` seam named above do not exist.
