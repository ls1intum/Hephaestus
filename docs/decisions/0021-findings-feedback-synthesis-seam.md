...
> `report_feedback` tool. The finding/feedback *separation* — the thing this ADR is actually for —
...
this ADR substantially. The register above is left intact because the rejected shapes are the part
worth keeping — F-1's rejection of a whole-comment row, F-14's deletion of `FeedbackPost`, F-17/F-25's
refusal of 0-writer columns and speculative tables are all still binding, and re-proposing any of them
should cost an argument, not a shrug. What follows is what survived contact with the code.

### Decisions that shipped

- A **finding** is audience-neutral evidence about one practice and one artifact (the principle
  above, and F-9's reaction-blind firewall).
- **Feedback** is a separate, durable ledger of messages derived from findings — not a rendered blob
  (F-1).
- A message may bind several findings and a finding may contribute to several messages (F-1's M:N).
- Delivery state and suppression reason belong to the message (F-20).
- Provider placements are separate rows, because one message can produce several posted notes
  (F-14's rejection of the `deliveryCommentId` scalar).
- Subject and recipient are distinct identities even when they refer to the same person (F-12).

### Decisions that did not ship

- **Agent-authored feedback through a `report_feedback` tool** (F-2, F-10). `DeliveryComposer`
  creates feedback server-side from findings persisted by `report_finding`. The producer/renderer
  split described in the Principle section therefore has one producer, not two.
- **Policy-floor and fallback feedback authors** (F-4, F-8, and the `origin {AGENT,POLICY_FLOOR,
  FALLBACK}` column in F-11). With no LLM author there is no floor to enforce against one. The
  underlying rule — a blocking finding is never silently dropped — survives inside the composer.
- **Reviewer and facilitator delivery axes** (F-12's `audience_role`, the `FACILITATOR` surface value
  already flagged in the note at the top).
- **A separate reaction-event log** (F-24's open `verb` discriminator). F-17 and F-24's shared
  conclusion — that passive engagement must never become a second table — still stands; it simply has
  no writer yet.
- **Speculative placement fields for content that was never posted** (parts of F-21, F-22, F-23).
...
Delivery and suppression can be inspected without reconstructing provider comments. A new delivery
channel reuses the ledger rather than changing finding identity. The "no future migration" proof above
is correspondingly weaker than claimed: it was proved against a schema with columns that were then
not built, so a future surface needing agent-authored or reviewer-scoped feedback will pay a
migration.

### Where the current contracts live

This ADR is historical rationale, not a field or enum inventory. For what the code holds today:

- [Practice review data model](../contributor/practice-feedback-schema.md);
- [Evaluation provenance](../contributor/evaluation-provenance.md) — owner of the
