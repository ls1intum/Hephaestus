...
See the dedicated [Slack Integration guide](./slack-integration) for full setup instructions. Summary of required variables:

| Variable | Purpose |
| --- | --- |
| `KEYCLOAK_SLACK_ENABLED` | Enable Slack identity provider in Keycloak |
| `KEYCLOAK_SLACK_CLIENT_ID` | Slack App Client ID |
| `KEYCLOAK_SLACK_CLIENT_SECRET` | Slack App Client Secret |
| `SLACK_BOT_TOKEN` | Bot token for posting messages |
| `SLACK_SIGNING_SECRET` | Webhook signature verification |
| `SLACK_LINKING_ENABLED` | Enable user account linking feature |
