...
| --- | --- | --- |
| `HEPHAESTUS_LLM_DISPLAY_CURRENCY` | *(empty)* | ISO 4217 code to show spend in **alongside** USD. Empty turns the second currency off entirely. Only `EUR` is honoured today; any other well-formed code logs a warning and falls back to USD-only. A value that is not shaped like a currency code fails startup, so a typo cannot hide as a missing feature. |
