...
| `GH_OAUTH_CLIENT_ID` | GitHub OAuth app client ID (callback `https://<host>/api/login/oauth2/code/github`). In `docker/.env` the key is `GH_OAUTH_CLIENT_ID` — GitHub Actions reserves the `GITHUB_` prefix, so Compose maps it to the server's `GITHUB_OAUTH_CLIENT_ID`. Setting `GITHUB_OAUTH_CLIENT_ID` in `docker/.env` has no effect and the provider is silently skipped. |
| `GH_OAUTH_CLIENT_SECRET` | GitHub OAuth app client secret (same `GH_`-prefix rule). |
...
| Variable | Purpose |
| --- | --- |
| `PRACTICE_REVIEW_FOR_ALL` | Enables the feature flag and the detection gate for all users. **Compose-file alias only:** it fans out to the two variables the server actually reads, `PRACTICE_REVIEW_RUN_FOR_ALL` and `HEPHAESTUS_FEATURES_FLAGS_PRACTICE_REVIEW_FOR_ALL`. Outside these compose files, set those two directly. |
...
4. A workspace admin binds an available model to each purpose — **practice detection** and **mentor**.
   If the instance admin permits bring-your-own connections, the workspace admin may instead create
   and price a workspace-owned connection and model through the same staged flow.

Connection routing identity (base URL, wire API, and authentication mode) and a model's upstream ID
are immutable. To change either, create a replacement, test it, move the bindings over, then disable
...
3. **Is a budget exhausted?** Administration → **AI usage** names which purse paused the workspace
   ...
   queued is cancelled rather than held forever. Note that a held job carries a future
   `available_at`, and `agent.queue.depth` counts only jobs eligible to run *now* — so a
   budget-paused workspace shows a queue depth of zero, not a backlog. Depth alone cannot distinguish
   "idle" from "everything is held".
...
and failed". Note it does not surface *why* a job is held: a budget-held job renders as an ordinary
queued row, so confirm step 3 on the AI usage page rather than from this table.
...
2. **Configure authentication:**
   - Copy `docker/.env.example` to `docker/.env`, then populate `GH_OAUTH_CLIENT_ID`, `GH_OAUTH_CLIENT_SECRET`, `HEPHAESTUS_AUTH_ISSUER`, and `HEPHAESTUS_AUTH_STATE_COOKIE_KEY`. The GitHub OAuth app's callback must be `https://<host>/api/login/oauth2/code/github`. (`server/.env` is the local-development file for `mvn spring-boot:run`; no Compose service reads it.)
