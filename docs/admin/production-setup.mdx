...
The bot token scope set should stay minimal:
...
Subscribe to these bot events for the Agent messaging experience (`agent_view`):
...
`app_mentions:read`, `im:write`, or `users:read.email` unless code is added that actually uses
them. `channels:join` is intentionally present so Hephaestus can join selected public channels from the admin picker. `mpim:read` is present only because Slack's `member_joined_channel` event can require
it during app configuration; Hephaestus still ignores MPDM message content and does not request
`mpim:history`.
