...
6. **Verify integrations:**
   ...
   - Verify that the global admin user can access workspace admin endpoints across all workspaces (they are automatically elevated to workspace ADMIN level, but cannot perform OWNER-only operations like ownership transfer).
...
- Automatic workspace ADMIN privileges across all workspaces (they are auto-elevated to workspace ADMIN level)
- Ability to manage workspace settings, members, and repositories
...
This allows platform administrators to troubleshoot and manage workspaces without requiring explicit membership in each workspace.
