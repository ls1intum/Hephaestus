...
6. **Same page → Review settings.** Set **How much the system does on its own** and **Where feedback goes**
   for the whole workspace. These two are the only settings you need to start; everything below inherits
   from them.
7. **Administration → Practices → Practice setup.** Override step 6 only for the areas or practices where
   the workspace answer is wrong. You are not expected to touch most of them.
...
- **How much the system does on its own** — `Off`, `Observe`, `Deliver`. Only `Off` stops the review.
  `Observe` runs it and records every observation exactly as `Deliver` does, and tells nobody. Turning a
  noisy practice down therefore does not put a hole in the measurement, which is the entire reason this is
  a dial and not a checkbox. (`Propose`, which would hold feedback for you to approve, is not available
  yet — the screen shows it greyed out rather than pretending it works.)
...
The **Practice setup** page shows, above the list, how many practices currently sit at each setting — for
the workspace and per area. Read that line before changing anything; it is the fastest way to see whether
an override you made years ago is still doing what you think.

Upgrading from the earlier per-practice loudness setting: `Off` and `Measure` carried over unchanged
(`Measure` is now called `Observe`), `Engage` became the workspace default so nothing got louder or
quieter, and `Coach` — which meant "speak in the mentor conversation but never on the work" — became
`Observe`. `Coach` mixed *how much* with *where*, and *where* is now a workspace setting, so it could not
be carried over exactly. It was resolved downwards deliberately: a system that comments on somebody's work
should get louder because a person decided so, never because of an upgrade. If you had practices on
`Coach`, set the workspace to *In the mentor conversation only* and raise those practices to `Deliver` to
...
| Workspace default and where feedback goes | Administration → Practices → Review settings |
| Per-area and per-practice overrides | Administration → Practices → **Practice setup** |
