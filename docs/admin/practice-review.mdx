...
- **Send feedback**, under **Sending feedback**, controls whether feedback may leave Hephaestus. It stops
  ...
  one control here that leaves work already in flight alone, so it is safe to reach for. A proposal you
  approved in the moment before the pause is held rather than lost, and goes out when you resume — the
  usual checks run again then, so one whose pull request has since closed is refused instead of posted.
