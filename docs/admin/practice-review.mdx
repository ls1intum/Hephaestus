...
- **How much the system does on its own** — `Off`, `Propose`, `Deliver`. Only `Off` stops the review.
  ...
  `Deliver` when you want it heard.
...
inheriting at any time. Start with the two workspace answers, then override the handful of areas where they
are wrong; a workspace runs dozens of practices across a dozen areas, and a setting you have to apply per
row is a setting nobody applies.
