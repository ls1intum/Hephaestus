...
| `PRACTICE_REVIEW_MAX_REQUESTS_PER_REQUESTER_PER_HOUR` | `5` | The per-person hourly allowance in the table above. Keyed on the person, not the work, so it is the one limit that catches somebody asking for a review on twenty colleagues' merge requests. `0` removes it |
| `PRACTICE_REVIEW_PROGRESS_FOOTER` | `false` | Appends the run-to-run progress footer and the re-review reply. Renders nothing until a target has been reviewed twice |
| `PRACTICE_REVIEW_REACTION_SUPPRESSION` | `false` | Stops re-raising a point the author already disputed or marked not applicable |
