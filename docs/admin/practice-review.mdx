...
- **Send feedback**, under **Sending feedback**, controls whether feedback may leave Hephaestus. It stops
  ...
  does not blank the developer's own feedback pages here, which are not something leaving. Paused feedback
  is terminally withheld; resuming applies only to work reviewed afterwards and never flushes old work.
  Pausing is the one control here that leaves work already in flight alone, so it is safe to reach for.
