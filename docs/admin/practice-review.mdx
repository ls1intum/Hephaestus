...
- **Send feedback**, under **Sending feedback**, controls whether feedback may leave Hephaestus. It stops
  ...
  have not been decided stay in the approval queue; after resume, an administrator may still inspect and
  decide each one under the current policy.

These settings advance the rollout revision and invalidate admitted, unsent work: changing coverage,
...
neither does an area or practice override, which is why demoting one practice back to **Review before
...
screen is how many members are in scope. A covered person must appear on the **Members** screen. Manage a
missing person in the connected organization, group, or team roster and let membership sync pick them up;
signing in to Hephaestus alone does not create workspace membership.

An outside contributor who is not a workspace member is not covered, so their work is not reviewed and no
feedback is prepared about them. The same is true when the work has no identifiable author, or the author is a bot.
...
**Autonomy** is answered once for the **workspace**, and every area and every practice follows that
answer until you say otherwise. An area can override the workspace, and a practice can override its area:
