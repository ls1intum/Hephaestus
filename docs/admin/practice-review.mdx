...
5. Server-side policy admits or refuses each proposal. **Review activity** explains the result. Workspace
   admins can inspect the run under **Administration → Practices → Practice reviews**; a provider comment
   exists only when public delivery was both useful and permitted.
...
| Did a job run, fail, or produce observations and feedback? | **Administration → Practices → Practice reviews** |
| Which work may start reviews, and may feedback be delivered? | **Administration → Practices → Review** |
| What does one practice review and which evidence does it require? | **Administration → Practices → Practice setup** |
| Is the review model ready? | **Administration → AI models** |
| Did a budget stop new work? | **Administration → AI usage** |
