...
3. On the practice **Review** screen, start practice reviews and enable automatic reviews.
...
5. Server-side policy admits or refuses each proposal. **Review activity** explains the result. Workspace
   admins can inspect the run in the practice-review run list; a provider comment
...
3. Choose whether merged work remains eligible, who may be reviewed, and which repositories or target
   branches are in scope. Empty scope lists include all monitored work.
4. Set the workspace autonomy tier under **How much**, then add area or practice overrides only where the
