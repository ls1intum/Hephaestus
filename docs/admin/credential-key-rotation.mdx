...
The listing above finds every connection in this state;
...
(`PATCH /api/workspaces/{slug}/token`); the admin console has no form for it yet. A GitHub App
connection does not use its stored blob. Writing the replacement clears the marker. Confirm the
