...
Outside rotation, the same mark is set the moment a request needs a stored credential that the
configured keys cannot decrypt — after a key was changed, or a host was set up with a new key over
...
pages show the connection with a notice saying the stored token cannot be read. Two ways out:
...
case: the read is refused as a configuration fault and the row is not marked, because the fix is to
configure that key, never to touch the row.

While the mark is set the connection's health reads *failed*, since no sync can run on a credential
the server cannot read. The listing above finds every connection in this state;
