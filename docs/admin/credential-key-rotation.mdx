...
replacement clears the marker. A quarantined GitHub App row has nothing to replace, since nothing
runs on its blob: clear the blob itself, which also takes the row out of rotation for good:
