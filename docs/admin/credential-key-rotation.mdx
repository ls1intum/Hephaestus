...
- Restore the key the credential was written with, if it was changed by mistake; the next read
  succeeds and the mark clears when the credential is next written.
- Replace the credential: re-enter the token, or reconnect the integration. Writing a new credential
  clears the mark.

The listing above finds every connection in this state; `credentials_rotation_failed_at` holds the
time the credential was last found unreadable.
