...
- **Submission** (`AgentJobEventListener` / `IssueAgentJobEventListener` / `BotCommandProcessor`,
  ...
  where the flag is `true`, including a `prod,worker` pod — `application-worker.yml` defaults
  `AGENT_ENABLED` to `true`, so a worker pod does submit as well as execute.
