...
All three default-on means `pnpm run dev:server` starts a working monolith.
...
sandbox *is* the worker capability, so it follows `hephaestus.runtime.worker.enabled`.
