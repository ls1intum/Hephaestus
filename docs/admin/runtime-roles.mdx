...
Five rows are **not** role-differentiated — they are instance-wide defaults, shown per role only
because operators ask per role: `init-default` is `false` in the `prod` profile for every role, and
`git.enabled` (`GIT_CHECKOUT_ENABLED`), `agent.enabled` (`AGENT_ENABLED`),
`integration.outline.allowed-origins`, and `Needs user-auth wiring`
