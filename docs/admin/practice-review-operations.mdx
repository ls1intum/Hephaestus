...
Workspace administrators configure model readiness under **AI models**, review coverage, delivery status, and autonomy under
...
application configuration, and treat the fabric root as sensitive storage. Delivery-policy evaluations and
dispatch intents are deleted with their owning job or workspace; selected coverage targets are deleted with
their workspace membership or monitored repository.

**Dispatch recovery.** Automatic summaries and approved artifact comments first persist an immutable
destination and body, then claim it in a short transaction. Immediately before provider I/O, the dispatcher
persists `write_started`. Recovery always checks the deterministic provider marker first. If the marker is
found, it records the existing comment as sent. If lookup is inconclusive—or lookup reports no marker after
a write began—it does not post again; it retries reconciliation until the bounded attempt limit ends in the
terminal `FAILED` state. A dispatch with no prior write may post after a definitive not-found result. Provider
I/O runs without a database lock.

Do not mutate dispatch rows manually. The state, write fence, attempt count, next-attempt time, and lease
