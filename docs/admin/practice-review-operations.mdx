...
See the
[server configuration](https://github.com/ls1intum/Hephaestus/blob/main/server/src/main/resources/application.yml)
...
fabric root as sensitive storage. Nothing else needs a retention setting of its own: delivery-policy
evaluations and dispatch intents are deleted with their owning job or workspace, and selected coverage
targets with their workspace membership or monitored repository.

**Dispatch recovery.** A dispatch whose provider outcome is unknown is never re-posted on a guess. Before
writing, the dispatcher looks for a deterministic marker on the provider: if the comment is already there
it is recorded as sent, and only a definitive not-found lets a first write proceed. An interrupted write
therefore stalls in `UNCERTAIN` rather than duplicating, and reconciles or gives up on its own.

Two consequences to plan for:

- **Sustained `UNCERTAIN` means the provider is not answering the lookup**, not that feedback is queued.
  Fix provider connectivity or credentials; nothing in Hephaestus will clear it.
- **A dispatch interrupted mid-write can outlive its retry budget without reaching `FAILED`.** It is
  deliberately not failed, because the comment may be live, and it will log a warning until somebody
  confirms on the provider which it is. Treat a row warning on repeat as an operator task, not a bug.

Do not mutate dispatch rows manually. Their state, write fence, attempt count, next-attempt time and lease
owner jointly prevent an uncertain provider outcome from becoming a duplicate write.
...
- **No screen edits what the model is told.** What Hephaestus observes is shaped by each practice's
  ...
  latter is a code change, so a complaint about tone is an issue to file, not a setting to find.
