...
2. Re-confirm the deployment-dependent figures against production before submission. Each of these is already pinned in `03-vt-dsms.md` §13 — the phase-0 check is that the deployment has not drifted:
   - Container log retention is a deterministic per-container size cap enforced by the Docker `json-file` driver: `max-size=50m` × `max-file=5` for application services in `docker/compose.app.yaml`; `max-size=10m` × `max-file=3` for core infrastructure in `docker/compose.core.yaml`. Verify the per-service `logging:` blocks have not been removed from the compose files before submission (`grep -A3 "logging:" docker/compose.app.yaml docker/compose.core.yaml`).
...
- §4 lists: identity + authentication, development activity (GitHub + gitlab.lrz.de), account settings + recognition + the "AI review comments" Art. 21 objection switch, AI-assisted features, and server logs (deterministic per-container size cap via the Docker `json-file` driver, configured in `docker/compose.app.yaml` and `docker/compose.core.yaml`). It does **not** claim active Sentry or PostHog processing — both integrations are disabled in production.
...
- *"Löschkonzept fehlt"* — §13 plus the deterministic per-container size cap on server logs (Docker `json-file` driver `max-size` × `max-file`, configured in `docker/compose.app.yaml` and `docker/compose.core.yaml`), the account-deletion flow, and per-provider retention windows are the deletion concept. Off-host backups are not currently in place, so no separate backup-deletion flow is required; if backups are introduced later, the application-level deletion log must be re-applied on restore.
