...
2. Re-confirm the deployment-dependent figures against production before submission. Each of these is already pinned in `03-vt-dsms.md` §13 — the phase-0 check is that the deployment has not drifted:
   - Container logs are kept in a fixed-size buffer per service: **250 MB** (5 × 50 MB) per application service, **30 MB** (3 × 10 MB) per infrastructure service. When the buffer fills, the oldest log file is overwritten. Two checks before submission:
     1. Confirm the `logging:` blocks are still present in the compose files: `grep -A3 "logging:" docker/compose.app.yaml docker/compose.core.yaml`. If any service is missing one, fix it before submission.
     2. Measure the current rolling-window duration on production. SSH to the host, identify the busiest application-service container, and read the timestamp of the oldest rotated log file: `ls -lt /var/lib/docker/containers/<container-id>/<container-id>-json.log* | tail -1`. The age of that file is the lower bound of the rolling window for that service. **Record the resulting figure (e.g. "≈ 11 days for the application server under current production traffic") in the DSMS submission text** — do not commit it to the VVT, since it changes with traffic.
...
- §4 lists: identity + authentication, development activity (GitHub + gitlab.lrz.de), account settings + recognition + the "AI review comments" Art. 21 objection switch, AI-assisted features, and server logs (250 MB / 30 MB fixed buffer per service, oldest entries overwritten when full; effective duration is volume-dependent and reported with each submission). It does **not** claim active Sentry or PostHog processing — both integrations are disabled in production.
...
- *"Löschkonzept fehlt"* — §13 plus the 250 MB / 30 MB fixed-buffer cap on server logs (configured via the `logging:` blocks in `docker/compose.app.yaml` and `docker/compose.core.yaml`; oldest entries overwritten when the buffer fills; current effective rolling-window duration measured per Phase 0), the account-deletion flow, and per-provider retention windows together form the deletion concept. Off-host backups are not currently in place, so no separate backup-deletion flow is required; if backups are introduced later, the application-level deletion log must be re-applied on restore.
