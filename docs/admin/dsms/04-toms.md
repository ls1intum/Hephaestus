...
- **Server logs** are subject to a deterministic per-container size cap enforced by the Docker `json-file` log driver: `max-size=50m` × `max-file=5` (≤ 250 MB per service) for application services in `docker/compose.app.yaml`; `max-size=10m` × `max-file=3` (≤ 30 MB per service) for core infrastructure in `docker/compose.core.yaml`. Once a container exceeds its cap, the daemon discards the oldest log file. The cap ships with every deploy as part of the per-service `logging:` blocks in the compose files; there is no manual host-level configuration. Any change to those `logging:` blocks (raising the cap, or removing them) is a material change to the retention bound and must be reflected in the VVT and the privacy statement.
