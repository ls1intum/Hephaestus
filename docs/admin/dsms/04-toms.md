...
# Hephaestus — Technical and Organizational Measures (TOMs)

Documents the measures taken per Art. 32 GDPR. Structured along the categories of the BayLfD / DSK TOM-Leitfäden.
...
- Server logs record HTTP requests and allow traceability at the reverse-proxy level; retention is capped at 14 days.
...
- Regular PostgreSQL and Keycloak backups per AET operational baseline; fill in the specific retention value in `03-vt-dsms.md` §13 before submission.
- LLM-provider calls have a bounded timeout and retry policy; practice reviews degrade gracefully when an LLM provider is unreachable.
...
- Unit, architecture, integration, and end-to-end tests run on CI (GitHub Actions). The legal-page render path is covered by dedicated tests that exercise the three-layer cascade and the XSS guardrail on operator-supplied Markdown.
...
- Network traffic is TLS-encrypted end-to-end (browser → Traefik → internal services; Hephaestus → LLM provider over HTTPS).
...
- **Backups** follow AET's backup-retention policy; on restore from backup after an account deletion, the application-level deletion log is re-applied to prevent re-materialising deleted data.
...
- Incidents are recorded in GitHub Issues against the `ls1intum/Hephaestus` repository and, if they affect personal data, reported to the TUM DPO within 72 h.
...
| Component | Role | AVV / framework |
|---|---|---|
| GitHub, Inc. / Microsoft Corporation | Identity provider (OAuth) and source-system API | AVV at TUM/AET level |
| OpenAI, L.P. | LLM provider (per workspace) | AVV at TUM/AET level for TUM-operated tenancy; at workspace administrator's institution when that institution supplies credentials |
| Microsoft Corporation (Azure OpenAI Service) | LLM provider (per workspace) | AVV as above |
| Anthropic, PBC | LLM provider (per workspace) | AVV as above |
| Salesforce, Inc. / Slack Technologies, LLC | Workspace notifications (when enabled by the workspace admin) | AVV at TUM/AET level |
| TUM SMTP relay | Email delivery (Keycloak verification / password reset, optional notification emails) | In-house; TUM internal framework |
| Leibniz-Rechenzentrum der BAdW (gitlab.lrz.de) | Source system and OIDC identity provider | **Not Art. 28** — separate controller under § 16 Abs. 1 Satz 2 BayHIG + BAdW-Satzung (public-body cooperation) |
