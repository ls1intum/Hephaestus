...
- **Server logs** are subject to a two-layer cap: the Docker `json-file` log driver enforces a per-container size cap (`max-size=50m` × `max-file=5` for application services in `docker/compose.app.yaml`; `max-size=10m` × `max-file=3` for core infrastructure in `docker/compose.core.yaml`), and a host-level `logrotate` drop-in (`/etc/logrotate.d/hephaestus-docker-logs`, configuration tracked in `docker/logrotate/`) enforces a **hard 14-day age cap** via `daily` rotation, `rotate 14`, `compress`, and `copytruncate` (required because the Docker daemon keeps json-file logs open). Any change to `docker/logrotate/hephaestus-docker-logs` is a material change to the retention cap and must be reflected in the VVT and the privacy statement.
