...
- **Server logs** are subject to a two-layer cap: the Docker `json-file` log driver enforces a per-container size cap (`max-size=50m` × `max-file=5` for application services in `docker/compose.app.yaml`; `max-size=10m` × `max-file=3` for core infrastructure in `docker/compose.core.yaml`), and a dedicated `logrotate` sidecar service (defined in `docker/compose.app.yaml` as service `logrotate`, image built from `docker/logrotate/`, rotation rule inlined in the compose file's `configs:` block) enforces a **hard 14-day age cap** via `daily` rotation, `rotate 14`, `compress`, and `copytruncate` (required because the Docker daemon keeps json-file logs open). The sidecar ships with every deploy; there is no manual host-level configuration. Any change to the `logrotate-hephaestus-docker-logs` config block, the sidecar Dockerfile, or its entrypoint is a material change to the retention cap and must be reflected in the VVT and the privacy statement.
