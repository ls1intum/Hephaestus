...
- **Server logs** are bounded by a per-service storage cap (about 250 MB per application service, about 30 MB per core infrastructure service). Once the cap is reached, the oldest log entries are automatically discarded. The cap ships with every deploy as part of the per-service `logging:` blocks in `docker/compose.app.yaml` and `docker/compose.core.yaml`; there is no manual host-level configuration. Any change to those `logging:` blocks (raising the cap, or removing them) is a material change to the retention bound and must be reflected in the VVT and the privacy statement.
