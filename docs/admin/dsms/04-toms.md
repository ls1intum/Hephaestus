...
- **Web access logs** are generated by the embedded Tomcat inside the Spring Boot application server. Retention is enforced natively at 14 days. The log pattern is minimised to timestamp, client IP, method, path, protocol, status, response size, and processing time. Extending the retention window or widening the logged fields is a material change that must be reflected in the VVT and the privacy statement.
- **Container stdout** for each service is captured by the host's container runtime and rotated by size (50 MiB per file, 5 files retained per service). Stdout is not personal data by design; access is restricted to AET operators with container-host SSH.
...
| Leibniz-Rechenzentrum der BAdW (gitlab.lrz.de) | Source system and OIDC identity provider                             | **Not Art. 28** — separate controller; inter-public-body transmission under Art. 5 Abs. 1 Nr. 1 BayDSG, LRZ operating under the BAdW-Satzung (EDPB 07/2020 § 22) | Per-workspace, opt-in                     |
