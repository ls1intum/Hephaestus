...
Hephaestus is a practice-aware guidance platform for software projects, operated by the Research Group for Applied Education Technologies (AET, Prof. Krusche). The platform federates identities through Keycloak (GitHub OAuth + gitlab.lrz.de OIDC), synchronises repository activity from GitHub and gitlab.lrz.de, and engages the following external processors: an LLM provider selected at the deployment level and bound per workspace (Azure OpenAI by default on the TUM-operated deployment, or OpenAI) and Slack (when enabled per workspace). IP-bearing web access logs are generated natively by the Spring Boot application server and retained for 14 days via embedded Tomcat's built-in `max-days` retention; the log format is intentionally minimal (IP, timestamp, method, path, protocol, status, response size, processing time). Separate application runtime logs are rotated by Spring Boot / Logback with a 250 MB total-size cap.
...
- Are the retention figures in `03-vt-dsms.md` still matching the deployed config (14-day application-server access-log retention, Spring Boot runtime-log cap, whether off-host backups have been introduced, LLM provider retention windows)?
...
- _"Löschkonzept fehlt"_ → §13 of the VVT lists retention per category, including the account-deletion flow, the 14-day application-server access-log retention enforced natively by Spring Boot / Tomcat, and the capped Spring Boot runtime-log archive.
