...
Workspace administrators are **joint controllers** with TUM/AET under Art. 26 GDPR for the workspace-configurable decisions enumerated in privacy §3.2 (which Git repositories are connected, the practice catalog, the LLM provider and credentials, leaderboard / leagues / achievements, Slack routing). The Art. 26(2) Satz 1 allocation of duties and the Art. 26(2) Satz 2 essence of the arrangement are made available to data subjects via the privacy statement; TUM/AET is the single point of contact for data-subject rights, with the workspace administrator additionally addressable for workspace-specific questions.
