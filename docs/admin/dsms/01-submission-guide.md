...
1. Open `03-record-of-processing.md` alongside this guide.
2. Confirm the deployed configuration still matches the figures pinned in `03-record-of-processing.md` §13 (access-log retention, off-host-backup state, LLM-provider list, the disabled state of the Sentry / PostHog / Keycloak-SMTP integrations). Any drift triggers an amendment to the corresponding section before submission. The full amendment-trigger list is in [`05-processor-checklist.md`](./05-processor-checklist.md), "Follow-up if the VVT surface changes".
...
3. Copy **Title** and **Description and Purpose** from `03-record-of-processing.md` ("Step 1" block).
4. Select **Category**: `Administration / Teaching / Other`.
5. Click **Save**. DSMS redirects to the follow-up questionnaire.
6. Fill each follow-up field by copy-pasting from `03-record-of-processing.md` ("Step 2" block, §1 – §23). DSMS-label mapping:
   ...
   | DSFA                               | §17 (paste DPIA-light conclusion; full record in `02-dpia-prescreen.md`) |
   | Personalrat                        | §18                                                       |
   | IT-Sicherheit                      | §19                                                       |
   | Datenquelle                        | §20                                                       |
...
- _"DSFA erforderlich"_ — upgrade `02-dpia-prescreen.md` to the BayLfD DPIA template; the pre-screen already captures the residual-risk structure a full DPIA would elaborate and names the conditions under which a full DPIA must be opened (see §6 of the pre-screen).
...
2. Update `03-record-of-processing.md` and — if the substance changed — the privacy-page Markdown under `webapp/public/legal/profiles/tumaet/`.
3. Re-submit.
