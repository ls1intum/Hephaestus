...
A workspace administrator (typically a TUM chair, lecturer, or research-group lead) connects one or more Git repositories from github.com or gitlab.lrz.de. Hephaestus synchronises the pull/merge requests, issues, code reviews, review comments, and commit metadata authored in those repositories. Activity is ingested for everyone who has authored there, whether or not they have signed in to Hephaestus.

The synchronised activity is analysed against a workspace-configured catalog of practices to produce findings about each contributor. Two parts of the analysis use an external LLM provider configured per workspace. Automated practice review of pull/merge requests forwards the diff and relevant discussion to the provider and posts the AI-generated finding as a comment on the pull/merge request; this happens whether or not the author has signed in to Hephaestus. An in-app conversational mentor forwards contributors' follow-up questions and the relevant context to the same provider.

Contributors can sign in with their GitHub or LRZ-GitLab account to access their personal dashboard, the conversational mentor, and account preferences. In that case Hephaestus also receives a federated user identifier, username, display name, email, and avatar URL. The administrator can additionally enable a workspace leaderboard, league assignment, and achievements (off by default) and route notifications to Slack.
