...
| Server access logs (app server + reverse proxy) | Bounded by a per-service storage cap (about 250 MB per application service, about 30 MB per core infrastructure service). Once the cap is reached, the oldest log entries are automatically discarded. The cap is part of the deployment configuration itself (per-service `logging:` blocks in `docker/compose.app.yaml` and `docker/compose.core.yaml`) and travels with every deploy. Logs are retained beyond this rolling window only where strictly necessary for an ongoing security incident, then deleted at closure |
