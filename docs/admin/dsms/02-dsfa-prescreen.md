...
| Server access logs retain IP addresses | Low | Low | Size-bounded rolling window via the Docker `json-file` log driver (`max-size=50m`, `max-file=5` for application services; `max-size=10m`, `max-file=3` for core infrastructure), capped at approximately 250 MB per app service. Logs are not merged with other sources; only AET operators have access. A host-level time-based cap (e.g. `logrotate` with `rotate 14`) is an open operational item. |
...
- **Size-bounded rolling window** on server-log retention via the Docker `json-file` log driver, capped per container (~250 MB per app service). Raising these caps materially, or retaining logs off-container beyond this window, is a material change.
