...
| Server access logs retain IP addresses | Low | Low | Fixed-size buffer per service: 250 MB (5 × 50 MB) for application services in `docker/compose.app.yaml`; 30 MB (3 × 10 MB) for core infrastructure in `docker/compose.core.yaml`. When the buffer fills, the oldest log file is overwritten. Effective rolling-window duration is volume-dependent (measured per submission, see `SUBMISSION-GUIDE.md` Phase 0); the small buffer ensures the window stays bounded under any traffic level. Logs are not merged with other sources; only AET operators have access. |
...
- **Bounded server-log retention** via a fixed-size buffer per service (250 MB / 30 MB; configured in the `logging:` blocks of `docker/compose.app.yaml` and `docker/compose.core.yaml`). Raising the buffer size, removing the `logging:` blocks, or retaining logs off-container beyond the buffer is a material change. Traffic-driven changes to the effective rolling-window duration are reported per submission (see `SUBMISSION-GUIDE.md` Phase 0) and are not material changes.
