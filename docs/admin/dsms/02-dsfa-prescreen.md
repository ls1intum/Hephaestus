...
| Server access logs retain IP addresses | Low | Low | Two-layer cap: per-container size cap via the Docker `json-file` log driver (`max-size=50m` × `max-file=5` for application services; `max-size=10m` × `max-file=3` for core infrastructure), plus a **hard 14-day age cap** enforced by a dedicated `logrotate` sidecar service defined in `docker/compose.app.yaml` (image built from `docker/logrotate/`, rotation rule inlined as a compose `configs:` block so it ships with every deploy). Logs are not merged with other sources; only AET operators have access. |
...
- **Hard 14-day age cap** on server-log retention via the `logrotate` sidecar service (`logrotate:` in `docker/compose.app.yaml`, image `docker/logrotate/`, rotation rule inlined in the compose file's `configs:` block), complemented by per-container size caps on the Docker `json-file` log driver. Raising the 14-day cap, removing the sidecar, or retaining logs off-container beyond this window is a material change.
