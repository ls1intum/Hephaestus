...
| Server access logs retain IP addresses | Low | Low | Deterministic per-container size cap enforced by the Docker `json-file` log driver: `max-size=50m` × `max-file=5` (≤ 250 MB per service) for application services in `docker/compose.app.yaml`; `max-size=10m` × `max-file=3` (≤ 30 MB per service) for core infrastructure in `docker/compose.core.yaml`. Once a container exceeds its cap, the daemon discards the oldest log file. Logs are not merged with other sources; only AET operators have access. |
...
- **Deterministic per-container size cap** on server-log retention via the Docker `json-file` log driver (`max-size` × `max-file` configured in `docker/compose.app.yaml` and `docker/compose.core.yaml`). Raising the size cap, removing the per-service `logging:` blocks, or retaining logs off-container beyond this window is a material change.
