...
- **Hard 14-day age cap** on server-log retention via the host-level `logrotate` drop-in (`docker/logrotate/hephaestus-docker-logs`), complemented by per-container size caps on the Docker `json-file` log driver. Raising the 14-day cap, disabling the `logrotate` drop-in, or retaining logs off-container beyond this window is a material change.
