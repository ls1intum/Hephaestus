...
| Server access logs retain IP addresses | Low | Low | Bounded by a per-service storage cap (about 250 MB per application service, about 30 MB per core infrastructure service; configured in `docker/compose.app.yaml` and `docker/compose.core.yaml`). Once the cap is reached, the oldest log entries are automatically discarded. Logs are not merged with other sources; only AET operators have access. |
...
- **Bounded server-log retention** via a per-service storage cap (configured in the `logging:` blocks of `docker/compose.app.yaml` and `docker/compose.core.yaml`). Raising the cap, removing the `logging:` blocks, or retaining logs off-container beyond this rolling window is a material change.
