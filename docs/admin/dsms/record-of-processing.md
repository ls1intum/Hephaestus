...
- Authored repository artefacts (issues, pull/merge requests, comments, reviews) synchronised from GitHub / gitlab.lrz.de and their practice findings: retained while at least one workspace monitors the source repository. Removed when the last workspace stops monitoring it, when the last relevant GitHub or GitLab connection is disconnected, when the workspace is purged, or on verified erasure.
- Workspace memberships, AI conversations, and recognition signals: retained with the relevant workspace records. Removed when those records or the workspace are purged, or through operator-executed deletion or anonymisation on verified erasure. Content mirrored from Slack and from Outline is likewise removed when the corresponding connection is disconnected or the workspace is purged.
