...
```text
Self-hosted by AET at https://hephaestus.aet.cit.tum.de on AET-administered infrastructure at TUM. Application data — including authentication state (accounts, federated identity links, the cookie-session revocation list, and the auth-event log) — in PostgreSQL, which also holds the practice-review job queue; webhook and integration-sync events in NATS JetStream. Local working copies of monitored repositories may be stored on the host filesystem when practice-review code execution is enabled. Container stdout goes to the Docker json-file driver. Every service in the stack sets an explicit rotation cap in the compose files: 50 MiB per file × 5 files for the webapp, the release-pin init job, the application server, the worker and PostgreSQL; 10 MiB × 3 for the webhook receiver, NATS, the reverse proxy and the maintenance page. The application layers write no HTTP access log — Tomcat's is explicitly disabled in the production profile and the Traefik reverse proxy is not started with `--accesslog` (Traefik's default is off), so neither records a client IP address against a request. The nginx containers that serve the static frontend (`webapp`) and the maintenance page do write one combined-format line per request to stdout, and that line is a request-level record: it carries the request URL, referrer, user agent, status and timestamp. Because a deep link can name a person (`/w/<slug>/user/<username>`), a contributor's login can appear in it. It carries no contributor IP address: no `real_ip` module is configured, so the address logged is the reverse proxy's internal container address, not the client's. These lines live only in the container-stdout rotation window above and are never ingested elsewhere.
...
- Contributor profile (login, name, email, avatar) and authored repository artefacts (issues, pull/merge requests, comments, reviews) synchronised from GitHub / gitlab.lrz.de, plus AI conversations, recognition signals, and practice findings generated by the platform: retained while at least one workspace continues to track the contributor's repositories. Removed when the last workspace stops monitoring the source repository, when that workspace's GitHub or GitLab connection is disconnected, when the workspace is purged, or on operator-executed deletion against the production database on receipt of a verified erasure request. Content mirrored from Slack and from Outline is likewise removed when the corresponding connection is disconnected or the workspace is purged.
...
- Container stdout, including the per-request nginx access lines described under "Where stored" (request URL — which can name a contributor — referrer, user agent, status, timestamp; no contributor IP address): rotated by size by the container runtime, per the per-service caps described there. There is no time-based expiry; a line survives until the rotation window displaces it.
```
...
- Container stdout, including the nginx per-request access lines: rotated by size by the container runtime (per-service caps under "Where stored") and not selectively prunable — an erasure request cannot reach inside the rotation window, which displaces the lines on its own.
...
- From the HTTP connection: the nginx containers serving the frontend and the maintenance page write one access line per request (URL, referrer, user agent, status, timestamp) to container stdout, retained only for that stream's size-based rotation window. No contributor IP address is recorded there, and Tomcat's and Traefik's access logs are disabled. The source IP address and user agent are collected as such only for authentication events, and are retained under the auth-event log's own 12-month window described above.
```
