...
```text
Hephaestus exposes no in-app data-export control and no API endpoint for self-service export. Subject access and portability requests are fulfilled by AET operators on a verified request: the operator compiles the personal data the platform holds about the data subject — from the application database (account, federated identity links, preferences, auth-event log), the analytics integration where activated, and access-log entries within the 14-day retention window — into a single JSON archive and delivers it to the verified address. Source-side content on GitHub or gitlab.lrz.de is exported by those source platforms, not by Hephaestus. Identity verification, response timeframe, and contact path are the same as for erasure.
```

**Deletion guarantee**

```text
- The Hephaestus account, federated identity links, and (where PostHog analytics is activated; off by default in the TUM-operated deployment) the corresponding analytics identity: removed by the in-app account-deletion control.
