...
- **No HTTP access log.** Tomcat's access log is disabled in the production profile and the Traefik ingress runs without `--accesslog`, so request-level IP/URL data is never written at either layer. Enabling it at either layer is a material change.
