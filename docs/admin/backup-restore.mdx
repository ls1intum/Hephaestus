...
- **NATS JetStream:** the stream on `nats-data` may hold events the restored database has
  ...
  from an old backup: `dc down`, `docker volume rm <project>_nats-data` (named like the
  `_postgresql-data` volume above), `dc up -d` — events in the gap are re-fetched by the
  scheduled sync.
