...
1. Clone this repository to `/var/lib/hephaestus/checkout`. It belongs under the state directory
   because the reconciler fetches into it; the unit makes only that directory writable, so a
   checkout anywhere else needs a matching `ReadWritePaths` entry.
