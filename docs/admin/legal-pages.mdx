...
`<page>` is `imprint` or `privacy`. The pages are English-only; if you need a localised version, mount the appropriate language into the override layer. A banner alerts end users whenever the disclaimer is being served.
