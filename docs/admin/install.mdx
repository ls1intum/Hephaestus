...
```bash
APP_HOST_MATCH=Host(`hephaestus.example.com`,`old.example.com`)
```
...
configured for it, and a browser arriving on any other name is redirected there, so a session only
ever exists on one host — which is also why the OAuth callback URLs stay on `APP_HOSTNAME` and the
other names never need their own.
