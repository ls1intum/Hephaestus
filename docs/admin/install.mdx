...
- Outbound HTTPS to `ghcr.io` and `docker.io` (images — NATS and Traefik come from Docker Hub),
  `github.com` (release pin), `api.github.com`, and `fulcio.sigstore.dev`, `rekor.sigstore.dev`,
  `tuf-repo-cdn.sigstore.dev` (the `release-pin-fetcher` verifies the pin with keyless cosign, which
  reaches Sigstore, not just GitHub). On an egress-filtered host, a missing Sigstore domain shows up
  as the release-pin step failing even though `github.com` is reachable.
...
(`AGENT_ENABLED`, `GIT_CHECKOUT_ENABLED`, `PRACTICE_REVIEW_FOR_ALL` — set together; the last is a
compose-file alias that fans out to `PRACTICE_REVIEW_RUN_FOR_ALL` and
`HEPHAESTUS_FEATURES_FLAGS_PRACTICE_REVIEW_FOR_ALL`, which is what the server reads; the job
queue itself runs on PostgreSQL, no NATS setup needed for this bundle), **and** a model that the
workspace can actually run on. There are two ways to supply one, and either is enough: an instance
admin registers a connection and model under **Instance admin → AI models** and grants it to the
workspace, or — if instance settings permit bring-your-own connections — a workspace admin adds their
own connection and model under the workspace's **Administration → AI models**. Either way, a workspace
admin must then bind a model to the **practice detection** purpose on that same page. With the flags
on but nothing bound, nothing is reviewed: webhook-driven reviews are skipped with only a DEBUG log
line, since an unbound workspace is a normal state and one line per event would be noise. The manual
dev trigger answers with the reason, so use it to tell "unbound" apart from "broken".
...
| Encryption-key error mentions **bytes**, not characters | The key must be 32 *bytes*. A 32-character key containing non-ASCII is longer than 32 bytes; regenerate with the ASCII command above. |
