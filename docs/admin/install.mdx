...
migrating away from, or a `www.` variant — name every one of them in `APP_HOST_MATCH`, which is
handed to the reverse proxy as a host matcher:
...
resolve here holds up the whole certificate.
