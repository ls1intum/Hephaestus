...
move. One consequence is worth knowing — a sign-in *started* by opening `/api/login/...` on an
older name fails rather than completing, because the provider returns to `APP_HOSTNAME`, where the
state cookie set on the other name is not readable. Reaching the instance the normal way is
unaffected, since the redirect happens before any sign-in begins.
