...
:::info
**Who can see this.** Workspace admins can see your practice status to support mentoring, and every detailed view is recorded. The same line appears on the page itself.
:::
...
## How the aggregate counts protect people

The workspace health counts are aggregate by design: people per status, never named. The protection is enforced by the server. When it serves the aggregate to a regular member, an area's counts stay hidden until five or more developers are active there, and the same rule applies when any single status bucket is small enough to identify someone. An area with zero recent activity reads "No activity in this area yet", which is a different state from suppression.

On the practice overview page, administrators and owners see full counts. That is deliberate: they already see every developer by name on the roster directly below, so hiding their aggregate would protect nobody.
