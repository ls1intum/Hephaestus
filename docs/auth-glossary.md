...
| **`AuthEvent`** | Append-only auth / impersonation event. Monthly RANGE-partitioned on `occurred_at`, self-managed in-app by `AuthEventPartitionManager` (create-ahead + 12-month retention) on stock Postgres — no `pg_partman`. Records `(account_id, acting_account_id)` for every impersonation (`act`-claim) action. INSERT-only at the SQL-grant level in non-test environments. | `core.auth.audit.AuthEvent` | composite `(id, occurred_at)` (Postgres partitioning requires it) |
