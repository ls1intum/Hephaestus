...
| **`AuthEvent`** | Append-only auth / impersonation event. Monthly RANGE-partitioned on `occurred_at`, self-managed in-app by `AuthEventPartitionManager` (create-ahead + 12-month retention) on stock Postgres — no `pg_partman`. Records `(account_id, acting_account_id)` for every `SwitchUserFilter`-bound action. INSERT-only at the SQL-grant level in non-test environments. | `core.auth.audit.AuthEvent` | composite `(id, occurred_at)` (Postgres partitioning requires it) |
| **`IssuedJwt`** | Revocation list for Hephaestus-issued cookie JWTs. Inserted at issuance; consulted by `RevocationAwareJwtDecoder` on every request via a Caffeine-cached view. NATS subject `auth.jwt.revoked` propagates invalidations cross-pod (Caffeine TTL is safety-net). | `core.auth.jwt.IssuedJwt` | `jti` (UUID) |
...
| **`Connection` of `kind=OIDC_LOGIN_*`** | A workspace-scoped OAuth login provider (self-hosted GitLab, GHE). Rides PR #1306's `Connection` aggregate; the new sealed `OidcLoginConfig` permit holds `(issuerUrl, clientId, scopes, displayName)` and the secret rides `CredentialBundleConverter` via the new `OAuthClientSecret` bundle variant. **Not a separate table.** | `integration.core.connection.Connection` (kind discriminator) | natural key `(workspace_id, kind, instance_key)` |
