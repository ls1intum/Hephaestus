...
| **`JwtSigningKey`** | Hephaestus's own ES256 JWT-signing key set. One active key signs every token; a retired key still verifies until its row is removed. Private keys are sealed under the **system** master key (AAD = `system:jwt_signing_key.private_key_pem`) — distinct from the tenant-bound AAD domain used by `CredentialBundleConverter` for per-workspace integration secrets (confused-deputy defense). | `core.auth.jwt.JwtSigningKey` | `kid` |
