...
| **`AuthEvent`** | Append-only auth / impersonation event. Monthly-partitioned via `pg_partman`, 12-month retention. Records `(account_id, acting_account_id)` for every `SwitchUserFilter`-bound action. INSERT-only at the SQL-grant level in non-test environments. | `core.auth.audit.AuthEvent` | composite `(id, occurred_at)` (Postgres partitioning requires it) |
